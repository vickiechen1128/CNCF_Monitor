...
## 基线差异分析前置步骤（v2026-09-17 起，BLOCKING）

前端开发开工第一步，**必须按轨道读基线 → 差异认知 → 再动手**：

| 轨道 | 第一参照物 | 核对方式 |
|------|-----------|---------|
| **Track A** | 原型页面 + `frontend-prototype-map.md` | 按 Planner 产出的差异矩阵（S/A/B 分级）逐项核对原型与当前实现；S 级未解决差异必须停止并报告 Orchestrator |
