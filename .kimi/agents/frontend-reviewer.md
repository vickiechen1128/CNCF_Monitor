...
- **轨道标记**：Track A / Track B / Track B+（来自分轨判定记录；决定审查基线参照物）
- **前端原型映射表：`docs/05-execution-records/module-XX/frontend-prototype-map.md`（Track A 前端任务必读取；核对原型符合度的第一靶子；Track B/B+ 不要求）**
- L3 micro-task 序列：`docs/05-execution-records/module-XX/task-sequence.yaml`（缺失不阻断，见下）
- 原型路径：`docs/prototypes/module-XX/`（Track A 按任务卡 `prototype_pages` 读取，如缺失不阻断；Track B/B+ 无原型不阻断）
...
| 原型符合度 | **（Track A 专属）** 以 `frontend-prototype-map.md` 为第一核对靶子，按任务卡 `prototype_pages` / `ui_contract` / `nav_contract` / `clipping` 逐项核对：①**导航模型**——顶部一级 tab / Sider 二级文案是否与 `nav_contract` 一致，是否误用功能页名充当一级模块名；②**列 / 区块完整性**——实现列集合是否 = 原型列集合 ∩ MVP，删减项是否已在 `clipping` 登记理由；③**UI 展示名**——页面字段标签是否用 PRD 字段表「UI 展示名」+ 原型用户语言，禁止直接把 snake_case 字段名当文案；④**用户文案**——可见文案是否出现模板 ID / 内部枚举 / 模块代号等技术术语；⑤**交互组件**——组件选型与 `02_Frontend_Standard.md` 第 8-9 章一致（禁止散点手写 `maxWidth`、Popover 承载大列表）；⑥**冲突 / 偏离**——原型与 PRD 不一致，或实现偏离 `frontend-prototype-map.md` 时，实现侧是否报告 Orchestrator 并留痕，还是自行二选一。**Track B/B+ 不要求原型符合度，改为验收清单覆盖度 + AntD 标准模式符合性** |
| 契约一致性 | **（两轨通用，v1.2 起以契约快照为基准，v2026-09-17 起固化为强制项）** 前端类型 / 字段名（snake_case）/ 枚举值 / 响应结构与 **`api-contract-snapshot.md`** 是否一致；快照缺失时回退 **PRD 第 5/6 章 + `03_API_Standard.md`**；禁止以 `platform/models/*.go` 为实现依据（并行开发时后端未实现，抄对端代码是最高频翻车点）。**不一致必须标记为 HIGH** |
