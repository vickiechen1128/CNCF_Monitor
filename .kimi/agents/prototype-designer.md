...
```text
0. 需求背景与典型场景 （用户/产品经理阅读入口：业务痛点、典型场景、涉及用户故事编码）
1. 模块目标          （用户价值导向，讲业务价值）
...
7. **章节编号冻结（v1.27 起）**：上述 0–11 章编号为全局固定，**禁止模块间漂移、增删、换序**；模块不适用的章节保留标题并标注「（本模块不涉及）」。理由：frontend/backend agent 按固定章节号做章节级读取，编号漂移会使读取协议失效（历史教训：Module_07 与 Module_09 章节号不一致，章节级读取规则对二者无法同时成立）。既有模块 PRD 在下一次迭代时渐进迁移对齐，不做一次性大改写。
...
10. **§0 需求背景与典型场景（新增，v1.32 起）**：在模块目标之前插入业务叙事层，面向产品经理 / 新工程师快速理解「这个模块解决什么问题」。内容要求：
    - 用业务语言描述痛点，不出现数据模型 / API / 技术字段；
    - 包含 2-3 个典型场景（角色 / 触发条件 / 用户目标 / 成功标准）；
    - 列出本模块覆盖的用户故事编码（与第 2 章一致）；
    - 链接到 `01_User_Stories.md` 全局库。
