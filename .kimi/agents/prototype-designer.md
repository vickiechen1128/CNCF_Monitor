...
> **章节级读取（v1.24 起，控制上下文）**：读取当前模块 PRD 时按章节选择性读取——**必读**：3 核心功能（用户层，**含其下全部子节 3.N.M**）、4 核心流程、5 数据模型（技术层）、6 接口设计、8 数据模型状态机、9 验收标准；**按需**：1 模块目标、10 术语映射、Change Log（完整历史见 design-decisions.md）。
...
7. **章节编号冻结（v1.27 起）**：上述 0–11 章编号为全局固定，**禁止模块间漂移、增删、换序**；模块不适用的章节保留标题并标注「（本模块不涉及）」。理由：frontend/backend agent 按固定章节号做章节级读取，编号漂移会使读取协议失效（历史教训：Module_07 与 Module_09 章节号不一致，章节级读取规则对二者无法同时成立）。既有模块 PRD 在下一次迭代时渐进迁移对齐，不做一次性大改写。**子节编号（v1.34 起，与要求 12-5 配套）**：允许在既有 `### 3.N` 之下新增 `#### 3.N.M` 编号子节（按「用户可见能力」粒度拆分），并为其连续编号；**一级 0–11 编号与既有 `### 3.N` 编号不得改动、不得重排**——子节编号一经发布即成为对外引用锚点，确需重编号时必须在同一轮内完成全文锚点回写。跨模块**不要求** `### 3.N` 的子集一致（M07 与 M09 的 3.N 集合本就不同），但同一主题的跨模块命名与归属口径须一致（见要求 12-6）。
...
9. **前端交互契约章节（第 11 章，v1.27 起，v1.39 增补页面子节模板）**：原型代码是页面布局与交互流程的权威参考，PRD 对前端的价值是补原型所不能。本章由「两张总表 + 每页子节」构成：
   - **页面状态矩阵**（11.1）：页面 / 区块 × 状态（加载中 / 空态 / 接口错误 / 权限不足 / 数据超量与边界）→ 表现与文案；
   - **全局行为规则**（11.2）：轮询间隔与退避、破坏性操作二次确认口径、表单校验提示位置、提交中防重复等。
   - **页面子节（11.3~11.N）固定六段模板（v1.39 起强制）**：每个 `### 11.N <页面名>` 子节必须且只能按下列六段组织，本页不适用的段落显式标注「（本页不涉及）」：
     ①**用户任务**：一句话回答「用户在这个页面要完成什么任务」（如「确认某网域的配置变更是否生效」），**必须作为子节正文第一行**；
     ②**入口与路径**：用户从哪进入本页（菜单 / 深链 / 跨模块跳转来源）；
     ③**布局与列设计**：主区展示什么、分几列 / 几块、每块的用户可见信息；
     ④**关键交互规则**：按钮点击行为、禁用 / 隐藏条件、抽屉 / 弹窗开合、分页 / 筛选 / 排序规则；
     ⑤**状态与边界**：本页**特有**的状态处理；与 11.1 矩阵一致的不再复述，写「其余状态见 §11.1」；
     ⑥**跨模块跳转**：跳转到其他模块的入口位置、携带参数、目标页；一句话即可，完整调用时序图见 §4。
   - **页面子节三禁（v1.39 起，M09 §11.4/§11.5 可读性事故固化）**：①**禁止以设计理由开头**——「为什么不采用 X 形态」「与 YY 模块的区别」等取舍论证一律迁 `design-decisions.md`（历史教训：M09 §11.4 开头用整段论证「不采用 M06 点阵形态」，读者误以为是列设计说明）；②**禁止解释字段语义**——字段取值含义、枚举定义、取值来源属 §5 / §8，§11 只写「展示成什么文案 / 什么颜色 / 配什么按钮」并留指针（如「状态取值语义见 §8.3」）；③**禁止承载功能价值论述**——「该能力需要可观测 / 为什么需要」属 §3，§11 只写「怎么呈现 / 怎么操作」（历史教训：M09 §11.5 以「检测状态可观测 {P1}」开头，该句是功能价值而非页面交互）。**违反后果**：读者无法判断这段文字是在说「页面长什么样」「为什么这样设计」「字段什么意思」还是「后端怎么实现」——§11 因此沦为杂炖锅。
   - **页面子节机检约束（门禁 `scripts/check-prd-hygiene.sh` 检查 6j）**：①必须以「用户任务」段开头；②单节 **≤50 行**——超过说明混入了非 UI 内容，须拆分归位；③字段名反引号出现 **≤5 处**——超出说明字段语义未归 §5；④**不得出现状态流转箭头描述**（如 `pending → confirmed`）与 `stateDiagram` 之外的状态机铺陈，流转属 §8。**门禁口径**：存量模块 FAIL 属「迁移进度指标」不阻断提交，**本轮新改动或新注册模块必须达标**。
...
11. **规格归位与正文格式纪律（v1.33 起，M09 PRD 瘦身实践固化）**：
    - **单一权威章节**：同一规格主题只在一个章节承载权威版本——功能 / 用户层 → §3，字段语义 → §5，接口 / 协议 → §6，跨模块依赖边界 → §7，状态流转 → §8，验收 → §9，术语 → §10，页面状态与全局行为 → §11；其余章节至多保留一行引用（如「字段定义见 §5.2」），**禁止多处全文复述**（历史教训：M09 命名空间 / go-no-go 审批口径散布 3-4 章，决策落地时漏改）。**§11 的边界细化（v1.39 起，与要求 9 页面子节模板配套）**：§11 **只承载页面交互规则**（按钮 / 入口在哪里、什么条件下可用与禁用、跳转到哪、列与区块如何组织、状态如何呈现）；**不承载**——功能价值论述（→ §3）、跨模块调用时序（→ §4）、字段取值语义与枚举定义（→ §5 / §8）、设计取舍论证与历史演变（→ `design-decisions.md`）。判定口诀：**§11 回答「页面怎么用」，其余章节回答「是什么 / 为什么 / 数据怎么流」。**
    - **引用块纪律**：正文 `>` 引用块仅限「备注、一句话引用、指向权威章节」；**禁止在引用块中承载规格正文、表格、枚举清单或决策叙述**——引用块不在章节级读取的预期路径上，AI 与读者都易漏读。既有超限引用块在下一次迭代时拍平为正文或归位到权威章节。
    - **决策史零内嵌**：定版论证、演进选项讨论（如「http_sd 当前不采用，因为…」）、结构变更史、审批 / 职责重构过程，一律写 `design-decisions.md`；正文只留当前结论（与要求 8 同源，此处明确覆盖叙述性内容）。
    - **子标题编号强制**：`####` 及以下层级必须与父章节连续编号（如 3.11.1 / 3.11.2），**禁止无编号小标题**；正文标题层级不宜深于 `####`（四级），更深内容用表格或列表承载。
    - **图示统一 Mermaid**：PRD 内流程图 / 时序图 / 架构图统一用 Mermaid 代码块，**禁止 ASCII 字符画**（GitHub 可渲染、对 AI 读取 token 友好）；既有 ASCII 图在下一次迭代时转换。
    - **门禁既有口径（不改）**：章节表格「来源」列使用 `决策 NN`、正文 `{MVP}` / `{v0.x}` / `{v0.x+}` 交付范围标记均属既定口径，**不视为**「内联决策标注 / 内联版本标记」违规，门禁正则与验收口径维持现状（v1.35 起把 `{MVP}` 明确纳入该口径；v1.36 起**裸文字形态**由下条「版本差量形态」禁止）。
    - **版本差量形态（v1.36 起，全文强制；对齐 `02_Product_Roadmap.md` §0 关联规则）**：凡表示「某能力 / 字段 / 边界在哪个产品版本交付或生效」的**版本差量标注**，一律写**花括号形态**——`{v0.2}` / `{v0.2+}` / `{v0.3}` / `{v0.4+}` / `{v1.0+}`，**禁止裸文字形态**（`v0.4+`、`v0.2 起`、`（v0.3）` 等）。**MVP 边界（与要求 12-5 一致，不算违规）**：`MVP` 是**阶段词、不带版本号**，在「§3 交付版本列」与区间 / 序列表述（`MVP` 行、`MVP~v0.3`）中一律保持**裸文字**形态（要求 12-5 已明文要求 MVP 行统一写 `MVP`），花括号只用于**带版本号**的阶段标签与复合验收标记（`{P0 / v0.2}`）。**软件版本号不在本条管辖范围**：`"version": "v0.25.0"`、`v1.101.0`、`v1.2.0` 等 semver 令牌不是产品版本标注，须用尾随断言 `(?![\d.])` 排除，误改会破坏配置样例（Module_09 实测拦下 1 处）。要求 12-5 已对 §3 交付版本列强制此形态，本条将其推广至**全部章节**。**例外（保持既有形态、不算违规）**：①**头部元信息枚举行**（`产品版本覆盖: MVP / v0.2 / v0.3 / v1.0`）——是覆盖范围枚举而非差量标注；②**章节标题内的版本括注**（如 `5.16.4 网域归属解析链（v0.3 / v0.4）`）——改标题会改变 GitHub slug 并破坏跨文件锚点，收益低于风险；③**交叉引用 / 文档自身版本号**（`PRD 版本: v2.36`、`原型版本: v2.36`）；④**§9 验收标准章节内的版本标注**（含复合标记 `{P0 / v0.2}`、`{P1 / v0.4+}` 形态）——L226 已声明「验收口径维持现状」，且验收标记是发布门禁的比对基线，改动会改变验收语义。**实现要点（务必照做）**：规范化时须按「**花括号区间保护**」处理——先框出 `\{[^}]*\}` 区间，只替换区间**外**的裸文字，否则会把已有的复合标记 `{P0 / v0.2}` 误改成 `{P0 / {v0.2}}`（Module_07 首次 dry-run 实测 4 处）。**为什么强制**：门禁正则 `\{v[0-9]+\.[^}]+\}` 与「交付范围标记」计数器**只保护花括号形态**，裸文字不在计数范围内 —— 瘦身时会被当冗余散文清掉（Module_07 v2.36 实测：§3 内 5 处版本标注因裸文字形态丢失）；且裸文字与花括号混用会造成跨模块形态不一（M01 散在子节标题、M09 散在单元格说明、M07 曾散在字段说明）。**Module_07 v2.36 已按本条把 §3 之外的 57 处 / 50 行规范化**（涉及 §5 / §6 / §8 / §10 / §11；例外未动：头部元信息块、子节标题内括注 `5.16.4 网域归属解析链（v0.3 / v0.4）`、§9 验收整章、Change Log）；**Module_09 v1.75 同版补充已把 §3 之外的 63 处 / 52 行规范化**（涉及 §1 / §4 / §5 / §6 / §8 / §10 / §11，另拦下 1 处 semver 与 1 处交叉引用章节名 `design-decisions「v0.4+ 演化影响备忘」`），并按要求 12-5 为 §3 的 21 张功能表补齐「交付版本」列（21 表 / 100 行；取值 `MVP`×51 / `{v0.2}`×32 / 差异标注×6 / `后续版本`×7 / `{v1.0}`×3 / `{v0.2+}`×1）。
12. **§3 核心功能防叠加纪律（v1.34 起，M09 §3 可读性事故固化）**：
    - **说明列禁技术实现物**：§3 功能表"说明"列**禁止出现** code fence（\`\`\`yaml/json）、API 路径（`GET/POST/PUT/DELETE /api/...`）、环境变量名或 CLI 参数名（如 `CONFIG_CHANGE_DETECT_MIN_INTERVAL_SECONDS`、`--web.enable-lifecycle`）。这些属于技术层细节，应归位到 §5（数据模型字段注释）/§6（接口设计）/§8（状态机）/design-decisions.md（定版论证）。AI 章节级读取协议（v1.24）本就会去这些章节查找技术信息——写在 §3 反而三输（PM 读不懂、AI 不在这找、增加噪声）。
    - **禁否定式演变词表**：§3 正文**禁止出现**以下表述——「不再」「原先」「原列名」「替代原」「此前只」「不再单列」「历史表述」。每轮决策回写若只追加增量而不重写现状句，会形成叠加式演变化石层（M09 实测：15 处残留）。正确做法：直接陈述当前结论（如「列表展示 X、Y、Z 三列」而非「原列名 A 已替换为 X」）。
    - **单元格长度上限**：功能表每个单元格（含「说明」列）**目标 ≤300 字符、硬上限 400 字符**（300 字符 ≈ 100 汉字为可读目标；密集规格行允许到 400。M09 实测 Top5 = 2016/1308/1198/919/802 字符，一格 ≈ 一页纸，可读性极差）。超长内容拆出处理方式——用户可见行为要点归 §3 同一子节内的要点列表、字段语义细节归 §5、接口细节归 §6、页面行为细则归 §11。
    - **决策回写规则**：当决策变更影响 §3 描述时，**整句重写为当前结论**，禁止在旧句后追加「（2026-08-19 决策：…）」或「不再…改为…」等补丁式标注。决策理由与旧版描述迁 design-decisions.md。
    - **子功能统一模板**：每个 `### 3.N` 子节下的功能表遵循固定结构——一句用户价值（人话，回答「这个功能给用户什么价值」）→ 3~5 条行为要点（用户可感知的操作与结果，非实现步骤）→ 优先级（P0/P1/P2）→ **交付版本**（v1.35 起强制）。禁止按「技术能力」组织子功能（如「配置生成引擎」下列举 configgen 内部步骤），应按「用户可见页面/能力」组织。
    - **交付版本列与阶段标签形态（v1.35 起，对齐 `02_Product_Roadmap.md` §0 关联规则）**：§3 功能表必须含「**交付版本**」列，采用**差异标注法**——MVP 内即交付的行统一写 `MVP`；**存在跨版本差量的行必须写成花括号阶段标签**（`{v0.2}` / `{v0.3}` / `{v0.4+}` / `{v1.0+}`），多轴差量可并列（如 `MVP；变更单联动 {v0.2+}`）。该轴必须与三处口径一致：§1「MVP 边界」、Change Log「产品版本影响」列、`02_Product_Roadmap.md` §1.5 功能-版本矩阵。**为什么强制**：优先级（P0/P1/P2）与产品版本是**两个正交轴**（Roadmap §1.5 末注——矩阵只标版本是否包含某模块能力，模块内优先级另见各模块 PRD），只写优先级会让「哪个版本交付」在 §3 内不可见；且**裸文字**「MVP / v0.4+」不在门禁正则 `\{v[0-9]+\.[^}]+\}` 的计数范围内，瘦身时会被当冗余散文清掉（Module_07 v2.36 实测：§3 内 5 处版本标注因此丢失，修订后才恢复）。**故阶段标签一律用花括号形态书写，裸文字形态视为不合规。**
    - **跨子节主题单一权威化**：同一主题（如 WAL 参数、诊断看板、通道规则、空态引导）**只在一个 3.N 子节承载权威描述**，其余位置最多保留一句话 + 内链（如「WAL 参数详见 §3.1」）。M09 实测教训：WAL 散落 3.2/3.10/3.8.2 三处、诊断看板散落 3.2/3.8/3.8.2 三处、通道规则散落 3.1/3.9/3.11.1 三处。
    - **§3 技术产物枚举禁令（v1.40 起，M09 §3.3/§3.4 说明列混用产物清单事故固化）**：§3 功能表「说明」列及子节正文**禁止枚举具体产物文件名或工具命令**——如 `prometheus.yml` / `targets/*.json` / `rules.yml` / `blackbox.yml` / `metadata.json` / `promtool check config` 等。产物清单、配置文件结构、校验工具链归 §6 产物规则 / §5 数据模型 / `api-contract-snapshot.md`；§3 只写用户可见行为与结果（如「预览受影响的配置文件集」「校验失败阻止发布」），并用一句话 + 指针指向 §6。**判定口径**：说明列中出现 `*.yml` / `*.json` / 反引号包裹的工具命令 / 文件路径 ≥2 处即触发归属审计。独立参数表（如 WAL 默认值表）同样不得在 §3 承载，应归 §5 字段说明或 §6 运行时行为。
13. **核心章节形态纪律（v1.37 起，M07 §5 体积失衡 + M09 §1 引用块臃肿事故固化）**：要求 11/12 解决了「§3 层放错」与「§3 防叠加」，本条把同一套「章节级形态」约束覆盖到**技术层五章（§1 / §4 / §5 / §7 / §8）**。诊断口径：先算**内容形态构成**（表格行 / code fence / 引用块 / 标题 / 散文各占字符比）与**单章占比**，再定改造方案——**体积失衡通常不是内容多，而是形态与归属错位**。
    - **13-1 §1 模块目标**：固定三段结构——①**一句话定位**（正文散文，说明本模块在平台中的层次与存在理由）；②**核心职责**编号清单（`N. **名称**：一句话描述`，3~9 条，正文承载）；③**MVP 边界**（正文列表：做什么 / 不做什么 + 由谁承接，可含 `{v0.x}` 阶段边界）。**禁止用引用块承载职责清单、边界枚举或 MVP 边界**——引用块仅保留 `> 决策依据：` 一行（与要求 8 一致）；引用块不在章节级读取路径上，把职责写进去 = AI 与读者都漏读。**跨模块边界声明的唯一权威在 §7**（见 13-4）：§1 写「与 Module_XX 的边界」段落并与 §7 复述即违反要求 11 单一权威章节，§1 至多保留一行指针。实测：M07 §1 引用块占 53%（709/1,327 字符）、M09 §1 占 50%（1,702/3,398 字符）；M07 §1「与 M01/M09 的边界」与 §7 重复。**M09 §1 另需警惕把工程 / 部署细节（如部署目录规范、systemd 注册）写进本章**——此类内容归 §6 或 `design-decisions.md` 或专项工程文档。
    - **13-1-bis 核心职责条目长度**：每个编号职责条目**不超过 80 字符**；若需要说明实现细节，条目正文只保留「谁/做什么」，细节下放 §6 或 `design-decisions.md`。
    - **13-1-ter MVP 边界段落形态**：MVP 边界只写「做 / 不做 / 由谁承接」三列（可含 `{v0.x}` 阶段边界），禁止出现部署目录、系统初始化、Token 签发细节、退纳管详细语义、阶段演进等实现语义。
    - **13-2 §4 核心流程**：①每个流程一个 `### 4.N` 编号子节（编号连续），每节 = 一张 Mermaid 图 + **正文说明**（编号 / bullet 列表或说明段均可，**禁引用块**；节点已写人话的用户层图允许只用一段说明段带出，不必强凑列表）；②**用户层流程与技术层时序并存**——用户层用 `flowchart LR/TD`（节点写人话：用户动作 / 页面 / 结果），技术层用 `sequenceDiagram`（参与者 = 服务 / 接口）；**纯技术时序不得单独占据整章**（历史教训：M07 §4 仅 1 张 `sequenceDiagram`（UI→API→服务），PM 无法据此理解业务动线）；③图后的「流程说明 / 边界说明」**禁止写进引用块**（M07 实测 3 条说明在引用块内），应作正文与图并列。**④流程说明形态（v1.40 起，M09 §4.3 纯散文 9 行事故固化）**：Mermaid 图后的说明优先用编号列表 / bullet 承载；连续 >5 行的纯散文段落应拆分或审计归属——其中涉及接口/产物/状态细节的部分归 §6，涉及页面入口/跳转的部分归 §11。**不允许**用整段散文复述图中已表达的节点内容。
    - **13-3 §5 数据模型**：①**每个对象一个 `### 5.N <中文名>（<EnglishName>）`**，字段表置于其下；②**字段表表头全模块统一为 5 列**——`| 字段 | 类型 | 必填 | UI 展示名 | 说明 |`（M07 实测 5.3 / 5.10 / 5.11 三处用 4 列表头缺「必填」列，同章内表头不一致）；非字段表（枚举表 / 映射表 / 参数表）可按需定列，但**不得与字段表混同表头**；③**禁止用 code fence 承载数据模型定义**——Go struct、yaml / json 字段集都属「模型定义」，一律改写为字段表（示例值可用行内代码）；code fence 仅在确需展示**产物样例**（配置片段 / 报文）时使用，且应下沉 §6 或产物归属章节。M07 实测 §5 内 11 处 fence（2,730 字符），含 `type ResourceCategory` 枚举与 `ResourceStatusMapping` struct 两处**模型定义**；④**引用块占章字符比 ≤20%**：禁止用引用块承载字段语义与定版论证——语义写进字段表「说明」列或子节正文，论证迁 `design-decisions.md`；M07 实测 §5 有 16 处 >200 字符引用块、占比 29%（最长单块 1,628 字符）。⑤**子节编号连续、层级正确**：`### 5.N` 无断号；`#### 5.N.M` 必须连续数字编号，**禁止 `#### A. / B. / C.` 字母编号**（M07 实测 5.12 下三节用 A/B/C）；`### 5.7.1` 把 `####` 误写成 `###` 属层级错误（M07 实测）。**空号例外（唯一）**：历史上被**整体提升为独立章节**而永久空出的编号**保留空号、禁止复用重排**——M07 缺 5.14 的成因是「原 5.14 数据模型状态机提升为第 8 章」（见 `module-07/design-decisions.md` 冻结骨架重编号条目）；若把 5.15~5.18 顺移填补，将连锁改写 M01 / M06 / M09 与本模块内共 40+ 处 `5.15` / `5.16.x` 引用及跨文件锚点，收益远小于风险。判定口径：**断号在模块 design-decisions 有成因记载即合规**，无需在 PRD 正文加注。
    - **13-3-bis 字段「必填」列机读表达**：字段表「必填」列必须可机读，统一三态：
      - 无条件必填 → `✅`
      - 无条件非必填 → `❌`
      - 条件必填 → `条件必填：channel=agent_pull` / `条件必填：由系统生成` / `条件必填：channel=agent_pull 且 enabled=true`（等号后写触发条件，不展开长说明）
      **禁止**在同一列混用 `✅/❌` 这类无法解析的写法（历史教训：M09 §5.1 `token` / `agent_type` / `center_endpoint` / `remote_write_url` / `status` 全部写成 `✅/❌`，读者无法一眼判断条件）。条件化说明的详细语义放进「说明」列或 §6 契约。
    - **13-3-ter 字段节下方内容限制（v1.40 起增补机检口径，M09 §5.1 表下 6 段非生命周期说明事故固化）**：字段表下方的散文**只保留 2 类**：① 该对象生命周期总体说明（≤3 段）；② 指向其他章节的指针（如「字段展示规则见 §11.2」）。其余跨章内容必须迁走——M09 §5.1 现状：表下堆叠了 MVP 处理、行政约束、网域与业务正交、K8s 接入备忘、标签注入边界、配置目录组织、字段分类与抽屉三态、WAL 参数表、多网域能力表等 9 个主题，其中 WAL 参数应归 §6.4 或 §6.8，多网域能力应归 §3.1，字段分类/抽屉应归 §11，K8s 备忘应归 `design-decisions.md`。**机检口径**：每个 `### 5.N` 子节内，字段表（首列 == `字段` 的表格）下方非空行中，若出现非 `>` 引用指针、非 `##`/`###`/`####` 标题的散文段落数 >3，或连续出现跨章主题词（`MVP 处理`/`行政约束`/`边界`/`K8s`/`标签注入`/`多网域`/`配置目录`/`抽屉`/`空态`）≥2 个，即触发归属审计；存量模块 FAIL 作迁移进度指标，新改动或新注册模块必须达标。
    - **13-4 §7 依赖**：固定两节结构——①`### 7.1 模块边界与依赖关系`：**每个协作模块一个 `#### 7.1.N 与 Module_XX 的边界` + 责任矩阵表**（`| 职责 | <对方模块> | <本模块> |`，单元格用 ✅ / ❌ 标注归属，必要时附一句口径说明）；②`### 7.2 技术依赖`：平台目录 / 上游模块 API / 外部组件（bullet 列表）。**跨模块边界声明的唯一权威在本章**——§0 / §1 / §3 提到边界最多一行指针（「与 Mx 的边界见 §7.1」）。M07 实测 §7 仅 487 字符（散文 84%）且无矩阵，边界声明反被写在 §1 → 权威位置缺载体、非权威位置重复描述。
    - **13-5 §8 数据模型状态机**：①每个状态机一个 `### 8.N <对象>.<字段>（<中文名>）状态机`（编号连续）；②每节固定形态 = Mermaid `stateDiagram-v2` + 状态表 `| 状态 | 含义 | 进入条件 | 后续流转 |`（4 列固定），进入 / 退出条件与守卫写进表格而非引用块；③**禁止把非状态机内容塞进本章**——枚举取值域、优先级链、能力对比、归属解析链等属「规则」而非「状态流转」，归 §5（数据模型）/ §10（术语）/ 对应契约章节；M07 实测 8.2「标签来源优先级」用 ` ```text ` 代码块承载优先级链，既非状态机又非正文；④**禁止用 code fence（含 ` ```text `）承载状态流转或优先级链**；⑤引用块仅保留 `> 说明：` 一行（与 §4.2 / §8 现状一致），状态语义一律写进状态表——M07 实测 §8 引用块占 39%（708/1,833 字符），把「行为语义（目标采集契约）」大段规格写在引用块内。
    - **13-6 章节质量红线（可机检，门禁见 `scripts/check-prd-hygiene.sh` 检查 6）**：①**单章字符占比 ≤35%（诊断项，不作失败条件）**——超过只触发「**归属审计**」（逐子节核对该章是否在兼任其他章的职责），**不得直接判违规**：机器无法区分「层放错」与「对象多」，M07 §5 = 49.0% 但 17 个子节分布均匀（最大 5.16 = 13.9%），属对象数量带来的固有权重而非层放错（对照：M09 §3 改造前 = 38.9% 是层放错、可迁出；M09 §5 改造后 = 15.7%）；②**每章引用块字符占比 ≤20%**（**失败条件**；历史值：M07 §1 53% / §5 29% / §8 39%，M09 §1 50% / §5 33%）；③`### N.M` 编号连续性与字段表表头一致性（见 13-3，**断号按 13-3⑤ 空号例外判定**）。**门禁口径**：存量模块的 FAIL 是**迁移进度指标**（与检查 1 同性质），不阻断提交；**新改动或新注册的模块必须达标**。判定前先跑门禁取基线，改后复跑比对，报告须分开列出「本轮新增违规」与「存量待迁移」。
14. **§6 接口设计形态纪律（v1.38 起，M09 §6.8/§6.9 逻辑性事故固化）**：要求 13 覆盖了 §1/§4/§5/§7/§8 五章，但 §6 接口设计（含接口契约、产物规则、运行行为、变更机制）长期无章级形态约束，导致 M09 §6.8 120+ 行单节堆砌 12+ 个主题、标题层级事故（`#### 3.5.1` 出现在 §6.8.3 下），§6.9 把触发机制、摘要生成、全链路关联多个主题混成一个子节。本条把 §6 纳入同一套「形态与归属」约束。
    - **14-1 §6 结构总则**：`### 6.N <主题>` 编号连续，每节只承载一个主题；每主题优先用「输入 → 处理 → 输出」Mermaid 图或责任矩阵表表达，再用正文 bullet 补充。**图不是强制**：规则节可用表格为主、图为辅，但纯 bullet 堆砌超过 80 行必须拆分或补表/图。
    - **14-2 编号层级**：`#### 6.N.M` 必须与本章连续，**禁止出现其他章节编号**（历史事故：M09 §6.8.3 下出现 `#### 3.5.1 external_labels 注入说明`，把 §3 编号错写到 §6）。
    - **14-3 产物规则节形态（如 6.8）**：必须配两张表——①「源数据 × 配置文件」映射表（`| 源数据对象 | 驱动字段 | 生成文件 | 触发条件 |`）；②「文件 → 生成规则 → reload 条件」表（`| 文件 | 生成规则要点 | 变化时是否触发 reload |`）。单节超过 **80 行必须拆**为 6.N.1/2/3（用户已确认硬门槛）。
    - **14-4 生命周期 / 触发机制节形态（如 6.9）**：必须用 `stateDiagram-v2` 或 `flowchart LR` 表达状态 / 数据流；每个子主题独立成 `#### 6.N.M`（如 6.9.1 触发模式、6.9.2 审批分级、6.9.3 摘要生成、6.9.4 全链路关联）。摘要模板、混单规则等用表格枚举，不得用长段散文。
    - **14-5 §6 接口字段表**：若 §6 需定义请求 / 响应字段，表头统一为 `| 字段 | 类型 | 必填 | 来源/默认值 | 说明 |`，与 §5 字段表 `| 字段 | 类型 | 必填 | UI 展示名 | 说明 |` 区分。
    - **14-6 §6 跨章内容归位**：术语 / 命名空间规约归 §10，页面状态 / 字段展示归 §11，部署目录 / 工程细节归工程文档或 `design-decisions.md`，跨模块边界归 §7。
    - **14-7 §6 内容边界清单（v1.39 起，M09 §6 沦为「技术细节兜底筐」事故固化）**：§6 只承载三类内容——①**接口契约**（路径 / 方法 / 请求响应字段 / 错误码）；②**运行时行为**（心跳、拉包、本地 reload、幂等、重试）；③**产物生成规则**（输入数据 → 生成哪些文件 → 何时触发 reload）。下列内容**禁止在 §6 展开**，至多保留一句话 + 内链：地址语义 / 网闸穿越 → §3.1 能力索引或 §7.1 边界矩阵；跨模块校验分层与责任划分 → §7.1；跨模块变更触发链路 → §4 核心流程；共享数据模型字段语义 → §5；页面状态 / 字段展示 / 抽屉与空态 → §11；命名空间与标签键规约 → §10；部署目录 / systemd / 工程安装 → 工程文档或 `design-decisions.md`；完整 JSON 样例 / 全量字段 / 错误码枚举 → `api-contract-snapshot.md`。
    - **14-8 接口契约详版溢出门槛（v1.39 起）**：满足以下**任一**条件时，PRD §6 只保留「接口分组 + 方法 / 路径 / 关键字段 + 边界说明」汇总表，完整契约（含全部字段、JSON 样例、错误响应）下沉到 `docs/05-execution-records/module-XX/api-contract-snapshot.md`：①单节 >80 行；②同一组接口字段表 >12 行；③JSON / yaml 样例 >3 个；④错误码 / 状态枚举条目 >8 个。下沉后在该节留一行指针（如「完整请求 / 响应契约见 `api-contract-snapshot.md` §2.3」）。
    - **14-9 产物示例归属（v1.39 起）**：产物**规则**节（如 6.8）只写「输入 → 处理 → 输出」规则；产物**示例**（完整 prometheus.yml / targets.json 片段等）统一归 `design-decisions.md`「产物样例」小节或 §11 原型说明，每节最多保留 1 个最小 inline 示例。
    - **14-10 拆分门槛的层级澄清（v1.39 起）**：14-3 / 14-4 的 **80 行硬门槛按「叶子子节」判定**——任何 `### 6.N` >80 行必须拆为 `#### 6.N.1/2/3`；任何 `#### 6.N.M` 仍 >80 行须继续拆分，或按 14-8 溢出到 `api-contract-snapshot.md`。**不允许**用「父节总行数」掩盖子节臃肿。
    - **14-11 §6 信息密度审计（v1.40 起，M09 §6.8.2/§6.8.3 单段混多主题事故固化）**：§6 叶子子节虽满足 ≤80 行，仍可能因单段堆砌多主题而可读性差。判定口径：同一正文段落（以空行分隔）同时涉及 ≥3 个主题域（如 `scope` / `端口解析` / `规则粒度` / `校验分层` / `标签注入` / `命名空间` / `业务场景` / `版本差量`）时，提示拆分：①拆为独立 `#### 6.N.M` 子段；②或将非接口/运行时/产物规则的内容迁到 §5 / §10 / `design-decisions.md`。**不允许**用「单段合规」把 §5 数据模型语义、§10 术语规约、design-decisions 历史备忘塞进 §6。
...
- **「变更内容」列长度上限（v1.39 起，M09 v1.77 行 638 字符事故固化）**：该列**目标 ≤120 字符、硬上限 200 字符**——只写「改了什么 + 关键目的」，**禁止**罗列改造清单、迁移映射表、各章节行数 / 字符数变化等量化数据。超出部分一律下沉到 `design-decisions.md` 对应版本的建档小节，表格单元格以「（详版见 design-decisions）」一句话收尾。**为什么封顶**：Change Log 是给 PM / 业务方**扫读**的一句话决策记录，单格 600+ 字符 ≈ 一页纸，扫读功能完全失效（M09 v1.77 实测 638 字符，含十余项逐条改造清单）。**门禁**：`scripts/check-prd-hygiene.sh` 检查 6i 对超过 200 字符的版本行判 FAIL；**本轮新增行必须达标**，存量超长行在下一次迭代时压缩。
...
| "规格写进引用块显得重点突出" | 引用块不在章节级读取的预期路径上，AI 与读者都易漏读；规格必须在权威章节以正文承载，引用块只放备注与引用（见 3.1 要求 11） |
| "评审先看技术实现，用户视角后面再补" | 用户层错误（导航乱 / 文案看不懂）会导致技术层白返工。两段评审先用户走查后技术核对，技术段仍强制完整（见 Phase 8.1） |
| "§3 放技术细节方便 AI coding 直接取用" | AI 有章节级读取协议（v1.24），技术细节本应去 §5/§6/§8 找；写在 §3 反而三输——PM 读不懂、AI 不在这找、增加噪声。见要求 12（v1.34 §3 防叠加纪律） |
| "优先级 P0 就等于 MVP，§3 不用再写版本" | 优先级与产品版本是两个正交轴——P0 只说「本轮必做」，不说「哪个版本交付」（实例：`P0（首问分流 v0.2 起）`、`P0 / P2`）。只写优先级会让功能↔版本关系在 §3 内消失，且版本轴不被强制就会被瘦身当冗余散文清掉（Module_07 v2.36 实测 5 处）。见要求 12-5（v1.35 交付版本列）|
| "数据模型用 Go struct / yaml 代码块最精确，字段表太啰嗦" | 代码块是**实现语言**的绑定表达，PRD 是前后端共同契约——用 Go 就偏向前端读者、用 yaml 就偏向配置读者，且 AI 章节级读取无法从代码块稳定抽取字段清单。字段表（含类型 / 必填 / UI 展示名）才是唯一跨语言权威（见要求 13-3）。历史实例：M07 §5 `type ResourceCategory` / `ResourceStatusMapping` |
| "字段语义写引用块更醒目，表格里塞不下" | 引用块不在读取路径上、且不参与门禁的表格检查——语义进「说明」列、超长拆子节正文，论证迁 design-decisions。M07 实测 §5 引用块占 29%（16 处、最长 1,628 字符），读者要跳着读（见要求 13-3 ④） |
| "边界声明写在 §1 和 §7 两处，读者更容易看到" | 同一规格两处全文复述正是要求 11 禁止的「多处权威」——决策落地时必漏改一处。§7 是跨模块边界的唯一权威（责任矩阵表），§1 至多留一行指针（见要求 13-4）。历史实例：M07 §1「与 M01/M09 的边界」与 §7 重复、而 §7 反而只有 487 字符无矩阵 |
