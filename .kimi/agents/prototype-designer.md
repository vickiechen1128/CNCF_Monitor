...
11. **规格归位与正文格式纪律（v1.33 起，M09 PRD 瘦身实践固化）**：
    - **单一权威章节**：同一规格主题只在一个章节承载权威版本——功能 / 用户层 → §3，字段语义 → §5，接口 / 协议 → §6，跨模块依赖边界 → §7，状态流转 → §8，验收 → §9，术语 → §10，页面状态与全局行为 → §11；其余章节至多保留一行引用（如「字段定义见 §5.2」），**禁止多处全文复述**（历史教训：M09 命名空间 / go-no-go 审批口径散布 3-4 章，决策落地时漏改）。
