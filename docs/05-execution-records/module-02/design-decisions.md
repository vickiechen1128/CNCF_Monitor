...
## 分轨判定记录：2026-09-09（`/api/v1/alerts/history` MVP 增量，PRD v1.13）

- **触发**：M08「历史告警」独立页面（含已恢复）提级 MVP，需要 M02 提供历史告警只读 API；用户书面确认（2026-09-09）。
- **决策**：
  1. **新增 `/api/v1/alerts/history`（GET，P0/MVP）**：基于 Prometheus `ALERTS{alertstate="firing"}` 的 `query_range` 重建规则级触发/恢复区间；不走 Alertmanager（与告警是否配置通知链路解耦）、不新增控制面存储。
  2. **重建口径**：按 `alertname + instance + 其余 labels` 分组，连续 firing 样本合成同一触发区间，样本中断超过 `2 × step` 视为区间结束；`step` 默认 30s、最小 15s；时间窗默认 24h、最大 7d；`state=firing`（查询窗口结束仍在触发）时 `resolved_at=null`，`resolved` 时 `resolved_at = 最后一个 firing 样本时间 + 一个 step`（求值近似值）。
  3. **授权过滤**：沿用决策 56 三层语义，`tenant_id` 硬注入 + `network_domain` 授权集合收敛，服务端强制执行；MVP 单租户恒通过、机制保留。空结果 `list=[]`（非 `null`）。
  4. **分页与限额**：`page`/`page_size` 默认 1/50，`page_size` 上限 200；区间重建在服务端完成后分页。
  5. **边界**：本接口为规则求值视角历史，不含通知发送结果（通知历史属 M08/AM 域，后续版本评估 AM Webhook 事件表方案）；v0.4+ 边缘自治告警（边缘 vmalert 本地求值）不在中心 `ALERTS` 内，与现有 alerts 代理边界一致。
- **轨道**：**Track B+**（读路径授权过滤，security-reviewer 挂载）；免高保真原型（标准列表 + 筛选模式）。
- **影响范围**：Module_02 PRD v1.13（§1 版本分布、§3.1 功能表、§5.4 数据模型、§6.1 接口、§11.1/11.2 验收 9a/14a、§12 M08 边界、§13 术语）；消费方 Module_08 PRD v1.13。
- **关联决策**：决策 55/56（告警状态归属与授权过滤）、决策 47 系列（只读代理模式同构）。

## PRD v1.16 章节对齐 + 去历史化归档（2026-09-16）


> 章节对齐 11 章模板：旧 §7→§6.4（6.4.1 key 契约 / 6.4.2 注入规则）、旧 §8.1/8.2→§5.2（envelope 结构与数据来源语义合并）、旧 §8.3 UI 语义→§11、旧 §9→§7.1、旧 §10→§8、旧 §11→§9、旧 §12→§7.2、旧 §13→§10、新增 §11 前端交互契约。「提示分区规范」整节迁本档（权威源头为 `.kimi/agents/prototype-designer.md`「提示分区规范」）。正文 8 处内联决策标注迁本档；Change Log v1.13 行轮转本档、v1.15/v1.14 行压缩。规格语义零变更。
...
```
[四张 4 列字段表原样存档：见 git 历史 fa400ee^ 版本]
```

**旧 §7.1/§7.2 标题行**：

```
### 7.1 注入标签 key 契约（v1.2 修订，v1.15 定版，关键） / ### 7.2 注入规则 → 并入 §6.4
```

**Change Log v1.13 行（轮转归档）**：
