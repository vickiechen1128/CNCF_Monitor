...
- **已决策实现口径（用户批准）**：主键维度改为 `resource_id × scrape_job_id`，FK scrape\_job\_id→ScrapeJob.id；`exporter_template_id` 降为冗余缓存（来自 ScrapeJob，不参与唯一性）。
...
- **位置**：Module\_01 §5.4 `network_domain_id` 必填口径、§8/§9 网域约束；Module\_01 §1 边界（M09 负责配置生成）
...
- **待 PRD 修订**：在 §5.4/§9 补一句「is\_monitored 由 M09 维护，MVP 阶段 seed 预置已纳管」的过渡说明（可选，用于消除评审疑问）。

## 2026-08-23（审查登记 ② 偏差，非阻断，MVP 阶段裁剪）

> 以下 3 项已由审查判定为与契约/验收相关但受 Phase 阶段限制，评为非阻断；仅登记说明原因，不强制改逻辑。

### F-03：M2 mapping\_overrides 提交时未持久化
...
- **位置**：`ui-custom/web/src/pages/strategy/ScrapeJobFormDrawer.tsx`（编辑态网域 `disabled={isEdit}`）；契约 `api-contract-snapshot.md` §5/§10（`PUT /scrape-jobs/:id` 允许改 network\_domain\_id，仅当目标网域冻结时禁止新增该域实例）
...
- **决策（chenrt，2026-08-23）**：拆回两个独立 Form.Item：`resource_category`（选项=MONITOR\_TYPE\_CASCADE 各 category，展示 CATEGORY\_MAP）+ `monitor_type`（保留契约字段名，选项按已选 category 过滤，两级级联）；选定 monitor\_type 仍沿用 handleMonitorTypeChange 带出默认采集器与参数；编辑态由 record.monitor\_type 反推预填 resource\_category；提交载荷仍为 single monitor\_type（resource\_category 仅表单级联用，不落后端字段）；MappingDrawer 编辑态两字段保持 disabled。

- **留痕**：T01-F13 落地，见 frontend-prototype-map §6.1/§7；影响文件按 `fe-fix-monitor-type-split` 分组。

### F-03 补充：参数同步列三态回显（T01-F12）

- **背景**：dev F-03 登记「`mapping_overrides` MVP 不持久化」。T01-F12 在未落库前提下纠前端口径：黑盒 Job 显「同步」；有覆盖显「已覆盖 n 项」（蓝）；无覆盖时对比默认映射快照（ciExporterMappings is\_default）——不一致显「待同步」（橙）+ Tooltip「映射默认值已变更」，一致显「已同步」。
...
- **O1（T01-F14）**：Job 列表「标签模板」列「待配置」橙 Tag 改异常驱动 —— 正常继承（job.label\_template\_id 命中）只显模板名、不显「待配置」Tag；仅当 Job 未关联标签模板、但该 monitor\_type 默认映射已挂 label\_template\_id 时显橙色「待配置」（可点击补配引导）；其余显 `-`。复用 `labelTemplateById` / `defaultMappingByMonitorType`，不新增后端调用。
...
### F-09：采集器管理与采集 Job 由「页内 Tab」改为「两个独立页面」（① 类，已决策；2026-09-05 终验裁决（决策 63，chenrt 拍板）：**以生产导航为准**——保留「采集策略」一级分组（Sider 二级：采集器管理 / 采集 Job / 规则编辑 / 指标库），撤销「取消分组、提升一级」补充裁定；PRD v3.36 与原型 {v3.36} 已回改，本条闭环）
...
- **裁定修订（决策 63，chenrt 2026-09-05 终验拍板）**：**以生产导航为准**——生产 `MainLayout.tsx` 保留「采集策略」一级模块（Sider 二级四项：采集器管理 / 采集 Job / 规则编辑 / 指标库），上述「取消分组」部分**撤销**；PRD（§5.1 / §9 / §10，v3.36）与原型（{v3.36} MainLayout + README）已回改对齐。主决策（两独立页面）不受影响。

### F-10：默认采集配置抽屉「标签模板」字段偏离原型（② 类，待修复）

- **位置**：`ui-custom/web/src/pages/strategy/MappingDrawer.tsx:211-213`（label\_template\_id 为裸 Input + `disabled={!isEdit}` 新增禁填）；原型 `ScrapeJobsPage.tsx` L2075-2106（Select 选择器 + 自动预填 + 无模板创建引导）；PRD §5.1 L236（已有模板自动预填默认模板、无模板触发创建引导）

- **问题**：生产把标签模板关联重心放在列表「查看/更换/补配」两行卡片，抽屉内 label\_template\_id 退化为「新增禁用 + 手填 ID」，丢失了 PRD/原型「新增即自动预填默认模板 + Select 选择器 + 无模板创建引导（立即创建/稍后再说）」的要求；Job 侧标签模板非必填（✅ 与 PRD 一致，无偏离）。

- **决策（chenrt，2026-08-23）**：MappingDrawer label\_template\_id 改为 Select 选择器（按资源类别过滤 + allowClear），新增态可预填/可选（不再 disabled）；无可用模板时提供「前往标签模板管理（M07）创建」引导；已有默认模板自动预填。标签模板仍非必填、与采集器正交。
...
- **实现落库**：
  ...
  - 后端 `platform/strategy/scrapejob/create.go`：`CreateScrapeJobRequest` 新增 `draft_status` 字段；`draft` 时仅做基础校验（job\_name / job\_type / 唯一性）并保存，`ready` 时做完整校验并进入 M09 变更管线。
...
- [x] F-06 已闭环（2026-09-05 终验核验：`network_domain.go:64` Token `json:"-"` + `:85` AfterFind 派生 `token_masked` 已落地，M06/M09 跟进完成；登记核销）

- [ ] F-07 已裁决（编辑态网域按契约放开，T01-F11 落地）

- [x] F-11 已收割（chenrt 修订 PRD §6.2.1：`label_template_id` 唯一变更入口=列表「更换/补配」轻量抽屉，编辑抽屉不含该字段；原型动线已同步）

- [x] F-12 已对齐 PRD §5.1（2026-09-05 终验核验：PRD 映射明细/补配入口/M07 链接、原型（`ScrapeJobsPage.tsx:3808-3872`）、生产前端三侧一致，PM 复核通过；跨模块跳转未落地统一导航配置为既有债务，另登记）
...
- [x] F-27 A/B/C 已修正/实现 + 已收割（删除入口 / 下拉类型过滤 / 映射类型校验已落库）；**F-27 D 已由 F-32 决策落地**（MVP 开放 official/third\_party/internal 用户登记 + 预置保持 seed 初始数据，`is_builtin` 作为来源标记）
...
- [x] F-32 已修正/实现 + 已收割（代码已落库：三来源均可登记 / 所有来源 `default_port`/`metrics_path`/`scheme` 必填；PRD v3.31 / §3.1 / §5.2 / §9.1 / §10 已同步；原型登记表单文案已更新）

- [x] F-34 已修正/实现 + 已收割（代码已落库：静态资源隐藏「关联实例」Tab 与 badge；PRD v2.27 / §3.1 / §6.2 已同步；原型 `LabelTemplatesPage.tsx` 已条件渲染）
...
- **问题（用户实测提出）**：
  ...
  4. **登记采集器后无「查看」入口**：supported\_monitor\_types 等登记信息无法回显确认；
  5. **默认采集配置（映射行）无「删除」入口**：后端 DELETE 早已实现，前端未放按钮。

- **结论（用户拍板，方案 A / V1「层叠默认 + 稀疏覆盖」）**：

  - **层叠默认链**：Job → 默认采集配置（CITypeExporterMapping，is\_default）→ 采集器模板（ExporterTemplate，仅 metrics\_path/scheme 有模板级默认）→ 全局兜底常量（`models.DefaultScrapeInterval=15s` / `DefaultScrapeTimeout=10s` / `DefaultMetricsPath=/metrics` / `DefaultScheme=http`）。任一层留空 = 继承下一层。
...
- **决策（用户拍板）**：
  ...
  - **labels 最终挂在 target 级**（`targets/*.json` 中每个 target 的 `labels`），与 Prometheus file\_sd 语义和资源实例级差异化一致；`prometheus.yml` 的 job 级 labels 仅保留系统必要字段（如 `network_domain_id`），不承载标签模板映射。
...
- **PRD 章节 / 文件位置**：`Module_01_Metric_Collection_Center.md` §5.4（draft\_status / change\_status 流转）；源码 `platform/strategy/scrapejob/update.go`、`delete.go`、`ui-custom/web/src/pages/strategy/ScrapeJobListPage.tsx`
...
- **问题（联调实测）**：添加采集 Job 后 Prometheus 抓取报 `Get "http://1.15.94.116/metrics": dial tcp 1.15.94.116:80: connect: connection refused`——target 生成无端口，Prometheus 默认 80；而 node\_exporter 实际监听 9100（用户实测可达）。targets JSON 产物为 `["1.15.94.116"]`，`instance` 标签同为无端口。

- **根因**：`resolveResource` 的 Host 分支 `Address = host.PrivateIP` 只取 IP；Database/Middleware 分支虽拼端口但用的是**资源业务端口**（3306/6379），按 PRD §5.12C 应为 exporter 监听端口（9104/9121）。`ScrapeJob` 模型与前端表单均无 `port` 字段，PRD §5.4 内部对「端口是否进 Job 快照」表述冲突（416 行 vs 字段表 / mapping\_overrides）。

- **结论（用户拍板，决策 46）**：MVP 先取**生成器层最小方案**——不新增 ScrapeJob.port 字段（PRD §5.4 字段表与 mapping\_overrides 口径冲突另行列项，待设计侧统一后再评估 Job 级端口快照），由 M09 生成器解析策略层端口：

  - `LoadExporterPort`：优先 `CITypeExporterMapping.default_port`（monitor\_type 默认映射），回落 `ExporterTemplate.default_port`（exporter\_template\_id）；
...
- **是否需设计侧确认**：~~需——① PRD M01 §5.4 统一「端口是否进 ScrapeJob 字段 / mapping\_overrides」口径~~ **已收割**（PRD §5.4「端口不在 Job 层的理由」已按决策 46 统一：MVP 端口**不进** **`ScrapeJob`** **快照**、`mapping_overrides` 亦**不含** **`port`**，由 M09 生成器按 `CITypeExporterMapping.default_port` → 回落 `ExporterTemplate.default_port` 解析）；② 若确认 Job 级端口快照（v0.2+），补 ScrapeJob.port 字段与前端表单端口输入——仍待 v0.2+ 评估。

- **影响模块**：M01 采集 Job（模型/表单端口字段规划）、M07 §5.12C 契约（已按 default\_port 对齐）、M09 配置生成
...
- **问题（用户验收）**：禁用已生效采集 Job 后，配置变更确认页显示「本次配置无变化」，但版本对比 Tab 显示 scrape\_config 确实被删除，两者矛盾。
...
- **类别**：① PRD 空白判定（多记录如何合并为 rules.yml 未定义）+ ② 实现补齐（v0.2 的 monitor\_type 维度提级 MVP，用户拍板）
...
- **结论（用户拍板，方案 A）**：
  ...
  2. **组名全局唯一**：保存时校验（创建 + 更新路径）——文件内重名 / 空 name 报错；与其他生效规则（`enabled=true AND draft_status=ready`，排除自身）撞名返回 bad\_request 并点名占用方；停用规则不下发不校验，停用后组名释放；
  3. **monitor\_type 提级 MVP（可空）**：POST/PUT 接收 `monitor_type`（非空时校验 `models.ValidMonitorType`），GET 列表支持 `monitor_type` 筛选；前端挂载抽屉加「资源类别 → 监控对象类型」两级级联（复用 `MONITOR_TYPE_CASCADE`，与 MappingDrawer/ScrapeJobFormDrawer F1-8 同源），列表加「监控对象类型」列与筛选、详情抽屉补一行；
  4. **产物形态不变**：仍是单文件 `rules.yml` + `prometheus.yml` 注入 `rule_files: ["rules.yml"]`；monitor\_type 纯组织/筛选维度，不参与渲染；不持久化 `exporter_template_id`（MVP 无指标提示功能）。
...
- **验证**：后端 `go test ./platform/...` 全绿（generator 合并用例 + rule 包组名唯一 / monitor\_type 校验筛选 4 个新用例）+ `go vet` 通过；前端 `RuleMountDrawer` / `RulesPage` 9 用例通过、`pnpm lint`、`tsc --noEmit` 通过（全量 `pnpm test` 仅 `src/api/resources.test.ts` 1 个改动前已存在的 jsdom `Response.stream` 失败）。
...
### F-25：规则「停用可编辑」MVP 落地，规则草稿（draft\_status=draft）推迟至 v0.3（① 类，已决策）

- **类别**：① PRD 空白判定（MVP 范围裁定，用户拍板）

- **PRD 章节 / 文件位置**：`Module_01_Metric_Collection_Center.md` §5.5（draft\_status：「MVP 文件挂载默认 ready…；v0.3 字段化编辑开放『保存草稿』」）；`api-contract-snapshot.md` §18（草稿 draft api / clone / 批量提交 列为 **v0.2 能力、本期范围外**）；源码 `platform/strategy/rule/create.go`、`update.go`、`ui-custom/web/src/pages/strategy/RuleMountDrawer.tsx`、`RulesPage.tsx`
...
- **结论（chenrt，2026-08-26，用户拍板）**：
  ...
  2. **规则草稿功能推迟至 v0.3，不提前实现**：MVP 不做规则「保存草稿 / 批量提交生效」；yaml\_passthrough 整文件无「PromQL 半成品」场景，草稿价值绑在 v0.3 字段化编辑；api-contract 已将其列为 v0.2 能力、本期范围外，本次明确不提前。
...
- **待 PRD 修订**：~~§5.2 L81 动线「仅自研 / 第三方需要」与 L334「third\_party 平台预置只读」冲突，需设计侧统一为「用户登记仅 internal」~~ **已收割**（design 已统一 PRD §5.2——用户登记仅 `internal`，`official`/`third_party` 平台预置只读维护，见收割状态 F-26 注记）。
...
### F-27：采集器管理能力闭环——删除入口、supported\_monitor\_types 约束、端口/路径分层语义（② 类补齐已落地 + ① 类产品方向待设计侧）
...
- **用户问答结论（已在会话中向用户澄清并获认可）**：
  ...
  3. **登记时类型非必填 vs 默认配置必填** = 实体语义分层：ExporterTemplate 是「采集实现」（一个实现可支持多类型，允许先登记后绑定）；CITypeExporterMapping 是「类型 ↔ 默认实现」绑定，monitor\_type 是主键维度必填——PRD 口径合理，不改；
  4. **监控对象类型不可手填** = monitor\_type 是闭环枚举（`models/monitor_type.go` 9 项），驱动实例候选收敛（DeriveResourceFilter）/ 指标库分组 / 标签模板过滤；用户自带采集器（如 Spring Boot）归 `application_http`（PRD L594 免安装确认）。「自定义监控对象类型元管理」未设计，属 v0.2+；
  5. **登记端口 vs 默认采集配置端口** = 分层语义（模板=官方默认值参考 L332；映射=实际生效，决策 46 `LoadExporterPort` 映射层优先），F-26 起映射抽屉已从模板预填，默认一致、允许按环境覆盖——用户「应保持一致」的诉求即「默认一致」，已满足；路径（metrics\_path）同构。

- **已落地优化（用户拍板 A+B+C）**：

  - **A（前端）**：模板行新增「删除」按钮（Popconfirm 二次确认；仅非内置显示，内置/被引用由后端 forbidden 兜底）；

  - **B（前端）**：MappingDrawer 采集器下拉按 `supported_monitor_types` 对齐所选 monitor\_type（未标注的模板放行；已选中模板始终保留避免回显裸 ID；编辑态不过滤；空态引导「先登记采集器」）；

  - **C（后端）**：建/改映射校验——采集器 `supported_monitor_types` 非空且不含所选 `monitor_type` 时 bad\_request（`ensureExporterSupportsType`；update 路径按最终生效值校验；`verifyExporter` 重构为 `loadExporterByID`）。
...
### F-32：登记采集器来源放开——official / third\_party / internal 均可登记（① 产品方向拍板落地，② 已实现）
...
- **落地改动**：
  ...
  - **后端（create.go）**：`validateCreateExporterTemplate` 放宽 source 校验——三枚举（official/third\_party/internal）均可登记，非法值 bad\_request；`default_port` 必填从「source=internal 追加」改为**所有来源必填**（登记入库需完整采集参数）；与预置 seed 同名由既有 name 唯一索引返回 409 conflict（天然防重合），用户登记行恒 `is_builtin=false`、可编辑可删除。
...
- **验证**：后端 `go test ./platform/strategy/exporter-template/...` 全绿（`TestCreateExporterTemplateRejectsBuiltinButAllowsOfficialThirdParty` 覆盖 official/third\_party 成功登记 + 非法 source 拒绝 + is\_builtin 拒绝）；前端 `vitest run ExporterTemplateDrawer.test.tsx` 通过（来源三选项用例更新）、`tsc --noEmit`、`eslint` 通过。

- **影响模块**：M01 采集器管理 / 登记采集器。

- **发现场景**：用户提出放宽登记来源需求，要求评估放开时机。

- **状态**：closed（源码 + 契约 + PRD v3.31 / §3.1 / §5.2 / §9.1 / §10 已同步；原型 `ScrapeJobsPage.tsx` 登记表单文案已更新；待重新编译 metric-center 与前端 dev server 生效）
...
### F-34：静态资源（host/database/middleware/generic\_target）隐藏标签模板「关联实例」（② 实现偏差，已按方案 A 落地，① PRD 已明确待 design 同步）
...
- **背景 / 用户问答**：用户结合 PRD 分析指出——标签模板「关联实例」对静态资源无意义：
  ...
  - 静态资源（host / database / middleware / generic\_target）的标签治理在 **CMDB 侧**，平台只读展示，**不允许在实例级别自定义映射字段**，无需展示「关联实例」；
...
- **状态**：closed（前端已按方案 A 落地；PRD v2.27 / §3.1 / §6.2 已同步「关联实例仅业务类型资源（application）展示」；原型 `LabelTemplatesPage.tsx` 已条件渲染 badge / Tab；未来微服务/业务属性类型开放时扩展 `INSTANCE_LEVEL_CUSTOM_CATEGORIES` 即可）
...
## 2026-09-03（采集 Job 列表：实例采集状态文案指引 + 生效状态/变更进度角标与顺序）
...
- **PRD 章节 / 文件位置**：`Module_01_Metric_Collection_Center.md` §5.10（详情抽屉采集状态回显）；源码 `ui-custom/web/src/pages/strategy/ScrapeJobListPage.tsx`（实例采集状态列 header）

- **背景 / 用户问答**：用户指出列表「实例采集状态」列原 Tooltip「在线实例数 / 已选实例总数（…）；存在未在线实例时整格高亮，点击查看详情原因」对用户不友好，需改为便于指引的说法。

- **落地改动**：header Tooltip 改为「格式：在线实例数 / 已选实例总数。数据由『查询中心』（M02）按 Job 只读回显，约 20s 自动刷新。存在未在线实例（待采集 / 采集失败）时整格高亮为红色，点击整格可查看各实例采集状态与失败原因。」

- **影响模块**：M01 采集 Job 列表（纯文案，无逻辑变更）。

- **发现场景**：用户评审采集 Job 列表文案。
...
- **PRD 章节 / 文件位置**：`Module_01_Metric_Collection_Center.md` §5.4（状态列四态聚合）、§11.1/§11.2（批量提交与 M09 确认动线）；原型 `docs/prototypes/module-01/src/pages/ScrapeJobsPage.tsx`（D29 单一状态列、F-16 批量提交）；源码 `ScrapeJobListPage.tsx`（变更进度 / 生效状态列）

- **背景 / 用户问答**：用户提出：

  1. 列序应「先看生效状态、再看变更进度」（生效状态在前）；
  2. 「变更进度」「生效状态」两列均需增加头部角标指引；
  3. 批量口径需向用户说清两件事：① 变更生效需**点击批量确认**先行推进生效状态变化，但最终仍需到 **M09「配置变更确认」页手动确认一次**才会真正下发；② 变更进度**不强制逐个 Job 确认**——可等**所有监控配置调整完毕后一次性批量确认**，不必每次改动都确认一次。

- **落地改动**：

  - 列序调整：`间隔 → 生效状态 → 变更进度 → 参数同步`（原为 `间隔 → 变更进度 → 生效状态`）。

  - 两列 header 均加 `Tooltip + InfoCircleOutlined` 角标：

    - 「生效状态」：任务当前生效程度（草稿/待生效/已生效/已停用），提示「配置保存后不会立即生效，需到 M09『配置变更确认』页手动确认一次后真正写入并更新本状态」；

    - 「变更进度」：M09 管线进度（待确认/已确认待下发/已下发/无变更），提示「确认不限于本采集 Job，可待所有监控配置调整完毕后一次性到 M09 批量确认下发，无需每次改动都确认一次」。

- **验证**：`vitest run ScrapeJobListPage.test.tsx` 13/13 通过（测试断言单元格值而非列序/角标）；`tsc --noEmit`、`eslint` 通过。

- **影响模块**：M01 采集 Job 列表（列序与 header 渲染）。

- **发现场景**：用户评审采集 Job 列序与批量确认引导。
