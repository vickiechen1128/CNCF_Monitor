...
| 原型 | 生产 | 对齐 | 备注 |
|------|------|------|------|
| monitor_type 两级级联（资源类别 OptGroup→细粒度） | 两级 Select.OptGroup | ✅ | 等价 |
...
|-----------------------------|---------------------------------------|------|
| 「采集」父组（折叠）<br/>　· 采集器管理（?view=collectors）<br/>　· 采集 Job（?view=jobs） | 「采集策略」一级 tab<br/>　Sider 二级：采集 Job（内含 采集器管理 Tab） | ⚠️ **F1 折叠子菜单**：生产用页内 Tab 承载采集器管理，不采用折叠父菜单（D4） |
| 「指标库」父组（折叠）（技术/业务/视图） | 「采集策略」> Sider 二级「指标库」单一子项 | ⚠️ F1/D3：业务指标库与业务视图为 P2 不在本期 |
| 「规则编辑」独立项 | Sider 二级「规则编辑」 | ✅（层级从独立项变二级，语义词义一致） |
...
> 折叠子菜单偏差结论：原型「采集/指标库」折叠父菜单在生产以「一级 tab + Sider 二级拍平 + 页内 Tab/单一子项」替代，**已决策不启用**（F1），降低 Sider 层级（符合 Header+Sider 三层导航规范）。
...
| F1-5 | 采集器管理「未被引用」模板行并入 + 模板「去配置」动作缺失 | **P1** | 原型 CollectorRow template；生产仅映射行 | ✅ T01-F10（2026-08-23） |
...
- **P1 体验·验收 = 8（已全部修复 ✅，T01-F9~F12，2026-08-23）**：F1-1 / F1-2 / F1-4 / A4 / A7 / A9 / C1 / F1-5 / F1-6（其中 F1-1、F1-4 较重要）。→ 映射修复任务 T01-F9~F12。
- **P2 可延后 = 12**：D1 / E2 / F1-7 / B2 / F3 / F5 / D15 / F4 / E3 / A1 / A2 / D3 等（登记裁剪或迭代）。
