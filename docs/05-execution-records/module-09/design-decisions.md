...
## M09 拆分：边缘接入域独立为 Module_11（2026-09-17，决策 86）

> **触发**：v0.2 阶段功能详细分析（chenrt 提出）。M09 PRD v1.79 已达 229KB，Edge Sync Agent 功能域（网域纳管 / 节点状态 / 离线包 / 进程守护 / 升级维护）与配置生成下发域在用户任务、交付节奏、代码交付物上完全不同；「离线包下载入口」「升级维护」在 M09 无落点；进程守护仅一句话、缺工程化细节；心跳协议存在字段级缺口（hostname / 重启可观测性 / 离线阈值）。

### 结论（用户确认，2026-09-17）

1. **M09 1拆2**：边缘接入与 Agent 交付域独立为 **Module_11**（`Module_11_Edge_Access_and_Agent_Delivery.md`，中文题「网域边缘接入与 Agent 交付」）；**M09 不更名**，范围收窄为配置生成与下发中心（网域作为采集边界保留）。
2. **拆分判据（四维）**：① 用户任务——配置面是日常高频审批流（看懂变更、决定上线、出问题回滚），接入面是项目期一次性接入 + 低频运维（纳管、判断节点故障、升级），频率与心智不同；② 交付节奏——配置面 MVP 已交付且 ready，接入面全部 {v0.2}，不拆则 v0.2 每轮迭代都要动一份 ready 文档；③ 代码交付物——`platform/edge-sync-agent/` 为独立 Go module、独立构建目标（build-edge-agent / build-edge-package）、独立交付物（一体化离线包）；④ 新能力归宿——下载入口 / 升级维护塞进 M09 只会更重。M09 §3 菜单天然分「网域与节点管理」（接入面）与「配置下发」（配置面）两组，结构已承认两个任务域。
3. **拆分边界**（按 M09 PRD v1.79 章节切）：

| 迁 Module_11（接入面） | 留 Module_09（配置面） |
| --- | --- |
| §3.1 网域纳管全部（纳管动线 / Token / 安装指引 / WAL 参数 / 地址语义与网闸约束） | §3.3 配置变更确认与预览 |
| §3.2 采集节点状态全部（含诊断看板 P1/P2） | §3.4 配置下发与分发、§3.5 配置生成引擎 |
| §3.6.1 本地手工兜底（边缘侧主体）、§3.6.2 安全与证书、§3.6.3 边缘 Agent 交付方式 | §3.6.1 中 local 通道（中心 Prometheus 配置目录）口径 |
| §4.3 网域接入动线 | §4.1 轮询生成、§4.2 确认与下发时序、§4.4 变更触发链路 |
| §5.1 监控纳管字段 Owner（agent_type / token / center_endpoint / remote_write_url / 运行态字段） | §5.1 仅保留 `channel` 只读语义 + 多网域开关 |
| §5.2 EdgeAgent、§5.3 EdgeHeartbeat | §5.4/5.5/5.6 ConfigDraft / ConfigVersion / ConfigDeployment |
| §6.1 可达性约束、§6.2 心跳、§6.3 配置包拉取、§6.4 Agent 本地行为（含 6.4.1 WAL 参数） | §6.5 中心校验、§6.6 config-drafts / versions / deployments API、§6.7-6.9 生成规则 |
| §6.6.1 网域纳管 API（monitor / reset-token）、§6.6.4 采集节点状态 API | §6.6.2/6.6.3 变更确认与下发记录 API |
| §8.3 配置同步状态、§8.4 网域运行态、§8.5 纳管状态机 | §8.1 变更单状态机、§8.2 下发记录状态机 |
| §11.3 网域纳管页、§11.4 采集节点状态页 | §11.5 配置变更确认页与下发记录页 |

4. **页面归属**：网域纳管页、采集节点状态页**两页随数据所有权迁移至 M11**（数据 Owner 与页面 Owner 不分家，模式同 M06 行政 / M09 纳管先例）；**Web 门户菜单结构不变**——拆分仅发生在治理层（PRD / 原型目录 / feat 分支 / L3 任务序列），终端用户零感知；新增能力全部落两页内（下载入口并入纳管页安装指引区、守护可观测性与升级提示并入节点状态页），无全新页面。`docs/prototypes/module-11/` 新建，**迁移复用** module-09 两页骨架而非重做。M11 PRD 分层：UI-less 章节（心跳协议 / 进程守护工程化 / 离线包结构 / systemd 单元）走技术评审**免原型**；UI 章节走原型验证（基线为迁移存量页面，验证成本低）。
5. **本轮功能确认（四项）**：
   - **① K8s 采集场景整体推迟 {v0.4}**：中心无法直达 Pod 网络、采集器必须与中心分离的场景（K8s 集群采集），与同域多节点 / 混合通道 / 通道切换一并评审；决策 33「v0.4+ 演化影响备忘」需补「网络可达性」驱动因素（原备忘仅列容量 / 可用性 / 拨测多点，K8s 是最现实的网络可达性驱动）。
   - **② 进程守护工程化细节补全**（M11 PRD 落地）：健康检查双判定（进程存活 + HTTP 端点探活，默认 10s 间隔）；重启指数退避（5s 起步、上限 60s）；crash-loop 熔断（滚动窗口 10 分钟内重启 ≥5 次 → 停止自动重启、组件状态置持续异常态）——为决策 41-2「高危横幅」提供可计算触发条件；边缘 systemd 单 service 父进程模式（Agent 守护子进程、Agent 自身 `Restart=always` 兜底、blackbox ICMP 拨测需 `CAP_NET_RAW`）。
   - **③ 协议级缺口补全**：心跳协议补 `hostname` / `ip` 与组件级 `restart_count` / `last_restart_at`（§5.2 / §5.3 / §6.2 前后端契约对齐）；离线判定阈值量化——连续 3 个心跳周期（默认 90s）无心跳 → offline 并触发 `EdgeSiteOffline`；Token 重置边缘侧行为——Agent 收 401 后指数退避重试、持续失败记本地错误日志，reset-token UI 必须事前警示「重置后该网域节点将失联，需到节点更新 TOKEN 并重启 Agent」；时间戳基准——`last_heartbeat` / `last_config_pull` 以**中心接收时间**为准（边缘时钟不可信），心跳 `timestamp` 仅作参考。
   - **④ force_restart 本轮不做**：中心远程重启边缘进程的指令通道（决策 41-2 遗留评审项）就此关闭——与「中心不主动入站、outbound-only」安全边界冲突；维护动作 = Agent 本地自治重启 + 人工 systemd / 重装兜底。
6. **新能力落点**：离线包下载入口（包清单 / 版本 / sha256 校验和 / 下载——运维在中心门户浏览器下载后摆渡至边缘，符合隔离网域现实）与升级维护（{v0.2} 展示中心期望版本 vs 节点上报版本差异、{v0.3} 覆盖安装升级流程；Agent 不做自升级）为 M11 核心新增章节；v0.2 不建独立包版本模型（版本清单来自构建产物元数据），`EdgePackageRelease` 模型 {v0.3} 再评估。

### 耦合点处理

- **`channel` 字段**：M11 纳管写入、M09 只读消费（决定产物形态 local 文件集 vs zip 包）；M09 瘦身后保留只读语义声明，模式同「M06 行政 / M09 监控纳管」既有先例。
- **`config_sync_status` 状态机归 M11**；`out_of_sync_cause=pending_draft` 成因只读消费 M09 ConfigDraft 状态，M11 不反向依赖 M09。
- **agent_pull 下发链路**：M09 负责打 zip 配置包（ConfigVersion 产物）与心跳响应 `config_changed` 判定，M11 Agent 负责拉取与应用；§6.2 / §6.3 edge 协议权威定义迁 M11，M09 引用。
- **NetworkDomain 三家共管**：M06（行政）/ M11（监控纳管 + 运行态）/ M09（采集边界 + channel 只读），三方 PRD §5.1 职责声明同步改口径。

### 用户故事迁移映射

M09-ARCH-11 / M09-ARCH-12 / M09-OPS-11 / 12 / 13 迁移改码为 **M11-ARCH-11 / M11-ARCH-12 / M11-OPS-11 / 12 / 13**（语义不变）；M09-OPS-14 / 15 / 16（配置确认 / 下发 / 回滚）留 M09；新增 M11-OPS-14（下载离线安装包）、M11-OPS-15（版本差异与升级）、M11-OPS-16（组件重启与熔断状态可观测）。全局库 `01_User_Stories.md` 模块级用户故事扩展章节同步登记。

### 执行顺序

1. 本决策落档（本文）。
2. M11 PRD v0.1 新建（draft）：迁移 M09 边缘域内容 + 新增章节（下载入口 / 升级维护 / 进程守护工程化 / 协议补全），用户故事登记全局库。
3. M09 PRD 瘦身至 v2.0：迁移内容移除、内部锚点修复、`channel` 等保留项改只读引用 M11。
4. `Modules/README.md` 模块目录、`Module_00_Integration_Map.md` 集成地图登记 M11。
5. 全库锚点盘点修复：M01 / M06 / M07 / M08 / M10 及原型、执行记录中对 M09 §3.1 / §3.2 / §5.2 / §5.3 / §6.2-6.4 / §8.3-8.5 的引用改指 M11。
6. 原型目录拆分：`docs/prototypes/module-11/`（纳管页 / 节点状态页迁移），module-09 保留配置确认页 / 下发记录页。

### 历史与追溯

module-09 的 design-decisions / task-sequence / dev-feedback / tech-feasibility / api-contract-snapshot **原地保留不迁移**；M11 新建执行记录目录 `docs/05-execution-records/module-11/`，引用旧决策编号；决策 86 起 M11 域新决策落 module-11 目录（决策编号沿用全局计数）。

### 影响范围

M09 PRD（瘦身 v2.0）、M11 PRD（新建 v0.1）、`Modules/README.md`、`Module_00_Integration_Map.md`、`01_User_Stories.md`、`docs/prototypes/module-09|11/`、跨模块 PRD 锚点（M01 / M06 / M07 / M08 / M10）、开发分支规划（v0.2 起 `feat/module-11-*` 与 `feat/module-09-*` 分线）。

---

## M09 PRD 瘦身 v2.0 执行记录（2026-09-17，决策 86 执行第 3 步）

> 范围：按决策 86 拆分边界表执行 M09 PRD v1.79 → v2.0 瘦身，配置面规格语义零变更。1653 行 → 约 1090 行。
> 删除章节：§3.1 网域纳管、§3.2 采集节点状态、§3.6.2 安全与证书、§3.6.3 边缘交付、§4.3 网域接入动线、§5.2 EdgeAgent、§5.3 EdgeHeartbeat、§6.1-6.4 edge 协议、§6.6.1/6.6.4 纳管与节点状态 API、§8.3-8.5 状态机、§11.3 网域纳管页、§11.4 采集节点状态页；剩余章节整体前移重编号（3.3→3.1、3.4→3.2、3.5→3.3、3.6→3.4、4.4→4.3、5.4-5.7→5.2-5.5、6.5-6.9→6.1-6.5、11.5→11.3），正文引用同步更新。
> 保留项改只读口径：§5.1 NetworkDomain 压缩为三家共管声明（M06 行政 SSOT / M11 监控纳管 Owner / M09 只读 `channel`）；§6.1 校验分层保留、边缘侧行为改 M11 §6.4 指针；§7.1.2/7.1.3 边界表与 §7.2 技术依赖同步调整；§9 删除边缘验收 37 条；§10 边缘术语词条删除 6 条、改写 5 条加 M11 指针；§11.1 删除两页矩阵、§11.2/11.3 措辞收敛。

### Change Log 轮转出的 v1.77 行（主表只留最近 3 版）
...
### 决策 87：原型与开发分支共用 Module_09（2026-09-17）

**背景**：决策 86「M09 1拆2」原规划 M11 原型迁移至 `docs/prototypes/module-11/`、开发分支独立 `feat/module-11-*`。实际推进时评估：两模块交互性极强（网域纳管 → 采集节点状态 → 配置确认页「前往确认」 → 下发 → 边缘 Agent 状态回显形成强闭环，原型靠跨页桥演示才成立），且接入面原型内容量小，单独拆目录 / 分支收益很低。

**决策**：
- **原型不拆分**：M09 与 M11 共用 `docs/prototypes/module-09/` 原型目录。原计划迁出的网域纳管页 / 采集节点状态页骨架**保持原位**，M11 新增页面（离线包升级、进程守护等）同样落在该目录；`module-09` 原型 README 的模块范围说明覆盖 M09 + M11。
- **开发分支共用**：M11 开发并入 `feat/module-09-*`（或 `feat/module-09-11-*`），不新建 `feat/module-11-*`；代码仍按 `platform/` 下 M11 对应目录实现。
- **PRD 层面保持两个独立文档**：M09（配置面，v2.0）与 M11（接入面，v0.2）各自维护；M11 原型声明指向共用目录。
