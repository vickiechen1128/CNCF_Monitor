...
> **回写完成（2026-09-25）**：F-28 已合入 M09 PRD v2.1——§5.1 明确 `channel` 由 `domain_type` 派生（单一事实来源、未知不回退 `local`）、§6.4.1 MVP 边界同步措辞；M09 契约快照 §3/§9 `channel` 已同步。
...
> **回写完成（2026-09-25）**：已合入 M09 PRD v2.1 §3.2.3（空 targets 拦截，与边缘拒收对称）+ §3.3.3（地址解析不出不得静默放行），实施已完成、仅余回写已闭合。
