...
> **验证的 PRD 版本**: [Module_09_Network_Domain_and_Edge_Config_Center.md](../../02-product-requirements/Modules/Module_09_Network_Domain_and_Edge_Config_Center.md) v1.72
...
## v1.72 变更说明（同步 PRD v1.71→v1.72，决策 82「网域生命周期闭环」的 M09 侧落点 + 列名粒度澄清）

1. **网域纳管页第 3 列「运行状态」→「采集节点在线」（列名粒度澄清，chenrt 走查确认）**：该列原本与节点页「整体状态」同名却指代不同对象（**网域粒度聚合** vs **节点组件聚合**），MVP 下 1 网域 : 1 采集节点使两者数值恒等，极易被误判为**冗余列**。更名 + 列头 tooltip 点明「本网域采集节点的聚合心跳状态；节点与组件级诊断见『采集节点状态』页」，明确两页为「网域层概览 → 节点层诊断」的层级关系、**不是重复**；同时明确「纳管状态」列（配置态：接没接入监控）与「采集节点在线」列（运行态：在不在线）职责正交。
2. **决策 82 的 M09 侧义务在 MVP 无 UI 落点**（原型不改行为，此处仅登记）：①**网域删除级联清退**（82-1，MVP）由 M06 网域管理发起，M09 侧为**同请求内的后端义务**（废止 Token / 停止配置下发 / `EdgeAgent` 记录标 `retired`），本模块**不提供删除入口**（既有边界不变），原型无独立界面可演示；②**退纳管动作**（82-3）立项 **v0.2+**，MVP 不实现，故原型不加入口。
3. **`retired` 归属（与 PRD 对齐）**：`retired`（已清退）标记的是**该网域的 `EdgeAgent` 采集节点记录**（该记录的生命周期终态、保留供审计追溯），**不是**网域 `registration_status` 的取值——网域被删后本身软删、从本页列表消失，不进入纳管状态机；原型 `EdgeAgent` 现有 `status: unknown / online / offline` 保持不变，`retired` 为级联清退时的记录级标记，MVP 无可视界面。
4. **验证**：`pnpm build`（tsc + vite）/ `pnpm lint`（0 warning）/ `pnpm check:prototype` / `pnpm test`（54 passed）全过。

## v1.71 变更说明（决策 74：采集节点状态页两列与 M06 对齐裁定，PRD + 原型同轮落地）

1. **网域列不再展示接入方式 Tag（决策 74-1）**：本页实例按定义均属采集节点域（中心直连域不产生 `EdgeAgent` 实例、仅出现在空态说明），**恒定信息不进列**；接入方式归网域纳管页网域列（决策 70-1）。顺带修掉原型旧 Tag 直接用裸枚举 `agent_pull` 的口径问题。
2. **配置同步列形态定版 = Badge + 成因分档（决策 74-2）**：不采用 M06「接入进度」点阵——点阵表达**单调递进**生命周期（走完不回退），本列为**循环状态机 + 成因分支**（「生效中」是 `out_of_sync` 的成因之一而非更靠后的步骤），且状态由心跳自动流转、无需人工推进。列头 Tooltip 给一句短定义，每格悬浮展示该档明细（替代原「五档全口径塞进列头」）。
3. **mock 跨页流转演示（决策 74-3，原型行为）**：跨页流转桥——在配置变更确认页确认 `agent_pull` 变更后写入「生效中」，切到采集节点状态页重新挂载可见，约 10s 由心跳模拟自动流转为「已同步」；「立即同步」的结果回写该桥，**跨页不回退**。
4. **验证**：`pnpm build` / `lint` / `test`（54 passed）/ `check:prototype` 全绿 + puppeteer 截图走查 14/14 PASS（含流转全闭环）。

## v1.70 变更说明（决策 70~72 九条 PRD 显性化回写，原型版本轴对齐，**无代码改动**）

1. 本轮为**原型先行内容追认**：PRD 侧把已在原型中实现/体现的九条口径正式回写，原型本身**行为不变**，`package.json` 仅做版本轴对齐（1.69.0 → 1.70.0）。
2. PRD 落点摘要：§3.1 网域列表 + §9 验收改 **5 列口径**（网域列三合一：名称 + 网域 ID 等宽字体 + 接入方式 Tag；`zone_type` 下沉详情 Drawer）；**安装命令口径改静态模板**（`<网域 ID>` / `<凭据>` 占位符、不含真实凭据，「行内复制凭据」的动作不受限）；「纳管取凭据」为前置步骤、不占 3 步编号；§9 新增三条验收（详情抽屉不承载组件明细 / 摘要区三态出口 / 采集节点状态页支持 `network_domain` 深链）。

## v1.69 变更说明（决策 73：网域-采集节点部署口径 + 两类地址方向性指引，原型同步）

1. **安装指引面板补两句口径**：**MVP 单节点部署口径**——一个网域只需在网域内挑一台常开机器安装**一个**采集节点即可覆盖全域目标（域内互通），安装指引不引导多节点部署；分片 / HA / 多探测点维持 v0.4+ 演化。**中心接入地址方向性指引**——机器如何到达中心由纳管登记地址决定，需转发场景请先确认登记。
2. **纳管 / 编辑表单 `center_endpoint` / `remote_write_url` 的 `extra` 重写为方向性指引**：两类地址方向**均为「采集节点 → 中心」**（直连 → 填中心地址；经 nginx / 网闸转发 → 填转发侧地址；回传地址通常可由中心接入地址 + `/api/v1/write` 自动推导）。
3. **决策 70 两条遗留处置**：操作列「编辑」补 600 字重（对齐《前端标准》§8 主操作）；mock 的心跳 / 配置拉取时间改为**相对当前时间生成**（不再写死日期）。
4. `package.json` **1.52.0 → 1.69.0**（版本轴一次性对齐：v1.53~v1.68 期间原型未改动，故无对应版本条目）。**验证**：`pnpm build` / `lint` / `test`（54 passed）/ `check:prototype` 全绿 + puppeteer 4 状态截图走查。

> **关于本 README 的版本缺口（2026-09-15 已处理）**：v1.52 之后原型版本轴未发生中间版本——v1.53~v1.68 期间 PRD 多次修订但**原型均未改动**（记录见各版「原型不动 / 维持 v1.52.0」），因此缺失的**原型**版本条目只有 **v1.69 / v1.70 / v1.71 三版**，已于本轮按 [module-09/design-decisions.md](../../05-execution-records/module-09/design-decisions.md) 与 `Modules/README.md` §5 Change Log 忠实回填，**缺口已闭合**。注意：今后本 README 的标题版本号是**原型版本**（= `package.json` 的 `version`），不要与 PRD 版本混用。

## v1.52 变更说明（决策 60：alertmanager.yml 纳入变更确认，口径反转，2026-09-01）

- **alertmanager.yml 纳入本模块变更确认（决策 60，口径反转）**：原 v1.32 口径「alertmanager.yml 由 Module_08 直接管理、不进入本模块变更确认流程」反转——alertmanager.yml 作为**管理域（default）scope 产物**纳入本模块变更确认：Module_08 文件挂载提交 → 本模块变更单人工确认 → 确认后由本模块写中心 Alertmanager 配置路径并触发 Alertmanager reload，`change_status` 回写 Module_08；**不参与按网域扇出、不进入 agent_pull 配置包**（仅作用于中心 Alertmanager）。
- **新增演示**：待确认变更单 `draft-default-am-001`（CHG-20260831-013，default 域）——alertmanager.yml 政务云路由通知节奏调整（group_wait 30s→15s、group_interval 5m→2m），与 Module_08 原型配置版本 `acv-004`（change_status=pending）呼应，M08「待确认」点击直达本页（`#/config-preview`）。变更对象新增「通知配置」、影响文件新增 `alertmanager.yml`。
- 本批为 M08 契约的同步呈现（决策 60），版本声明 / ReviewNote / 原型版本同步 v1.52。
...
- `/network-domains`：网域纳管（决策 34/35，菜单常驻）— 列表列收敛为 5 列（决策 36-1 + v1.53 列数治理 + v1.70 网域列三合一 + v1.72 列名粒度澄清）：网域（名称 + 网域 ID 等宽字体 + 接入方式 Tag 三合一）、纳管状态（`created`/`monitored`，配置态）、**采集节点在线**（状态+最后心跳合并，仅 agent_pull 展示；v1.72 由「运行状态」更名，网域粒度聚合的运行态）、凭据（脱敏 Token+复制图标，仅 agent_pull 展示）、操作（三槽位固定结构：主操作=纳管/编辑随行状态变化+文本链接样式+详情常驻+更多仅 agent_pull 已纳管行显示重置 Token+二次确认）；网络分区（`zone_type`）与下发通道已下沉 / 并入网域列；center_endpoint/remote_write_url/agent_type/description 等低频配置字段进右侧详情 Drawer；Token 在 UI 中完全脱敏展示（不显示任何明文片段，完整值仅可通过复制按钮获取）；安装指引为页面顶部常驻提示区（决策 17，而非每行入口/弹窗）——「新网域接入操作流程」：3 步人工步骤 + 边缘节点组件构成 + 凭据获取方式，注册成功后自动滚动高亮；登记制+闭环（决策 14）：最小化表单，Agent 类型下拉 MVP 仅 vmagent 可选；字段语义对齐（决策 16）：配置字段 vs 运行态字段，列头 Tooltip + 页脚说明来源；网闸拓扑（{v1.31}）：纳管表单 center_endpoint 必填，安装指引含网闸/隔离区连接约束说明
...
- `/config-preview`：配置变更确认（决策 18/19/20/21/22）— **「变更单号」为主标识的变更列表（决策 20）** + 人话变更摘要 + **「风险等级」「确认人」列（决策 19）** + **「已发布版本」列（决策 22：已确认变更显示 cv-xxx + 「记录」快捷入口）**、**变更状态筛选（决策 21：待确认 / 已确认 / 已废弃 / 全部，默认待确认）**、**点击行打开右侧抽屉查看变更详情（决策 20：变更清单为核心 + 配置预览/diff + 技术信息折叠 + 确认/废弃按钮）**、**变更对象 = 源数据对象枚举 + 「影响的配置文件」列（决策 22）**、**变更单级确认（决策 22，整单 go/no-go）**、**抽屉「查看发布记录」入口（决策 22）**、**变更检测状态引导性（决策 20）**、**YAML 预览受影响文件高亮（决策 19）**、**审批分级策略（{v1.32}；决策 60 口径反转）**（prometheus.yml / targets / rules.yml / blackbox.yml 人工确认；alertmanager.yml 作为管理域 default scope 产物纳入本模块变更确认——Module_08 文件挂载提交，人工确认后写中心 Alertmanager 配置路径并 reload，change_status 回写 M08，不参与按网域扇出 / 不进 agent_pull 配置包）、**rules.yml 按 Prometheus group 语法组织（{v1.32}，分组自动派生、MVP 不暴露 RuleGroup 实体）**、配置产物结构（按域类型分层：default 管理域=本地文件集，边缘域=zip 配置包含 metadata.json，含 targets/ 目录；采集产物中不含 alertmanager.yml 内容，alertmanager.yml 为管理域 default scope 独立产物）、**external_labels 注入 zone_type（{v1.31}，登记了才注入）**、targets 前端数据驱动（动态遍历 targets_files 渲染子 Tab，新增 job 无需改前端）、file_sd 骨架与 targets 原子写说明、多文件 Tabs 预览 / 按文件 Diff、校验失败 Tab 定位、下发前校验提示、校验分层说明、**网闸 / 隔离区连接约束（{v1.31}，边缘域发布通道）**、**页面底部「原型与实现说明（面向产品 / 技术评审）」折叠区（决策 21：决策清单与 PRD 指引集中承载）**
...
Content 顶部提供可关闭的「Module_09 设计意图」Alert：监控对象、采集策略与告警规则变更后配置自动生成并汇总为待确认变更；运维在「配置变更确认」页做发布审批（go/no-go）——平台保证生成内容与策略一致，运维确认变更影响后决定是否发布；配置按网域生成且**产物形态按域类型分层**（中心管理域 default=本地文件集、无 zip/metadata.json；边缘域=zip 配置包 + metadata.json）；**审批分级（{v1.32}；决策 60 口径反转）**——prometheus.yml / targets / rules.yml / blackbox.yml 人工确认，alertmanager.yml 作为管理域 default scope 产物纳入本模块变更确认（Module_08 文件挂载提交），人工确认后由本模块写中心 Alertmanager 配置路径并触发 reload，不参与按网域扇出、不进 agent_pull 配置包。
