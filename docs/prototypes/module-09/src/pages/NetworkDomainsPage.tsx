    ...
    <MainLayout>
      <Card title="网域纳管">
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              // {v1.72} 列名明确粒度（决策 82 配套，chenrt 走查确认）：本列是「该网域采集节点」的心跳状态
        ...
        <ReviewNote title="字段语义说明（面向评审）" style={{ margin: '12px 0 0' }}>
          {'{v1.53} 列数治理：7 列 → 5 列（网域 / 纳管状态 / 采集节点在线 / 凭据 / 操作），符合《前端标准》§9.4 建议 ≤8 列。'}
          {'{v1.72} 第 3 列由「运行状态」更名为「采集节点在线」（决策 82 配套，chenrt 走查确认）：该列是本网域采集节点（Edge Sync Agent）的心跳状态、网域粒度聚合视图，与「采集节点状态」页的节点/组件级诊断构成「网域层概览 → 节点层诊断」的层级关系、不是重复；MVP 1 网域 : 1 采集节点使两者数值一致，原列名「运行状态」与节点页「整体状态」指代不同对象却同名，易被误判为冗余列，故更名并加列头 tooltip 点明粒度。「纳管状态」列回答「接没接入监控」，「采集节点在线」列回答「采集节点在不在线」，两者一为配置态、一为运行态，独立维护。'}
          ...
          中心直连域（default）由中心直接采集，不签发凭据、无采集节点安装步骤，故采集节点在线与凭据显示「-」；
          采集节点域的凭据在纳管时自动签发，采集节点在线状态由心跳上报更新；
