...
> **验证的 PRD 版本**: [Module_07_Monitoring_Object_Management.md](../../02-product-requirements/Modules/Module_07_Monitoring_Object_Management.md) v2.21
...
1. **资源管理（MVP / {v2.6} / {v2.18} 业务归属）**：四类资源（主机 / 中间件 / 应用服务 / 通用指标目标）的固定列列表、新增 / 编辑 / 删除、网域归属；**{v2.16} 已取消监控状态列 / 详情（采集状态归 M01/M02，M07 不展示）；{v2.20} 决策 31-M1 恢复为只读映射：新增「采集状态」列与「未监控」筛选**。**{v2.6} 网域仅作为列表筛选器**：M07 顶部不再提供全局网域上下文切换器，资源列表操作区提供「网域筛选」下拉（默认「全部网域」、可切单个网域），与搜索框并列。**{v2.18} 网域与业务双归属正交维度**：新增「业务」列与「业务筛选」下拉，展示业务分组字典展示名（`biz_name`），停用业务以「（已停用）」标识；业务列取不可变编码 `biz_code`，仅启用字典项可选。
