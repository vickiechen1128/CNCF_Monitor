...
> **验证的 PRD 版本**: [Module_01_Metric_Collection_Center.md](../../02-product-requirements/Modules/Module_01_Metric_Collection_Center.md) v3.41
...
## 本次 v3.41 相对 v3.37 的关键变更（决策 67-2 + 69：规则挂载「检查 → 提交」两段式）

> 本轮为**一次补齐两笔欠账**：v3.40（决策 67-2）的 job 引用分级阻断与逃生门原型从未呈现，v3.41（决策 69）的独立「检查」按钮与提交按钮状态机同期落地。

- **{v3.41} 规则挂载抽屉改为「检查 → 提交」两段式（决策 69-2）**：footer 由「取消 + 挂载并提交」改为**「取消 + 检查」常驻**，「**提交并进入变更确认**」按钮**条件渲染**（`canSubmit = 检查通过 且（无 error 级 job 引用 或 已勾选逃生门）`）——未检查 / 检查未通过时**按钮不出现**（非 disabled）。原按钮文案「挂载并提交」作废，与生产终态及 M08 参照物统一。
- **{v3.41} 独立检查结论与三类面板（决策 69-1 / 69-3）**：点「检查」后按后端 `validate-yaml` 口径做本地等价预检——① `valid=false` 硬失败：YAML 语法 / `groups` 结构、**组名全局唯一性冲突**（与既有**生效**规则文件比对，停用文件不参与，镜像提交侧条件）；② `job_ref` 分级：解析 `job="..."` / `job=~"..."` 与 `mockScrapeJobs` 中**生效** Job 名单比对，存活类（`up` / `absent(up)`）缺 job = **error**、其余 = **warning**。结果面板落在**表单下方、操作按钮上方**：「未通过（红）」/「通过（绿，含『提交时服务端仍会复核』边界说明）」互斥，job 引用提示按 severity 呈红/黄。
- **{v3.41} 逃生门提前到检查阶段（决策 67-2）**：error 级 job 引用不再等提交被拦才出现——检查阶段即展示逐条问题清单 + 复选框「已知晓：先挂规则，稍后补建 Job」，勾选后提交按钮出现（对应提交携带 `ack_job_ref_errors=true`、问题降级 warning 落库留痕）。
- **{v3.41} 内容变更即作废检查结论（决策 69-2）**：仅**规则内容**变更触发重置——检查结论与逃生门勾选一并作废、提交按钮重新隐藏并提示「请重新检查」；**规则文件名称**变更不触发（避免无效劳动）。
- **{v3.41} 文件选择入口维持在编辑框上方（决策 69-3）**：原型原本就是「字段说明 → 选文件 → 编辑框」的垂直读序，本轮**无需改动**——生产侧当初把它放到 16 行文本域下方并被顶出折叠线，v3.41 按原型回归。
- 连带说明：`package.json` 版本此前滞留在 `3.28.0`（v3.37 那次同步漏提），本轮一次性对齐 `3.41.0`。
- 对齐 Module_01 PRD v3.41 / 原型 v3.41；本轮为原型行为同步，与 PRD 落版同步提交。
