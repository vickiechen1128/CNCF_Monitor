...
3. **规则编辑**：告警 / 记录规则编辑，`rule_type=recording` 时隐藏 `duration` 与 `annotations`；Labels/Annotations key-value 动态表单；资源类型两级级联选择，选中 CI 类型后自动带出映射默认采集器（可覆盖）；PromQL 保存前强制校验，expr 引用的指标必须存在于指标库（失败给具体错误如「未知指标名 xxx」；{v3.8} 按 CI 类型校验，同名指标显示来源区分）；指标库数据基于**当前页面状态**（用户新增/禁用实时生效）；{v3.8} 选中 CI 类型后按该类型指标集过滤预览；P1「规则模板一键填充」占位按钮。
