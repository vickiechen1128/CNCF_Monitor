...
提供 MetricCenter 平台级管理能力，包括多租户隔离、数据存储管理、审计日志、平台全局配置。
...
| 功能 | 说明 |
|------|------|
| TSDB 状态查看 | 展示 Prometheus TSDB 状态（通过 `/api/v1/status/tsdb`） |
| Retention 配置 | 设置数据保留周期 |
| Remote Write 配置 | 配置 VictoriaMetrics / Mimir 等长期存储转发 |
...
| 功能 | 说明 |
|------|------|
| 全局 scrape 默认值 | 默认 `scrape_interval`、`scrape_timeout` |
| 通知默认配置 | 默认接收人、告警模板 |
