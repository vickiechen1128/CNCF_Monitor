...
> **原型版本**: v2.13（✅ 已对齐，2026-09-14：硬劝阻 / 术语「中心直连域·采集节点域」/ 引导信息归位，见决策 79）
> **更新日期**: 2026-09-14
...
- [ ] {P0} 「网域管理」页首展示概念卡（网域 = 网络可达性区域的一句话定义 + 「中心能否直连」二选一判断规则 + 拓扑示意）；空态为提问式引导「你的所有机器都能被平台中心直接访问吗？」（MVP，决策 73 / 72）
- [ ] {P0} 新建网域表单第一问为「这个区域的机器，平台中心能直接访问吗？」；选「能」为**硬劝阻终点**——提示「无需新建网域（能被中心直接访问的机器统一放在中心直连域 `default`，由中心直接采集），请关闭本窗口」，表单不展开、无继续填写路径（决策 79）；选「访问不到」才展开行政登记字段，表单顶部显示身份说明「你正在登记一个采集节点域」+「什么是网域？」概念详解折叠入口（MVP，决策 75 / 79）
...
- [ ] {P0} 页首概念卡与「网域接入指引」合并为一个**可折叠面板**的两个区块——「什么是网域」（一句话定义 + 判断规则 + 拓扑示意）与「怎么接入」（四步生命周期：登记 → 纳管取凭据 → 装采集节点 → 配采集出数据，同时解释列表「接入进度」四态列含义）；折叠状态记 `localStorage`（MVP，决策 79）
- [ ] {P0} `zone_type` 以「网络分区（可选）」展示，表单提示「不确定可留空，仅用于分类筛选」，选项带部署级字典说明（政务云安全分区 / 公有云 region 含义）（MVP，决策 76）
- [ ] {P0} {v0.2} 网域列表展示「接入进度」四态（已登记 → 已纳管 → 采集节点已上线 → 已出数据），行内主操作 = 下一步动作（去纳管 / 查看安装指引 / 去配置采集 Job 跳 M01 预选该网域）（决策 75）
...
- **登记域类型（只读）**：网域登记（新增）时「域类型」固定为**采集节点域**（`edge`，用户侧叫法，决策 79），只读、不可选择；管理域为系统预置（`default`），不可通过登记创建，提示语「中心直连域为系统预置，由平台管理员维护」。
- **跨模块跳转**：网域列表可跳转 Module_09 网域纳管（预选该网域）；不重复提供网域纳管的监控参数维护入口。
- **页首概念卡与接入指引合并面板（决策 73 / 75 / 79，MVP）**：网域管理页首为一个**可折叠面板**（折叠状态记 `localStorage`），含两个区块——①「什么是网域」：网域 = 网络可达性区域的一句话定义、「中心能否直连」二选一判断规则、三行拓扑示意（中心 ─直连→ default 中心直连域机器；中心 ─✕→ 采集节点域 ← 采集节点回传）；②「怎么接入」（可折叠「网域接入指引」）：四步生命周期**登记 → 纳管取凭据 → 装采集节点 → 配采集出数据**，同时解释列表「接入进度」四态列的含义（已登记 / 已纳管 / 采集节点已上线 / 已出数据与四步一一对应）。
- **新建表单前置判断问题（决策 75 / 79，MVP）**：表单第一问「这个区域的机器，平台中心能直接访问吗？」；选「能」为**硬劝阻终点**——展示 warning「无需新建网域——能被中心直接访问的机器统一放在中心直连域（`default`），由中心直接采集。请关闭本窗口。」，**表单不展开、无继续填写路径**（三条理由：管理域全系统唯一不可登记 / 强制 `agent_pull` 通道与用户声明自相矛盾 / 「能直连」与「建域」语义互斥；独立授权诉求走「授权租户」字段，见决策 18~20）；选「访问不到」→ 展开行政登记字段，**表单顶部放身份说明**「你正在登记一个采集节点域：中心访问不到这个网络，登记后需在其中一台常开机器上安装采集节点」，旁附「什么是网域？」展开链接（点开显示两种域的区别 / 拓扑 / 登记不采集数据，不强制阅读）；**抽屉内不放置后续步骤预告**（3 步预告由成功行动卡与外层接入指引承载，决策 79）。
...
> 完整 Change Log 历史（v2.10 及以前）见 `docs/05-execution-records/module-06/design-decisions.md`「Change Log（完整历史）」。
