...
> **PRD 状态**: `dev-ready`（Track B 增量 v2.3，2026-08-28 用户书面确认；v2.2 及之前内容为 ready）
> **PRD 版本**: v2.3
> **产品版本覆盖**: MVP / v0.2 / v0.4 / v1.0
> **原型版本**: v2.3（Track B 增量免高保真原型，决策 44）
> **更新日期**: 2026-08-28
...
>
> **MVP 轻量认证声明（决策 44，修订决策 8）**：MVP 实现**轻量认证**——用户名 + 密码登录、会话 Token、认证中间件拒绝匿名请求（由 [Module_03](Module_03_Gateway_and_Auth.md) 落地）；但**无授权隔离**：不做角色 / 权限点 / 租户数据隔离，所有登录用户在单租户 `platform_admin` 下等价，「无授权隔离」属**已知风险**，依赖部署网络隔离（内网 / VPN）兜底；本文所述「不存在跨租户的全局可见性」描述的是**数据模型与设计目标**，不代表 MVP 已启用授权拦截。生产化前必须完成授权体系接入。
>
...
> **MVP 租户/用户管理子集（决策 44，Track B）**：MVP 纳入「租户管理（仅 `platform_admin` 单租户的查看 / 编辑，不开放新建、唯一租户禁止禁用）」「用户管理（租户内用户 CRUD + 重置密码，无角色体系）」「登录日志查看」；登录认证机制由 [Module_03](Module_03_Gateway_and_Auth.md) 承接。初始管理员账号由后端启动时 migration upsert 预置（与决策 23 种子机制一致）。
...
> MVP 落地最小用户模型；角色 / 权限字段不引入（v1.0+）。用户必须挂靠租户（决策 8 第 4 条预留不变），MVP 固定 `platform_admin`。
...
> **初始管理员**：`admin` 账号由后端启动时 migration upsert 预置（幂等，与决策 23 种子机制一致）；初始密码为部署配置项（默认 `admin123`），首次登录引导改密。
...
| message | string | ❌ | 仅技术信息 | 失败原因（密码错误 / 账号禁用等） |
...
- [ ] {P0} 初始管理员 `admin` 由后端启动时 migration upsert 预置，幂等可重复执行（MVP，与决策 23 种子机制一致）
- [ ] {P0} 密码以 bcrypt 哈希存储；任何接口与日志不返回明文或哈希（MVP）
- [ ] {P0} 匿名请求（除登录接口与健康检查外）被认证中间件拒绝并返回 401；登录后持 Token 可正常访问（MVP，Module_03 落地）
- [ ] {P0} 无授权隔离：所有登录用户等价，接口不校验角色 / 权限点 / 租户隔离（MVP 已知风险，决策 44）
...
| `User` | 用户 | 租户内的登录账号与操作人；MVP 无角色，所有用户等价 |
...
- **侧栏承接二级页面导航**：模块内各功能页面（租户管理、网域管理、用户与权限、审计日志、平台配置等）通过**左侧栏**做二级页面导航，顶部一级 tab 不展开二级菜单。
...
| 登录页 | 登录失败 | 表单内错误提示「用户名或密码错误」（不区分具体原因，防账号枚举）；连续失败提示稍后重试 |
| 登录页 | 未登录访问 | 路由守卫跳转登录页，登录成功后回跳原页面 |
| 用户管理 | 加载中 | 表格骨架屏 |
