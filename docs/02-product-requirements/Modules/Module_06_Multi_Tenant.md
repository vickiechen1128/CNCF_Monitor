...
> **原型版本**: v2.11（✅ 已对齐，2026-09-14：概念卡 / 空态 / 前置判断 / 行动卡 / 术语，见决策 75）
> **更新日期**: 2026-09-14
...
> - **判断规则（二选一）**：中心能直接访问 → 不用建网域（放 `default` 中心直连区）；中心访问不到（专网 / 隔离 DMZ / 另一个 VPC / K8s overlay 集群）→ 建一个网域，并在该网络内挑一台常开机器安装**采集节点**，由它把数据带回中心；
...
| domain_type | enum | ✅ | 接入方式 | 管理域（`management`，如 `default`）/ 边缘域（`edge`）；管理域禁止删除。**用户侧叫法（决策 74）**：`management` = 「中心直连区」、`edge` = 「隔离区（采集节点接入）」；用户侧只呈现只读「接入方式：中心直连 / 采集节点接入」（按网域自动推导、不可选择），模型枚举与后端术语不变 |
...
- [ ] {P0} 新建网域表单第一问为「这个区域的机器，平台中心能直接访问吗？」；选「能」提示通常无需建域，选「不能」才展开行政登记字段并预告后续三步（纳管取凭据 → 装采集节点 → 上线后配采集）（MVP，决策 75）
- [ ] {P0} 网域创建成功后展示行动卡（结果弹窗：「还需 3 步产出数据」+ 直达 M09 纳管按钮），不再仅为 toast 提示（MVP，决策 75）
- [ ] {P0} 网域列表「接入方式」列按用户侧叫法展示（中心直连 / 采集节点接入），不出现「管理域 / 边缘域 / Edge Sync Agent」字样（MVP，决策 74）
...
| `domain_type` | 接入方式 | 管理域（`default`）/ 边缘域（行政分类，M06 维护）；**用户侧叫法（决策 74）**：中心直连区 / 隔离区（采集节点接入），页面只读展示「接入方式：中心直连 / 采集节点接入」 |
...
- **页首概念卡（决策 73 / 72，MVP）**：网域管理页首常驻概念卡——网域 = 网络可达性区域的一句话定义、「中心能否直连」二选一判断规则、三行拓扑示意（中心 ─直连→ default 区机器；中心 ─✕→ 隔离区 ← 采集节点回传）；可关闭并记住 `localStorage`。
- **新建表单前置判断问题（决策 75，MVP）**：表单第一问「这个区域的机器，平台中心能直接访问吗？」；选「能」→ 提示通常无需建域（或登记为中心直连区）；选「不能」→ 展开行政登记字段，表单底部预告后续三步（纳管取凭据 → 装采集节点 → 节点上线后配采集）。
- **登记成功行动卡（决策 75，MVP）**：创建成功展示结果弹窗（非 toast）：「网域已登记，还需 3 步产出数据：① 去纳管获取接入凭据 → ② 在该网域一台常开机器上安装采集节点 → ③ 节点上线后即可为该网域资源配置采集」，附直达 M09 纳管按钮。
...
- **用户侧术语（决策 74）**：页面与用户文案一律使用「中心直连区 / 隔离区（采集节点接入）/ 采集节点 / 网络分区（可选）」；「管理域 / 边缘域 / Edge Sync Agent / 网络区域类型」仅出现在技术层（接口契约 / 代码注释 / 折叠说明），不作为用户界面文案。
...
| v2.10 | 2026-09-10 | 修改 | 租户标签键定版（决策 68-5，源自 M02 §7.1 原「遗留待决」升级）：§2 用户故事表「多租户数据隔离」行的注入名由 `tenant_id` 更正为 **`tenant`**——**标签键一律不带 `_id` 后缀**（`_id` 只属 DB 列与 API 字段，通用命名规约）；租户标签唯一来源为 Module_07 LabelTemplate 的 **target 级注入**（非 Module_09 `external_labels`）；v0.2 配套 fail-closed **严格派**语义与 Module_09 生成期门禁。本模块「租户不进入采集拓扑」的既有定性不变。详见 `docs/05-execution-records/module-09/design-decisions.md` 决策 68-5 | 2 | v0.2 | dev-ready |
> 完整 Change Log 历史（v2.9 及以前）见 `docs/05-execution-records/module-06/design-decisions.md`「Change Log（完整历史）」。
