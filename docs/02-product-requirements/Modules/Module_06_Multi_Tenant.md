...
> **原型版本**: v2.15（✅ 已同步决策 82——已纳管网域删除入口恢复可用 + 「删除网域（退场回收）」级联清退二次确认弹窗（含级联影响清单）/ 禁用弹窗补强（【重要】行政冻结提示）/ 「更多」菜单不可操作原因修正，见 `docs/prototypes/module-06/README.md` v2.15 变更说明；v2.14 已对齐 2026-09-14 引导条扁平化 / 表单改抽屉 / 判断口径澄清 / 列表列数治理 + 详情抽屉，见决策 80）；v2.16 为 `zone_type` 字典命名口径与「分区唯一权威」跨模块声明（决策 83），属契约级增量、**原型无需同步**；v2.17 为实体模型收敛（NetworkDomain 增 `cloud_code` 必填 + `zone_type` 必填 + 位置三标签 system 层 + region 字典取值，决策 78 / M07 决策 103），属契约级修订、原型无需同步
> **更新日期**: 2026-09-26
...
**唯一入口原则（单一事实来源）**：网域定义（名称 / 登记归属 / 授权租户 / `zone_type`）只在 M06 登记一次，下游模块（M07 资源导入/录入、M09 纳管、CMDB 同步）**只引用** `network_domain_id`，不在资源上冗余区域属性（[Module_07 5.4](Module_07_Monitoring_Object_Management.md)）。**网络分区（`zone_type`）同样遵循本原则——分区为网域级唯一权威，资源侧不冗余该属性；M07 主机详情如需展示分区由所属网域只读派生（决策 83）。**

**`zone_type`（网络分区，必填）**：网域的网络隔离/位置**分类标签**（决策 78 / M07 决策 103，**必填**）；腾讯云中分区 = 地域（Region）这一真正隔离边界——同地域内网互通、跨地域默认不通，AZ 不进 `zone_type`；字段模型、部署级字典（政务云安全分区：互联网区 / 政务外网区 / 专线区 / DMZ；公有云 region：上海 / 北京等）、UI 展示名见 §5.2；`zone` 经所属网域派生、在 target 级 system 层注入 Prometheus label（决策 103，非 external_labels）；用户侧一句话语义与双路由判断规则见 §11.2「登记前置判断」。禁止方向：不做死枚举、不赋予行为语义（如按 `zone_type` 决定采集通道）。
...
| zone_type | string | ✅ | 网络分区（必填） | 网络隔离/位置语义**分类标签**（决策 78 / M07 决策 103，**必填**——网域登记须选填分区，分区承载腾讯云地域（Region）这类真正隔离边界：同地域内网互通、跨地域默认不通，AZ 不进 `zone_type`）；值集为部署级字典（政务云 = 安全分区：互联网区 / 政务外网区 / 专线区 / DMZ；公有云 = region：上海 / 北京等），见 [3.2](#32-租户与网域关系)；下拉数据来自 `GET /api/v2/platform/zone-types` 只读接口（见 6.2），不开放自由文本。**字典命名口径（决策 83）**：1 个 code + 1 个 display_name，统一为 `internet` = **互联网区**、`extranet` = **政务外网区**（不得再出现「外网区」「政务网区」等旧叫法）。**分区唯一权威**：`zone_type` 是网络分区的唯一权威（网域级属性），下游（M07 资源 / CMDB 同步）**只引用不冗余**——M07 主机不设独立分区字段，详情如需展示由所属网域**只读派生**（见 [Module_07 §5.6](Module_07_Monitoring_Object_Management.md)）。**`zone` 经所属网域派生、在 target 级 system 层注入 Prometheus label（决策 103；非 external_labels，沿用决策 68-§2.5 本地 TSDB 结论）**（分区用于分类筛选、定位与告警分组 / 去重） |
| cloud_code | string | ✅ | 云归属 | 网域的云标识（决策 78 / M07 决策 103，**必填**），取值须为云字典启用条目（M07 §5.20），由平台管理员在网域登记时选定；资源的 `cloud` label 经所属网域派生注入（target 级 system 层，不落库、不走 external_labels，见 M07 §5.13）。本字段是云归属的**唯一事实来源**，资源侧不再维护 `cloud_code`（M07 §5.4 / §5.6） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v2/platform/zone-types` | **部署级字典只读接口（MVP）**：返回启用中的网络区域类型字典项 `[{code, display_name, description}]`；数据源 = 部署配置文件预置字典；响应走统一 `{status, data, ...}` 格式。**命名口径（决策 83）**：返回项含 `code`（`internet` / `extranet`）与 `display_name`（互联网区 / 政务外网区），仅含启用项 |
...
- [ ] {P0} `zone_type` 以「网络分区（必填）」展示，表单提示「须选填分区（政务云安全分区 / 公有云 region），分区为隔离边界、AZ 不进分区」，选项带部署级字典说明（互联网区 / 政务外网区 / 专线区 / DMZ；公有云 region 含义）（MVP，决策 78）
...
- [ ] {P0} 网域创建/编辑表单**必填**维护 `zone_type`（网络区域类型，决策 78），选项来自部署级字典（政务云预置互联网区 / 政务外网区 / 专线区 / DMZ，公有云预置 region 列表），不开放自由文本（MVP）
...
- [ ] {P0} zone_type 字典命名唯一：`internet` = 互联网区、`extranet` = 政务外网区，全文档 / 接口 / 模板不再出现「外网区」「政务网区」等旧叫法；分区权威值仅存在于网域，资源侧无独立分区字段（决策 83）
...
| `zone_type` | 网络分区（必填） | 网域的分类标签（决策 78 / M07 决策 103，**必填**）；政务云 = 安全分区（互联网区 / 政务外网区 / 专线区 / DMZ），公有云 = region（上海 / 北京等），承载腾讯云地域这类真正隔离边界、AZ 不进 `zone_type`；部署级字典。**命名口径（决策 83）**：1 个 code + 1 个 display_name，统一为 `internet` = 互联网区、`extranet` = 政务外网区（不再出现「外网区」「政务网区」等旧叫法）；分区唯一权威在网域，资源侧不冗余、只读派生；`zone` 经网域派生、target 级 system 层注入 label（决策 103，非 external_labels） |
...
- **列表列数治理与详情抽屉（MVP）**：列表默认只展示扫读必需列（**网域名称 / 接入方式 / 接入进度 / 状态 / 登记归属 / 授权租户 / 网络分区 / 操作，共 8 列**），网域 ID、授权租户全量、网段（CIDR）、描述、创建与更新时间**下沉详情 Drawer**（宽 720px，`Descriptions` 两列；头部含网域名称 + 接入方式 + 状态 + 接入进度，底部提供「编辑行政信息」）；网域名称列为主标识列（左固定）且为详情入口；行内授权租户标签**最多 2 个 + 「+N」折叠**（悬浮展示全量）；操作列右固定。
...
- **用户侧术语**：页面与用户文案一律使用「中心直连域 / 采集节点域 / 采集节点 / 网络分区」；「管理域 / 边缘域 / Edge Sync Agent / 网络区域类型」仅出现在技术层（接口契约 / 代码注释 / 折叠说明），不作为用户界面文案。

## Change Log

| 版本 | 日期 | 变更类型 | 变更内容 | 影响范围 | 产品版本影响 | 状态 |
|------|------|----------|----------|----------|--------------|------|
| v2.17 | 2026-09-26 | 修改 | **实体模型收敛（决策 78 / M07 决策 103）**：NetworkDomain 行政字段表新增 `cloud_code`（必填，云归属唯一事实来源，资源侧不再维护）；`zone_type` 由可选改为**必填**且承载腾讯云地域（Region）隔离边界（AZ 不进 `zone_type`），`zone` 改为经网域派生、target 级 system 层注入 Prometheus label（非 external_labels）；§3.2 / §10 术语同步；§9.1 验收改必填。属契约级修订，原型无需同步（无新增用户可见字段）。详版见 design-decisions | 3.2 / 5.2 / 9.1 / 10 / Change Log | MVP 生效 | dev-ready |
| v2.16 | 2026-09-25 | 修改 | **`zone_type` 字典命名口径统一 + 分区唯一权威跨模块声明（决策 83）**：统一 `internet` = 互联网区、`extranet` = 政务外网区，收敛「外网区 / 政务外网区 / 政务网区」三种旧叫法；明确分区权威值仅存于网域，M07 主机不冗余、只读派生；`Host.zone_env` 降级为兼容导入列（`INT→internet`、`GOV→extranet`）；`zone` 不进 label。吸收 design-proposal `host-cloud-identity-and-zone-convergence` | 3.2 / 5.2 / 9.2 / 10 / Change Log | MVP 生效（命名收敛零外部行为影响） | dev-ready |
...
> 完整 Change Log 历史（v2.13 及以前）见 `docs/05-execution-records/module-06/design-decisions.md`「Change Log（完整历史）」。
