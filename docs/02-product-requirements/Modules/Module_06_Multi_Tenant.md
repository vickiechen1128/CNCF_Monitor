...
> **网域生命周期三动作语义边界（决策 82，2026-09-15）**：用户「不再使用某个网域」有三种强度，对应三个**正交**动作，覆盖从「暂时不用」到「彻底移除」的完整谱系：
>
> | 动作 | 语义 | 归属模块 | 对新资源 / 新纳管 | 对存量采集 | 可逆性 |
> |------|------|---------|------------------|-----------|--------|
> | **禁用**（`disabled`） | **行政冻结**——不准新入，不强制停旧 | M06（本模块） | 拒绝 | **不受影响、继续采集** | 可恢复启用 |
> | **退纳管**（v0.2+） | **停止监控**——保留网域，清空纳管与凭据 | M09 | 网域仍为「未纳管」，可重新纳管 | **停止**（Token 废止、停止下发） | 可重新纳管 |
> | **删除** | **退场回收**——从平台移除该网域 | M06（本模块） | —（网域已不存在） | **级联清退**（M09 废止 Token / 停止下发 / Agent 转离线） | 软删，不可自助恢复 |
>
> 三者**不可互相替代**：禁用管「能不能新建」（准入），退纳管管「现有采集停不停」（运行），删除管「还要不要这个网域」（存在性）；**删除包含级联清退**（等价于「退纳管 + 软删」，一次动作完成）。详见 §6.2 接口契约与 §11.2 交互契约。
...
② 引用：M07 资源导入 / 录入时选择已登记网域（M06 单一事实来源）
   │
   ▼
③ 纠错 / 回收：编辑行政信息（登记归属创建后不可变更）/ 禁用（冻结：拒绝新登记与新纳管，存量不受影响）/ 删除（退场回收：空网域直删；已纳管未引用资源 → 级联清退纳管；有资源引用 → 拒绝并引导先处理资源）
   │
   ▼
④ 纳管：交 Module_09 完成监控纳管（不在 M06 闭环）
```

- **纠错与回收路径（决策 82）**：行政信息填错 → 直接编辑（登记归属除外）；误登记（尚未引用资源 / 未纳管）→ 直接删除；网域随业务下线需要回收 → **删除**（已纳管场景删除时级联清退 M09 纳管状态：废止 Token / 停止配置下发 / 采集节点转离线；**有 M07 资源引用时拒绝删除**——资源是有主数据，须先在 [Module_07](Module_07_Monitoring_Object_Management.md) 迁移或删除）；网域暂时不用但需保留登记 → **禁用**（行政冻结：拒绝新登记与新纳管，存量采集不受影响）；v0.2+ 如需「保留网域但停止采集」→ M09 **退纳管**（决策 82-3）。
...
> **删除不在本状态机内（决策 82）**：删除是**终态动作**（软删，与 `BaseModel` 软删除体系一致），不经 `enabled → disabled` 流转——`disabled` 是**可恢复**的行政冻结态，删除是**不可自助恢复**的退场回收；故「禁用」与「删除」是两条独立路径，不可互推（deleted 记录不出现在列表与任何可选范围）。
...
- [ ] {P0} 禁用网域时二次确认弹窗展示后端返回的影响范围（该网域下 M07 资源数 / 已纳管 EdgeAgent 数）；**已纳管且存在在线 Agent 时追加固定提示**「禁用为行政冻结：新资源登记与新纳管将被阻止，但已接入的采集节点不会自动停止采集；如需停止采集并回收网域请使用『删除』」（决策 82-2）；禁用后拒绝新资源登记与新纳管，存量资源与采集不受影响（MVP）
- [ ] {P0} 网域删除为**退场回收**（决策 82-1）：**空网域可直接删除**；**已纳管但无 M07 资源引用**的网域可删除——二次确认弹窗展示级联影响清单（N 个采集节点将断连、凭据将废止），确认后 M09 纳管状态被级联清退（Token 废止 / 停止下发 / 采集节点自动转离线）；**有 M07 资源引用时拒绝删除**并列出引用资源名单与 M07 跳转（引导先迁移或删除资源）；管理域（`default`）不可删除（MVP）
...
- [ ] {P0} 网域列表**状态与操作层次**：状态列用 Badge + 文字（启用 / 禁用），禁用态补「冻结」语义说明；启用 / 禁用 / 删除等次要操作收进「更多」菜单，**不可操作原因仅在真实不可操作时给出**（系统预置域 → 「系统预置网域：不可禁用 / 删除」；有 M07 资源引用的网域 → 「存在资源引用，请先迁移或删除资源」——均为菜单内直接提示，不以点击后报错代替）；**已纳管网域的删除入口可用**（点击进入级联清退二次确认，见 §9.1「网域删除为退场回收」条，决策 82-1）；主行仅保留 主操作 + 编辑 + 更多（决策 80 / 82）
...
- [ ] {P0} 网域删除前置校验（决策 82-1）：**唯一硬拒绝条件 = 存在 M07 资源引用**（返回引用资源名单供 M07 跳转），**无引用即允许删除**（已纳管场景不再拒绝、改为**级联清退**——返回级联影响清单，确认后 M09 废止 Token / 停止配置下发 / `EdgeAgent` 标 retired）；采用软删（与 `BaseModel` 软删除体系一致）；`management`（`default`）域禁止删除（MVP）
- [ ] {P0} **网域删除级联清退（决策 82-1）**：删除已纳管网域时，M06 软删与 M09 纳管清退在**同一次请求**内完成，任一环节失败则整体回滚（不得出现「网域已删但 Token 仍有效 / 采集仍在回传」的中间态）；清退后该网域不出现在 M09 网域纳管页与采集节点状态页的**可选范围**中（既有 EdgeAgent 历史记录保留并标 `retired`，供审计）；采集节点侧无平台主动连接动作（Token 废止即失效，Agent 心跳鉴权失败自然离线）（MVP）
...
- **删除网域（退场回收，决策 82-1）**：硬拒绝条件仅为**「存在 M07 资源引用」**——该场景删除入口置灰并在「更多」菜单内直接提示「存在资源引用，请先迁移或删除资源」（附 M07 跳转）；**已纳管网域可删除**，二次确认弹窗展示**级联影响清单**（「该网域已纳管：N 个采集节点将断连、接入凭据将废止；采集节点侧无需线下操作，心跳鉴权失败后自动离线」），确认后 M06 软删 + M09 级联清退纳管状态；**空网域**删除仅需常规二次确认；管理域（`default`）不提供删除入口。
- **禁用网域弹窗补强（决策 82-2）**：禁用二次确认弹窗除「影响范围」外，当该网域已纳管且存在在线 Agent 时追加固定提示——「禁用为**行政冻结**：新资源登记与新纳管将被阻止，但**已接入的采集节点不会自动停止采集**。如需停止采集并回收网域，请使用『删除』（级联清退）；如需保留网域仅停止采集，v0.2+ 提供『退纳管』。」
...
- **状态列与操作层次（决策 80 / 82，MVP）**：状态以 **Badge + 文字**呈现（启用 = 绿色、禁用 = 灰色），禁用态悬浮补「冻结」语义说明（不再接受新登记与新纳管，存量资源与采集配置继续运行）；**启用 / 禁用 / 删除**等低频次要操作收进行内「**更多**」菜单，**不可操作原因仅在真实不可操作时给出**（「系统预置网域：不可禁用 / 删除」；「存在资源引用，请先迁移或删除资源」——供有 M07 资源的网域），不以「点击后报错」代替提示；**已纳管网域的删除入口保持可用**（进入级联清退二次确认，不再是「不可操作原因」，决策 82-1）；禁用行的操作列**不重复渲染状态文字**、也不提供接入动作（状态语义由状态列承载）；主行仅保留 **主操作 + 编辑 + 更多** 三项。
...
|------|------|----------|----------|----------|--------------|------|
| v2.15 | 2026-09-15 | 修改 | **网域生命周期闭环——回收路径修正 + 禁用/纳管联动具象化（决策 82）**（chenrt 走查提出两问：①「已纳管网域在其生命周期里没有可删除的条件和按钮，难道装了采集节点就不允许回收吗」②「M06 启用/禁用若只是行政登记，就和 M09 网域纳管割裂了」；纯文档不改数据模型、原型待同步）：①**删除约束修正（82-1）**——`DELETE /api/v2/platform/network-domains/:id` 硬拒绝条件**收敛为「存在 M07 资源引用」单一条件**（资源是有主数据，报错须列出引用资源名单 + M07 跳转），原「无已纳管 EdgeAgent」由硬拒绝改为**级联清退**——删除弹窗展示级联影响清单（N 个采集节点将断连、凭据将废止）→ 确认后 M06 软删 + M09 级联清退纳管状态（废止 Token / 停止配置下发 / `EdgeAgent` 标 retired；采集节点侧不要求线下卸载，Token 废止后心跳鉴权失败自然离线）；②**禁用弹窗补强（82-2）**——已纳管且存在在线 Agent 时，禁用二次确认在「影响范围」之外追加固定提示「禁用为行政冻结……已接入的采集节点不会自动停止采集；如需停止采集并回收网域请使用『删除』」，把「禁用不停采集」从隐性设计变为显式告知；③**退纳管动作定义（82-3，v0.2+）**——把决策 62 口径① 具象化为 M09 可实现动作（废止 Token → 停止下发 → `registration_status` 归位 `unregistered`），补齐 §6.2 早已许诺「是否停止采集由 M09 退纳管决定」但 M09 从未定义该动作的**契约悬空**；④**三动作语义边界定版**——§3.2 新增对照表：禁用 = 行政冻结（不准新入、不强制停旧）/ 退纳管 = 停止监控（保留网域、清空纳管）/ 删除 = 退场回收（级联清退、从平台移除），三者**正交不可互替**，覆盖「不再使用某网域」的三种强度。落点：§3.2 / §4 / §6.2 / §9.1（修订 2 条 + 新增 1 条）/ §9.2（改写 1 条 + 新增 1 条）/ §11.2（重写 2 条 + 新增 1 条）；Change Log 轮转 v2.12 迁完整历史 | 3.2 / 4 / 6.2 / 9 / 11.2 | MVP / v0.2 | dev-ready |
