...
| ip_cidrs | []string | ❌ | 网段（IP 段） | **v0.2（决策 52/58）**：该网域覆盖的 CIDR 列表（如 `10.20.0.0/16`），供 M07 资源导入 / CMDB 同步时按 IP 推导网域归属（四级解析链第③级）；**最长前缀优先**，同前缀跨网域冲突判「歧义」不猜测；可由管理员手工维护，也可由 M07「待分配队列」规则化动作一键生成（按未决 IP 聚合候选 CIDR 追加）；纯平台侧数据，不回写 CMDB、不要求 CMDB 加字段；MVP 不启用（资源网域仍手动选择） |
