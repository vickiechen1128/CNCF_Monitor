...
| v2.8 | 2026-09-04 | 修改 | §0「需求背景与典型场景」深化：基于 dev-feedback 与 design-decisions 真实记录，新增「用户需求的演进过程」（单团队起步→多网域登记→多租户扩展→治理完善）与「不同技术背景用户的痛点分层」（4 类用户）；典型场景从 3 个扩展为 6 个，补充「禁用不再使用的网域」「查看登录日志」「多租户数据隔离」真实场景 | 0 | 文档自身 | dev-ready |
> 完整 Change Log 历史（v2.7 及以前）见 `docs/05-execution-records/module-06/design-decisions.md`「Change Log（完整历史）」。
