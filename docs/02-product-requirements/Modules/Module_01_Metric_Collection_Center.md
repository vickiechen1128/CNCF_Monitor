...
| 规则文件挂载 | MVP 规则管理入口（承载于独立「规则编辑」页，导航位于「指标库」之后）：上传 / 粘贴完整 `rules.yml`（**整文件透传**，`content_mode=yaml_passthrough`）→ 点**独立「检查」按钮**（决策 69-2，两段式第一段）→ 在**表单下方、操作按钮上方**呈现检查结果面板（决策 69-3）：`valid` = **预检是否可提交**（YAML 语法 / `groups` 结构 / **组名全局唯一性**，决策 69-1）+ **job 引用语义校验（决策 66）**：解析规则表达式中 `job="..."` / `job=~"..."` matcher 与当前生效 `ScrapeJob` 列表比对，`up` / `absent(up)` 类不匹配 = error、其他 job 引用不匹配 = warning；error 在检查阶段即展示逐条问题清单 + 逃生门（用户勾选「已知晓：先挂规则，稍后补建 Job」显式覆盖后方可放行，降级为 warning 落库留痕）、warning 仅提示 → **「提交并进入变更确认」按钮仅在检查通过（`valid=true`）且（无 error 级 job 引用 或 已勾选逃生门）时出现**（条件渲染，非 disabled）；**规则内容变更后检查结论与 ack 一并作废、提交按钮重新隐藏，须重新检查**（规则名等非内容字段变更不触发重置）→ 落库 `MonitoringRule`（`draft_status=ready`）→ 触发 M09 变更检测，由 M09 生成 / 确认 / 下发 `rules.yml`（**人工确认档**，对齐决策 38-1）——**保证规则变更与采集 Job 一样走 M09 统一配置下发闭环，不绕过 M09**；列表展示规则名 / 规则条数 / 更新时间 / 启用状态 / 下发状态（`change_status`）；支持启停 / 删除 / 详情（YAML 只读视图）；**不做字段级 PromQL 创作编辑**（v0.3 升级 `structured` 字段化表单，见 3.2） | P0         |
...
>
> **规则挂载「检查 → 提交」两段式 + job 引用校验双层模型（交互经决策 69-2 重排；校验口径为决策 66、第 2 / 4 条经决策 67-2 修订；2026-09-11 随 v3.41 同步）**：
> - **M01 编辑期：显式「检查」动作（fail-fast，error 默认阻断 + 显式覆盖）**：抽屉改为**两段式**——「检查」按钮常驻，点按后调用 `validate-yaml` 并在**表单下方、操作按钮上方**呈现结果面板；**提交按钮按状态「出现」而非恒可点**（`canSubmit = 检查通过 且（无 error 级 job 引用 或 已勾选逃生门）`）。新建与编辑**同一套状态机**，按钮文案统一为「**提交并进入变更确认**」（原新建「提交生效」、编辑「保存变更」，与决策 43 口径及 [Module_08](Module_08_Alertmanager_Notification_Management.md) 参照实现一致）。**规则内容变更后，检查结论与 ack 一并作废、提交按钮重新隐藏、须重新检查**；规则名 / 资源类别 / 监控对象类型等**非内容字段不触发重置**。逃生门（error 级 job 引用）**提前到检查阶段**展示，不再等提交被拦。
> - **`valid` 语义 = 预检是否可提交（决策 69-1）**：`valid` 由「仅反映 YAML 语法」扩展为「**预检是否可提交**」，硬失败（**不可覆盖**）共三类——YAML 语法 / `groups` 结构非法、**组名全局唯一性冲突**（与提交侧 `validateGroupNamesAvailable` **同实现、同输入集**：仅当目标规则生效（`enabled=true AND draft_status='ready'`）时校验、**停用规则不校验**、`:id` 为真实 ID 时**排除自身**）。`job_ref.severity=error` 是**可经逃生门覆盖**的另一根轴、**不改写 `valid`**——门禁收敛为两根轴：硬失败（`valid`）+ 可覆盖（`job_ref` + ack）；响应形状 `{valid, error?, job_ref?}` 不变。
> - **job 引用分级（决策 66）**：解析 `job="..."` / `job=~"..."` matcher 与当前生效 `ScrapeJob` 列表比对；`up` / `absent(up)` 类不匹配 = error、其他不匹配 = warning。**error 默认阻断**，展示逐条问题清单（规则名 / matcher / 引用 job / 说明）+ **逃生门**「已知晓：先挂规则，稍后补建 Job」，勾选后提交按钮出现、问题降级为 warning 落库留痕；**warning 仅提示、不阻断**。
...
> - **失败不锁死源数据（决策 67-1）**：M09 草稿校验失败且归因为 `user_config` 时，平台自动清除源数据 `pending` 锁（草稿保留可重校/废弃），用户回 M01 修改后保存即生成新单重校——避免「失败单不可确认却锁死规则、只能靠废弃解锁」的死循环。
...
| PromQL 校验 + job 引用校验 | 调用 [Module\_02: 查询中心](Module_02_Query_Center.md) 或 Prometheus 接口校验表达式语法；**同时按决策 66 校验 `job` matcher 与当前生效 `ScrapeJob` 列表的匹配情况**（`up`/`absent(up)` 不匹配 = error，其他不匹配 = warning；**error 默认阻断 + 逃生门显式覆盖，warning 仅提示**，决策 66 / 67-2）；**沿用 MVP 已交付的「检查 → 提交」两段式状态机**（决策 69-2：按钮文案「提交并进入变更确认」、检查通过后提交按钮才出现、内容变更即作废检查结论），v0.3 **不另起一套**，仅把校验对象从整文件扩为逐条字段；**组名唯一性预检（决策 69-1）**同口径沿用 | P0 / v0.3 |
| 指标实时预览    | 在编辑规则时展示指标名、标签键值提示与最近样本预览                                               | P0 / v0.3 |
| 规则模板      | 按监控对象类型 / 采集实现预置常用规则模板，支持一键填充（P1）                                  | P1 / v0.3 |
| per-instance 规则模板 | 提供「按实例分组」的常用规则模板（如主机失联 `up == 0`、主机重启、磁盘满），用户选择聚合粒度（全局/按实例）后自动生成对应 PromQL（决策 65） | P1 / v0.3 |
...
| change\_status            | enum                | Module\_09（回写） | 下发状态       | `pending` / `confirmed` / `deployed` / `none`；**MVP 随规则文件挂载启用**——规则保存 / 启停 / 删除即触发 M09 变更单（同采集 Job 动线）；`change_status` 随 M09 **确认 / 下发 / 废弃**回写（`pending→confirmed→deployed` 或废弃，与采集 Job 同机制，见 5.4「下发状态感知」）；**校验失败自动解锁（决策 67-1）**：M09 草稿落到 `failed + user_config` 时清除本规则 `pending` 锁（草稿保留、可重校/可废弃），用户得以直接修改规则；`platform_fault` 不清锁；v0.3 随字段化编辑增强为逐条 |
...
> - **草稿与提交生效（v0.3）**：规则编辑 UI 新建规则时提供「保存草稿」与「提交并进入变更确认」（决策 69-2 统一文案，替代原「提交生效」）两种保存模式；草稿态允许 PromQL 半成品暂存，不做完整校验；提交生效时调用 Module\_02 进行 PromQL 语法/指标存在性校验，通过后 `draft_status` 转 `ready`；**草稿仅存在于新建阶段**，已生效规则的修改直接走正常变更管线；
...
> **规则粒度与前端展示（决策 65）**：规则按求值粒度分为**聚合规则**与 **per-instance 规则**两类，对 `MonitoringRule` 数据模型与 M09 生成逻辑完全透明（不新增字段、不区分处理）：
>
...
> **规则挂载「检查」与提交门禁（决策 69，MVP 随规则文件挂载启用）**：
>
...
> - **`valid` = 预检是否可提交**（决策 69-1）：硬失败三类——YAML 语法 / `groups` 结构、**组名全局唯一性冲突**；`job_ref.severity=error` 为**可覆盖轴**、不改写 `valid`；
...
> - **规则草稿（`draft`→`ready` 批量提交）推迟至 v0.3**：`MonitoringRule` 的「保存草稿 / 提交并进入变更确认 + 批量提交」能力随 **v0.3** 字段化编辑交付（提交按钮文案统一为「提交并进入变更确认」，决策 69-2），MVP 阶段规则仅 `ready`（文件挂载保存即 `ready`）；
> - **规则 pending 期锁定（同采集 Job）**：`change_status=pending` 期间**禁止编辑 / 启停 / 删除**该规则；解锁路径同采集 Job（M09 变更单**确认**、**废弃**），**并新增「校验失败自动解锁」（决策 67-1）：M09 草稿 `validation_status=failed` 且 `validation_cause=user_config` 时，平台自动清除本规则 `pending` 锁，草稿保留可重校/可废弃——避免失败单不可确认却锁死源数据、用户只能靠废弃解锁的死循环**（`platform_fault` 不清锁）。
...
| POST | `/api/v1/monitoring-rules` | 5.5 字段（`content_mode=yaml_passthrough` 时**必填** `rule_content`；可选 `ack_job_ref_errors` 默认 `false`） | 创建后的完整对象（保存即 `draft_status=ready`，进入 M09 变更检测） | `bad_request`：`rule_content` 为空 / YAML 语法非法（至少校验 `groups` 存在且为数组）/ **group 名全局冲突** / **存在 error 级 job 引用且未置 `ack_job_ref_errors=true`**（决策 67-2） |
...
| POST | `/api/v1/monitoring-rules/{id}/validate-yaml` | `{ rule_content }`；`:id` 为真实规则 ID 或 `0`（新建，前端传 `0`，不排除自身） | `{ valid, error?, job_ref? }`——`valid` = **预检是否可提交**（YAML 语法 / `groups` 结构 / **组名全局唯一性**，决策 69-1）；`job_ref[]` 元素 `{group, rule_name, expr, matcher, referenced_job, type, severity, message}`，`severity∈{error,warning}`（决策 66） | —（校验结论经响应体表达，不抛业务错误） |
...
>
> **规则侧另有「校验失败自动解锁」路径（决策 67-1）**：`MonitoringRule` 在 M09 草稿 `failed + user_config` 时自动清锁（草稿保留），避免不可确认的失败单锁死源数据（见 5.5「规则 pending 期锁定」）。**采集 Job 侧是否同等覆盖为未决项**——Job 的废弃涉及「新建回退 draft / 删除停用自动恢复」的分类回写（决策 43），与规则的纯清锁语义不同，需单独评估后再定（见 `docs/05-execution-records/module-09/rule-jobref-validation-lifecycle-fix.md` §7）。
...
- [ ] {P0} 规则挂载抽屉为**两段式**（决策 69-2）：footer 常驻「取消」+「检查」，**未检查 / 检查未通过时没有提交按钮**；提交按钮文案新建与编辑统一为「**提交并进入变更确认**」（**条件渲染而非 disabled**）
- [ ] {P0} 点「检查」后调用 `validate-yaml`，结果面板呈现在**表单下方、操作按钮上方**：YAML 语法 / `groups` 结构非法 → 红面板「规则检查未通过」+ 提交按钮不出现；**与既有生效规则组名冲突 → 红面板点明占用方 + 提交按钮不出现**（决策 69-1 核心验收点）；编辑该规则自身、组名不变 → 检查**通过**（自排除生效）；编辑**停用**规则、组名与生效规则重复 → 检查**通过**（镜像提交侧条件，避免无出路）
- [ ] {P0} job 引用分级阻断在**检查阶段**即呈现（决策 66 / 67-2）：引用不存在 job 的**存活类**（`up` / `absent(up)`）规则 → 红面板 + 逃生门「已知晓：先挂规则，稍后补建 Job」，提交按钮不出现；**勾选逃生门 → 提交按钮出现**，提交成功后问题降级为 warning 落库留痕（请求携带 `ack_job_ref_errors=true`）；仅含 **warning** 级 job 引用的内容 → 黄面板 + 提交按钮直接出现
...
- [ ] {P0} {v0.3} 规则编辑 UI 同样提供「保存草稿」与「提交并进入变更确认」（仅新建阶段；提交按钮文案与 MVP 文件挂载统一，决策 69-2）；草稿态允许 PromQL 半成品，提交时调用 Module_02 做 PromQL 校验
...
- [ ] {P0} `POST /api/v1/monitoring-rules/{id}/validate-yaml` 响应语义 = **预检是否可提交**（决策 69-1）：`valid=false` 覆盖三类硬失败——YAML 语法 / `groups` 结构、**组名全局唯一性冲突**（与提交侧 `validateGroupNamesAvailable` **同实现、同输入集**：仅当目标规则生效时校验、停用规则不校验、`:id` 命中自身时排除自身）；`job_ref[].severity=error` **不改写 `valid`**（决策 67-2）；响应形状 `{valid, error?, job_ref?}` 向后兼容
- [ ] {P0} POST / PUT `monitoring-rules` 支持可选 `ack_job_ref_errors`（默认 `false`）：存在 error 级 job 引用且未置 `true` → `bad_request`（`errorType=job_ref_unresolved`，消息汇总问题条数）；`warning` 级不受影响（决策 67-2，**本轮不改后端判定逻辑**，仅前端提前调用同一接口并据此排布按钮）
...
- **规则挂载「检查 → 提交」两段式（决策 69，交互重排；校验口径为决策 66、经决策 67-2 修订）**：
...
- **失败单不锁死源数据（决策 67-1）**：M09 变更单校验失败且归因 `user_config` 时，本模块源数据的 `pending` 锁**自动清除**（草稿保留可重校/可废弃），用户可直接回本模块修改规则；列表「变更进度」列刷新后不再显示锁定态。归因 `platform_fault` 时保持锁定（非用户可修）。
