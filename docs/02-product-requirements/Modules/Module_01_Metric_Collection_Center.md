...
> **原型版本**: v3.28（已对齐；v3.29 ~ v3.35 为契约收紧、来源开放与 §0 深化，原型行为不变）
> **更新日期**: 2026-09-04
...
| v3.34 | 2026-09-04 | 修改 | §0「需求背景与典型场景」再深化：①补充「规则期」阶段——指标库缺失导致规则编写门槛高、YAML 文件挂载对非专业用户不友好；②新增「不同技术背景用户的痛点分层」——按 Prometheus 专家/有经验运维/普通运维/监控新手四类用户分别列出典型问题与应对；③典型场景从 6 个扩展为 8 个，补充「编写告警规则」「选择监控指标」两个真实场景；④用户故事补 M01-OPS-05/06 | 0 | 文档自身 | 设计中 |
