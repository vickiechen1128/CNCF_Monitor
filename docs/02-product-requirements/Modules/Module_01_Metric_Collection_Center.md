...
运维团队需要为大量主机、中间件、应用服务配置指标采集与告警规则，但手工编写 Prometheus 配置文件复杂易错，且缺乏统一的策略管理与状态回显。本模块把「采什么、怎么采、怎么判」从产品化视角封装，让运维工程师无需直接操作 `prometheus.yml` 即可完成采集策略配置。

### 用户需求的演进过程

从 MVP 开发期的真实反馈看，用户痛点不是一次性出现的，而是随着使用深入逐步暴露：

**阶段 1：接入期——「如何快速把资源纳入监控？」**
- 用户最初面对的核心问题是：有 50 台新上线的 MySQL 实例，如何快速配置采集？
- 痛点：手工编写 `prometheus.yml` 中的 `scrape_configs` 和 `targets` 极易出错，且无法批量操作
- 对应能力：Excel 批量导入（M07）→ 采集器管理（登记/默认配置）→ ScrapeJob 创建 → 实例勾选

**阶段 2：配置期——「参数为什么要填这么多遍？」**
- 用户在配置过程中发现：端口/路径要在「采集器登记 → 默认采集配置 → 采集 Job」填 3 遍，间隔/超时要填 2 遍
- 痛点：重复劳动、无法留空继承、参数生效链路不透明
- 对应能力：层叠默认链（Job → 默认配置 → 采集器模板 → 全局兜底）+ 稀疏覆盖（留空=继承）

**阶段 3：联调期——「配置保存了，为什么没生效？」**
- 用户在测试中发现：Job 保存后 Prometheus 没抓到数据、target 端口不对、状态一直「待生效」
- 痛点：配置生命周期不透明（草稿/待生效/已生效/已停用）、变更需要人工确认但入口隐蔽、实例采集状态不可见
- 对应能力：Job 状态四态回显（草稿/待生效/已生效/已停用）+ 变更进度列（M09 管线状态）+ 实例采集状态回显

**阶段 4：规则期——「我想写告警，但不知道有哪些指标可用」**
- 用户在配置告警规则时发现：不知道有哪些指标可以选、PromQL 语法不会写、规则文件格式不理解
- 痛点：指标库缺失导致规则编写门槛高、YAML 文件挂载对非专业用户不友好、无法实时预览规则效果
- 对应能力：指标库管理（M01-OPS-05）+ 规则文件挂载（MVP 简化版）+ v0.3 字段化编辑 + PromQL 校验与指标预览

**阶段 5：优化期——「批量操作和细节体验能否更好？」**
- 用户在规模化使用后提出：需要批量提交生效（F-16）、需要更清晰的列展示（F-37）、需要放开采集器来源（F-32）
- 痛点：单条操作效率低、列表信息密度不合理、自定义采集器受限
- 对应能力：批量提交生效（draft→ready 单向）+ 列序优化（生效状态→变更进度）+ 三来源采集器登记

### 不同技术背景用户的痛点分层

同一个功能，不同技术背景的用户会提出完全不同的问题：

| 用户类型 | 典型问题 | 本模块的应对 |
|----------|----------|-------------|
| **Prometheus 专家** | 「这个配置最终生成的 `prometheus.yml` 是什么样的？我能直接改吗？」 | 提供 M09 配置预览与 diff；规则文件挂载支持整文件透传 |
| **有经验的运维** | 「采集参数为什么要填这么多遍？能不能只改一处？」 | 层叠默认链 + 稀疏覆盖（留空=继承） |
| **普通运维** | 「我不知道该选哪个采集器，也不知道端口填什么」 | 默认采集配置预填 + 空态引导 + 层叠默认值兜底 |
| **监控新手** | 「什么是 PromQL？什么是 labels？我该从哪里开始？」 | 指标库提供可选指标列表；规则编辑提供字段化表单（v0.3）；原型双层设计（用户层讲人话，技术层折叠） |

### 典型场景（基于真实用户反馈）

| 场景 | 角色 | 触发条件 | 用户目标 | 成功标准 | 来源 |
|------|------|----------|----------|----------|------|
| 批量接入新资源采集 | 运维工程师 | 新上线 50 台 MySQL 实例 | 快速为这批实例配置指标采集 | 5 分钟内完成 Job 创建并看到实例上线状态 | 原始需求 |
| 采集参数层叠继承 | 运维工程师 | 某类资源采集频率需要调低 | 只改默认配置，存量 Job 自动继承新值 | 清空 Job 参数字段保存即恢复继承，无需逐条修改 | F-28 |
| 端口错配导致抓取失败 | 运维工程师 | 添加 Job 后 Prometheus 报 `connection refused` | target 地址自动带正确端口 | Host/Database/Middleware 地址自动拼接 exporter 端口（9100/9104 等） | F-20 |
| 配置变更需人工确认 | 运维工程师 | 修改了采集 Job 或规则 | 确认变更内容后下发 | 变更单在 M09 配置中心人工确认后生效，避免误操作 | |
| 批量提交多个草稿 Job | 运维工程师 | 初次配置 20+ 个 Job | 先批量编辑为草稿，再一次提交生效 | 支持「保存草稿」+ 列表批量提交生效，减少逐条确认 | F-16 |
| 登记自定义采集器 | 运维工程师 | 需要使用社区版 Redis exporter 替代内置 | 登记官方/第三方/自研采集器 | 三种来源均可登记，与预置同名时提示冲突 | F-32 |
| 编写告警规则 | 运维工程师 | 业务团队要求新增 CPU 告警 | 基于指标库选择指标并编写 PromQL | 规则文件经确认后生效，触发告警可查 | M01-OPS-06 |
| 选择监控指标 | 运维工程师 | 不确定某类资源有哪些可采集指标 | 浏览指标库并选择合适的指标 | 指标库按监控对象类型分组，可查看 HELP/UNIT | M01-OPS-05 |
...
1. **监控策略配置（MVP）**：建立监控对象类型与默认采集器（采集实现）的绑定关系，基于该绑定创建并维护 `ScrapeJob`，在隔离网域场景下手动勾选需要监控的实例；**选中即生成 target**，实例采集状态（在线数 / 待采集 / up / down）在 Job 详情与编辑抽屉实时回显，Exporter 安装确认降级为**可选登记**、不再作为生成 target 的前置。
...
- 本模块**不建设独立的**运行时采集状态页面（全局目标列表、拨测结果、probe 结果、抓取诊断、Job 健康度、覆盖率视图），这些职责已分别移交至 [Module\_02: 查询中心](Module_02_Query_Center.md) 与 [Module\_08: 告警收敛与通知管理](Module_08_Alertmanager_Notification_Management.md)；但 **Job 配置上下文内的只读采集状态回显**（实例状态列 + 在线数汇总，数据消费 Module_02 `/api/v1/targets` 代理）回流本模块（见 5.10）。
...
- M01-OPS-04：可选登记目标实例的 Exporter 安装/注册情况（不再作为生成 target 的前置闸门）。
- M01-OPS-08：在采集 Job 详情/编辑抽屉直接看到已选实例的采集状态（在线数 / 待采集 / up / down）与「已下发未采到」提醒。
...
- M01-ARCH-01：快速发现未被任何 `ScrapeJob` 选中的实例——主落点为 [Module_07](Module_07_Monitoring_Object_Management.md) 资源列表三态「采集状态」badge 与筛选；本模块实例选择器「未纳入任何 Job」筛选为辅助落点（**目标语义，MVP 不保证，随本模块开发节奏落地**）。
...
| 功能 | 说明 | 优先级 |
| --- | --- | --- |
| 默认采集配置（预设维护） | 为每种监控对象类型预设「默认采集器 + 采集参数 + 安装指南 / 下载地址」，创建采集 Job 时自动带出。入口承载于「采集 Job」页「采集器管理」Tab（不独立导航）：①一个监控对象类型可有多个可选采集实现、`is_default` 标记默认，按 OS 平台区分；②创建 Job 自动套用默认值、可覆盖；③实例级安装登记在「采集 Job」选中实例时进行（5.6，可选），本 Tab 不重复登记。 | P0 |
| 采集实现池列表 | 采集器选择器 / 列表展示采集实现池全貌：数据源 = 映射行 + 已登记但未被任何映射引用的采集器（未引用行标「未被引用」）。「登记即入池」对用户可见：无合适采集器时经「登记采集器」入口（低频补救），页面引导动线顺序见下方说明。 | P0 |
| 标签模板创建引导 | 新增默认采集配置时检测监控对象类型是否已有标签模板；无模板时弹出轻量提示，支持「立即创建」（预填推荐映射）或「稍后再说」（列表显示待配置 badge）。 | P0 |
| 标签模板展示列 | 默认采集配置列表「标签模板」列展示模板名称 + 默认/自定义标记 + 类别·模板ID；支持查看（只读预览抽屉）、更换（同资源类别其他模板）、补配（重新触发创建流程）。 | P0 |
| ScrapeJob 管理 | 创建与维护采集任务：①创建/编辑、命名、启用/禁用、关联监控对象类型与默认采集器、实例选择模式、标签模板引用；②MVP 绑定且仅绑定单一已纳管网域（未纳管域不可选），v0.2 起放宽为网域集合、M09 按域自动拆分扇出；③创建自动套用该类型默认采集配置；④选中实例即生成 target，实例采集状态在 Job 详情/编辑抽屉回显（5.10）；Exporter 安装登记可选（5.6）。 | P0 |
| ScrapeJob 草稿与批量提交生效 | 新建 Job 支持「保存草稿 / 提交生效」两种模式（默认提交生效）：草稿仅做基础校验、不进入 M09 生成；提交生效做完整校验后转 `ready` 进入 M09 变更检测。草稿仅在新建阶段存在，提交生效后不回退草稿态；可多选「批量提交生效」（draft→ready 单向）；状态列四态（草稿/待下发/已生效/已停用）。UI 交互 v0.3（字段本 MVP 已落库默认 ready）。 | P0 / v0.3 |
| 克隆 Job | 将既有 Job 一次性克隆为新 Job（同/跨网域）：复制 job_name（自动追加后缀）、监控对象类型、采集实现、采集参数、标签模板；network_domain_id 可改选；selected_instance_ids 不携带、克隆后按目标网域重新勾选；Exporter 安装登记不携带、对新实例重新登记。产物是独立 Job、与源无持续绑定；复用经一次性克隆完成，不引入「Job 模板」持久实体。待评估（网域集合扇出已覆盖跨网域复用主场景）。 | P0 / v0.3+ 待评估 |
| 实例选择 | 为 Job 挑选采集实例：MVP 按类型+网域自动收敛候选 + 手动勾选（一键全选/反选、关键字筛选）；offline 实例显示但置灰不可选，已选转 offline 后 M09 生成跳过（maintenance 口径与 M07 8.1 对齐、MVP 不保证）；v0.2 起支持 filter 模式按资源属性条件表达式筛选并实时求值、新匹配资源自动纳入；筛选字段为 Resource 属性字段，label 仅作 UI 别名、不写标签。 | P0 / v0.2 |
| Exporter 安装/注册登记（可选） | 在 Resource × Job 维度登记 exporter 安装/注册情况（confirmed_by / notes / actual_port），纯留痕与人工背书、不作为生成 target 的前置；未登记实例照常生成 target。 | P1 |
| 实例采集状态回显 | Job 详情/编辑抽屉实例列表新增「采集状态」列 + 外层汇总（在线数 up / 实例总数 / 待采集数）：存量实例显示真实 up/down；新保存未下发显示「待采集」；已下发但 down 提醒检查采集器安装与网络连通。数据源 = Module_02 `/api/v1/targets` 代理，只读消费（设计见 5.10）。 | P0 |
| ScrapeJob blackbox 类型支持 | Blackbox 拨测作为 ScrapeJob 的一种类型，经 job_type / blackbox_module / blackbox_targets 配置。 | P0 |
| 技术指标库管理 | 平台可识别指标的元数据（指标名 / 类型 counter/gauge/histogram/summary / HELP/UNIT），分组锚点为监控对象类型（monitor_type，多对多，关联带来源采集器标注，见 5.3）；规则编辑按监控对象类型提示指标与 PromQL 校验；与业务指标库（业务语义契约）并列互补、UI 互链、统一归组于「指标库」。 | P1 |
| 业务指标库 | 业务指标登记（BusinessMetric）：业务负责人定义语义 / 阈值 / 所属业务域 / 负责人（owner 必填）；登记不绑定角色（业务负责人自录或运维接工单代办，owner 仍指向业务方）；运维落地采集后标记「已上线」；登记表补「采集落地」列（online 显示关联 Job）；业务视图按 business_domain 聚合成员与采集落地状态，语义层不改变采集配置逻辑。MVP 轻量版；v0.3+ 独立业务负责人入口 + 业务健康度看板。 | P0 / v0.3+ |
| 规则文件挂载 | 在「规则编辑」页上传 / 粘贴完整 rules.yml（整文件透传），点「检查」呈现结果面板（valid=预检是否可提交 + job 引用校验；error 可经逃生门覆盖、warning 仅提示），「提交并进入变更确认」仅在检查通过且满足门禁时出现；提交后落库 MonitoringRule（draft_status=ready）→ M09 变更检测/确认/下发（人工确认档）；列表展示规则名/条数/更新时间/启用/下发状态，支持启停/删除/详情（YAML 只读）。MVP 不做字段级 PromQL 创作（v0.3 见 3.2）。机制推导见 3.1.1。 | P0 |
| Exporter 指标库 | 静态内置库覆盖常见采集实现（node-exporter、mysqld-exporter、redis-exporter 等）的指标，按监控对象类型组织 + 用户扩展入口（自定义指标集挂监控对象类型）；完整管理页面 P1/P2。 | P1 / P2 |
| 高级 Relabel 管理 | 标签丢弃/保留/重写、正则替换、hashmod（未来）。 | P2 |

**动线顺序与按钮层级**：「采集器管理」Tab 的**主流程**是「配置默认采集配置（监控对象类型 ↔ 采集器）」；「登记采集器」是**低频前置补救动作**，仅在没有合适采集器时执行。页面顶部**主按钮为「新增默认采集配置」**，「登记采集器」入口降级为次级按钮，并同时内置于采集器选择器的空态引导中；页面引导以编号动线说明呈现：「① 登记采集器（无合适官方/第三方/自研采集器时需要）→ ② 配置默认采集（监控对象类型绑定采集器 + 参数）→ ③ 到「采集 Job」创建任务并选实例（可选登记安装）→ ④ 保存后在 Job 详情查看采集状态回显」，一眼看清顺序与可选性。
...
### 3.1.1 规则挂载「检查 → 提交」两段式 + job 引用校验双层模型（2026-09-11 随 v3.41 落版）
- **M01 编辑期：显式「检查」动作（fail-fast，error 默认阻断 + 显式覆盖）**：抽屉改为**两段式**——「检查」按钮常驻，点按后调用 `validate-yaml` 并在**表单下方、操作按钮上方**呈现结果面板；**提交按钮按状态「出现」而非恒可点**（`canSubmit = 检查通过 且（无 error 级 job 引用 或 已勾选逃生门）`）。新建与编辑**同一套状态机**，按钮文案统一为「**提交并进入变更确认**」（原新建「提交生效」、编辑「保存变更」，与口径及 [Module_08](Module_08_Alertmanager_Notification_Management.md) 参照实现一致）。**规则内容变更后，检查结论与 ack 一并作废、提交按钮重新隐藏、须重新检查**；规则名 / 资源类别 / 监控对象类型等**非内容字段不触发重置**。逃生门（error 级 job 引用）**提前到检查阶段**展示，不再等提交被拦。
- **`valid` 语义 = 预检是否可提交**：`valid` 由「仅反映 YAML 语法」扩展为「**预检是否可提交**」，硬失败（**不可覆盖**）共三类——YAML 语法 / `groups` 结构非法、**组名全局唯一性冲突**（与提交侧 `validateGroupNamesAvailable` **同实现、同输入集**：仅当目标规则生效（`enabled=true AND draft_status='ready'`）时校验、**停用规则不校验**、`:id` 为真实 ID 时**排除自身**）。`job_ref.severity=error` 是**可经逃生门覆盖**的另一根轴、**不改写 `valid`**——门禁收敛为两根轴：硬失败（`valid`）+ 可覆盖（`job_ref` + ack）；响应形状 `{valid, error?, job_ref?}` 不变。
- **job 引用分级**：解析 `job="..."` / `job=~"..."` matcher 与当前生效 `ScrapeJob` 列表比对；`up` / `absent(up)` 类不匹配 = error、其他不匹配 = warning。**error 默认阻断**，展示逐条问题清单（规则名 / matcher / 引用 job / 说明）+ **逃生门**「已知晓：先挂规则，稍后补建 Job」，勾选后提交按钮出现、问题降级为 warning 落库留痕；**warning 仅提示、不阻断**。
- **检查通过 ≠ 提交必过（用户可见的免责边界）**：「检查通过」的绿提示须写明**服务端校验覆盖「YAML 语法 + 组名唯一性 + job 引用」，提交时服务端仍会复核**；唯一残余差异是 TOCTOU（检查与提交之间他人改动规则导致组名被占用），该竞态用户已确认可忽略。后端不可用回落本地兜底（无 `job_ref`）时同样展示该边界——提交若被后端以 `errorType=job_ref_unresolved` 拦下，面板给出逃生门并隐藏提交按钮，勾选后按钮重新出现。
- **M09 发布期校验（部署期护栏，强制阻断）**：规则进入 M09 变更单后，`rules.yml` 生成完毕会按**同一套判定逻辑 + 同一输入集**复验；`up` / `absent(up)` 类不匹配 = error，**阻断确认发布**；其他不匹配 = warning，允许确认但高亮提示。
- **失败不锁死源数据**：M09 草稿校验失败且归因为 `user_config` 时，平台自动清除源数据 `pending` 锁（草稿保留可重校/废弃），用户回 M01 修改后保存即生成新单重校——避免「失败单不可确认却锁死规则、只能靠废弃解锁」的死循环。
- **推荐动线为先 Job 后规则**：规则编辑页/校验提示文案引导用户「建议先创建对应采集 Job」；若用户确需先挂规则，可经逃生门显式覆盖，待 Job 创建后发布即可通过。

### 3.2 规则编辑 UI（v0.3）

| 功能 | 说明 | 优先级 / 版本 |
| --------- | ----------------------------------------------------------------------- | --- |
| 类 YAML 表单 | 提供 expr / for / labels / annotations 字段，字段化编辑告警规则与记录规则（v0.3）。MVP 阶段规则经 3.1「规则文件挂载」整文件透传落库。 | P0 / v0.3 |
| PromQL 校验 + job 引用校验 | 调用 [Module\_02: 查询中心](Module_02_Query_Center.md) 或 Prometheus 接口校验表达式语法；**同时校验 `job` matcher 与当前生效 `ScrapeJob` 列表的匹配情况**（`up`/`absent(up)` 不匹配 = error，其他不匹配 = warning；**error 默认阻断 + 逃生门显式覆盖，warning 仅提示**）；**沿用 MVP 已交付的「检查 → 提交」两段式状态机**（按钮文案「提交并进入变更确认」、检查通过后提交按钮才出现、内容变更即作废检查结论），v0.3 **不另起一套**，仅把校验对象从整文件扩为逐条字段；**组名唯一性预检**同口径沿用 | P0 / v0.3 |
| 指标实时预览 | 在编辑规则时展示指标名、标签键值提示与最近样本预览 | P0 / v0.3 |
| 规则模板 | 按监控对象类型 / 采集实现预置常用规则模板，支持一键填充（P1） | P1 / v0.3 |
| per-instance 规则模板 | 提供「按实例分组」的常用规则模板（如主机失联 `up == 0`、主机重启、磁盘满），用户选择聚合粒度（全局/按实例）后自动生成对应 PromQL | P1 / v0.3 |
| 图形化规则构建器 | 拖拽式构建复杂规则（未来） | P2 |

MVP 阶段规则经 3.1「规则文件挂载」上传 / 粘贴完整 rules.yml 整文件透传落库，变更进入 M09 变更检测 → 确认 → 下发闭环，不绕过 M09。规则创作体验（字段表单 / 校验 / 预览 / 模板）留待 v0.3 字段化编辑：规则编辑 UI、PromQL 校验、指标实时预览及规则模板随 [Module_08](Module_08_Alertmanager_Notification_Management.md)（v0.3）一同交付；PromQL 校验与指标预览调用 [Module_02](Module_02_Query_Center.md) v0.3 提供的 `validate` / `preview` 接口。
...
**规则粒度说明**：规则按求值粒度分为两类——
- **聚合规则**（如 `HostTargetsMissing: absent(up{job=~"node|node-exporter|linux"})`）：对整批目标做全局判断，告警序列无 `instance` 标签，前端告警状态/历史告警页「实例」列显示「全局/聚合」；
- **per-instance 规则**（如 `HostDown: up == 0`）：为每个实例单独求值，告警序列携带 `instance` 标签，前端可直接展示实例 IP/名称。

两类规则对用户入口一致（MVP 均为文件挂载，v0.3 均为字段化编辑），差异仅在表达式写法。M09 生成 `rules.yml` 时对两类规则一视同仁，不感知粒度差异；MVP 种子规则已同时包含两类（见 `config-output/rules.yml`）。
...
2. **创建采集 Job**：基于映射创建 ScrapeJob（快照继承默认参数），选择实例——MVP 手动勾选（同类型同网域候选收敛）→ v0.2 条件筛选（filter）→ v0.3+ 服务发现（service_discovery，微服务动态实例）；
...
4. **运行时采集**：Prometheus/Edge Agent 抓取，全局目标状态与采集诊断视图由 Module\_02 提供；**Job 上下文采集状态回显**（实例 up / down / 待采集 + 在线数汇总）经 Module\_02 `/api/v1/targets` 代理回流到本模块 Job 详情/编辑抽屉，闭合「配置 → 下发 → 验证」动线。
...
**入口与命名**：本预设层为**独立页面「采集器管理」（`/collectors`）**，与「采集 Job」页（`/scrape-jobs`）**为「采集策略」一级模块下的 Sider 二级导航项**（与「规则编辑」「指标库」并列；，2026-09-05 以生产导航为准）——创建 Job 时自动套用该监控对象类型的默认值，用户在独立页面内查看/维护预设（采集器 / 参数 / 安装指南 / 标签模板）。
...
**来源动线统一（登记即入池）**：官方 / 第三方 / 自研采集器均可先在本 Tab「登记采集器」（`source` 任选，表单引导见 5.2），登记完成即进入采集实现池，**之后与平台预置采集器同一条动线**（同样被映射引用、同样预填 Job、同样走 5.6 可选安装登记）；预置参数为官方默认值参考，不随动线砍掉。
...
生效参数按以下层叠链解析，**任一层留空 = 继承下一层**，保存时解析为生效快照（快照语义不变）：
...
监控对象类型**新增**（首次引入、无模板）时的正确动作是**创建模板**；监控对象类型**已有**（有模板）时的正确动作是**直接选择模板**（常态，无需重复创建）。此分叉必须在「标签模板」选择交互上显性化：
...
- 本模块 `monitor_type` 使用**细粒度监控对象类型**（`host_linux` / `host_windows` / mysql / redis / kafka / nginx / application\_http / snmp；增 `k8s_apiserver` / `k8s_kube_state_metrics` / `k8s_etcd`），直接映射 Exporter 绑定与指标库；
...
| generic\_target + `exporter_type=kubernetes-apiserver`（K8s 集群登记入口） | `k8s_apiserver` |
| generic\_target + `exporter_type=kube-state-metrics` | `k8s_kube_state_metrics` |
| generic\_target + `exporter_type=etcd` | `k8s_etcd` |
...
- **generic\_target 端点子类型判别（2026-09-15）**：generic\_target（M07 UI 名「其他监控目标」）的 `monitor_type` 不再单值映射 `snmp`，而以 M07 `GenericTarget.exporter_type`（**端点子类型判别值**：由 M07 登记表单的端点类型预设写入、对 M07 用户隐藏）为判别键。MVP 仅 `snmp_exporter → snmp` 可建 Job 走通完整映射（内置 ExporterTemplate / CITypeExporterMapping 种子已具备）；三个 k8s 监控对象类型随 M07 K8s 集群登记双入口落地（内置 ExporterTemplate / CITypeExporterMapping 种子由本模块侧配置，模型不变、仅枚举值扩展 + 内置映射种子新增）；其他 / 空判别值无默认映射。blackbox 拨测不经本推导表——拨测 URL 由 `job_type=blackbox` Job 的 `blackbox_targets` 直接承载。跨模块主记录见 `docs/05-execution-records/module-07/design-decisions.md`。
...
### 5.1.9 application_http 语义澄清

`application_http` 对应"业务指标端点采集"（应用服务资源自带 `/metrics`，见 Module\_07 5.8 `endpoint`），其"采集实现"实为**HTTP 抓取**——业务应用无独立 exporter 进程，由 Prometheus client / 框架埋点暴露指标：
...
CMDB 新监控对象类型经 Module\_04「待分类队列」完成映射同步后，本模块需要形成**新类型接入引导闭环**（复用标签模板创建引导模式）：
...
| source | enum | 来源 | `official`（开源官方）/ `third_party`（第三方）/ `internal`（自研）；内置采集器默认 `official` / `third_party`，**用户登记可任选三种来源之一** |
...
**登记表单按来源引导**：「采集器管理」Tab 列表支持按 **监控对象类型 + 来源** 筛选；登记 / 编辑表单按 `source` 做字段引导——三种来源（`official` / `third_party` / `internal`）均可由用户登记，`default_port` / `metrics_path` / `scheme` 均**必填**并提示「请按实际部署填写」；`download_url` 提示填对应下载地址或内部制品库链接；`name` 建议 `xxx-exporter` 命名规范。

**登记来源口径（统一）**：用户登记**开放 `official` / `third_party` / `internal` 三种来源**——同一监控对象类型下，用户可登记官方/第三方/自研采集实现作为备选（如 MySQL 场景不用内置 mysqld-exporter，改用社区/厂商版），不再被迫标注为 `internal`。用户登记行的 `is_builtin` 恒为 `false`，与平台预置行区分；**名称唯一索引天然防止与预置 seed 重合**（同名返回 409 Conflict）。

**预置采集器定位**：平台内置采集器（`is_builtin=true`）作为**初始数据**由 seed 写入，只读维护；用户可登记同名或不同名的官方/第三方采集器作为替代实现，与内置采集器共存于采集实现池。
...
| network\_domain\_id | string | Module\_09 | 网域 | 归属网域；**必填**，MVP 所有 ScrapeJob 必须绑定且仅绑定单一**已纳管网域**（未在 M09 完成监控纳管的网域不可选，保存时校验）；**v0.2 起扩展为网域集合**——一个逻辑 Job 可勾选多个已纳管网域，M09 生成器按域拆分扇出（每域独立 scrape_configs / targets / 变更单），MVP 存量单值自动迁移为单元素集合；「采集 Job」列表页提供网域查询条件（选项 = 已纳管网域） |
| instance\_selection\_mode | enum | 策略配置 | 实例选择方式 | manual（MVP）/ **filter（v0.2：条件表达式每生成周期实时求值，M07 新增资源匹配即自动纳入，无需编辑 Job）** / service_discovery（v0.3+ 预留） |
...
| auth\_type | enum | 用户 | 认证类型 | **采集认证/TLS 最小集（MVP 必实现）**：`none` / `basic` / `bearer`，默认 `none`（无认证）；全部可选、默认不启用，不影响既有裸 http 场景 |
...
| draft\_status | enum | 用户/策略配置 | 草稿状态 | `draft`（编辑中；**字段 MVP 已落库、默认 `ready`，「保存草稿」交互 v0.3 开放**）/ `ready`（待下发）；默认 `ready`（**默认「提交生效」**）；**draft 单向流转 draft→ready，ready 不再回退**（仅新建阶段可为 `draft`，提交生效转 `ready`）；**例外：新建未生效 Job 随 M09 变更单废弃时，由系统自动回退为 `draft`**；已生效对象的后续修改直接更新主字段、走正常变更管线；仅 `ready` 状态的对象进入 M09 配置生成候选集 |
| change\_status | enum | Module\_09（回写） | 下发状态 | `pending`（存在 M09 待确认变更单）/ `confirmed`（变更单已确认，待下发）/ `deployed`（已下发生效）/ `none`（无在途变更）；**MVP 阶段 M09 回写 `pending/confirmed/none/deployed`**（`deployed` 提前到 MVP），M09 依据 ConfigDeployment success 记录回写，消除「已生效 vs 无变更」歧义 |
...
- **`offline` 实例显示但置灰不可选（MVP 必实现）**：候选集中 `Resource.status=offline` 的实例照常展示（保证下线台账可见），但**置灰禁止勾选**（`maintenance` 排除口径与 [Module_07 8.1](Module_07_Monitoring_Object_Management.md) 一并对齐、MVP 不保证）；已选实例事后转 `offline` 的，M09 配置生成跳过（`offline` 后下一配置生成周期即从 `targets/*.json` 移除）；
...
**filter 模式字段语义（v0.2，由 v0.3+ 提前）**：`instance_filter` 的筛选字段 = **Resource 属性字段**（`env` / `cluster` / `app_name` / `business_domain` / `service_name` / `middleware_type` 等，即标签模板映射的**源字段**），筛选**不写任何标签**、与标签管理正交（选择器 vs 描述器，见 Module\_07 5.3「标签配置唯一入口原则」）：
...
- **新增资源自动纳入（核心语义）**：filter 模式下 Job 不持有静态实例清单，M09 每次配置生成周期对条件表达式重新求值——M07 新导入 / 同步的资源匹配条件即自动进入 targets（无需编辑 Job），资源下线或属性变化导致不再匹配时自动移出；「待采集」回显（5.10）对自动纳入的新实例同样生效。
...
**生效参数层叠解析**：Job 保存时按「层叠默认链」将留空字段逐层继承到下一层、落入全局兜底常量（`DefaultScrapeInterval=15s` / `DefaultScrapeTimeout=10s` / `DefaultMetricsPath=/metrics` / `DefaultScheme=http`），保存即解析为生效快照（快照语义不变）；层叠链详见 5.1「层叠默认链」。
...
- **「生效状态」+「变更进度」拆列（F-37，替代原「状态」聚合列）**：「采集 Job」列表以两列替代原单一「状态」聚合列，列序为 … → 间隔 → 生效状态 → 变更进度 → 参数同步；两列均带列头 Tooltip 角标（? 图标）——先回答「当前是否已真正生效」，再标示「M09 管线挂在哪一环」：
- **生效状态**列（用户视角生命周期）：**草稿**（`draft_status=draft`，灰显；MVP 阶段无实例，Tooltip「v0.3 支持保存草稿」，状态筛选器中「草稿」选项禁用；v0.3 起真实草稿对象显示为正常 Tag 且可筛选）；**已停用**（`enabled=false`）；**待生效**（`enabled=true` 且 `change_status=pending/confirmed`：存在 M09 待确认或已确认但未最终下发的变更单）；**已生效**（`enabled=true` 且 `change_status=none/deployed`：无在途变更或已确认并下发成功；`deployed` 提前到 MVP，确认下发成功后由 M09 依据 ConfigDeployment success 回写 `deployed`）。列头 Tooltip：「这份配置当前是否真正生效。刚保存不会立刻生效，需到『配置变更确认』页点一次确认后才会更新」；
- **变更进度**列（M09 管线视角，`change_status` 直显映射）：`none`→无变更 / `pending`→待确认 / `confirmed`→已确认待下发 / `deployed`→已下发。列头 Tooltip：「配置下发到哪一步：待确认 / 已确认待下发 / 已下发 / 无变更。**可等所有监控配置调好后再一次性确认下发**」——批量确认口径：确认不必逐条进行，可待所有监控配置调整完毕后一次性到 M09「配置变更确认」统一确认；
- **数据回写来源**：`change_status` 由 M09 变更单 / 下发记录状态回写（pull 模式，列表查询时随 `GET /api/v1/scrape-jobs` 返回）；`deployed` 提前到 MVP，MVP 阶段 `change_status` 取 `pending/confirmed/deployed/none`，deployed 由 M09 依据 ConfigDeployment success 回写，消除「已生效 vs 无变更」歧义；
- **规则列表同用该两列（生效状态 + 变更进度，MVP 起）**：规则经「规则编辑」页整文件挂载（`content_mode=yaml_passthrough`）落库后，保存 / 启停 / 删除即进入 M09 变更检测，`change_status` 与采集 Job 同源同机制展示（两列拆列口径同上）；v0.3 随字段化编辑增强（同 5.5）。
...
**MVP 启用说明（规则文件挂载形态）**：MVP 阶段 `MonitoringRule` 即作为规则的**实际载体**（不再仅是预留定义）——规则内容以**整文件透传**（`content_mode=yaml_passthrough`，上传 / 粘贴完整 `rules.yml`）方式经本模块「规则编辑」页挂载入库，保存即 `draft_status=ready` 进入 Module_09 变更检测管线，由 M09 生成 / 确认 / 下发 `rules.yml`——**保证规则变更同样走 M09 统一配置下发闭环，不绕过 M09**（对齐：规则变更属人工确认档，需 reload）。v0.3 起升级为**字段级结构化编辑**（`content_mode=structured`，类 YAML 表单 + PromQL 校验 + 指标预览，见 3.2），M09 生成逻辑结构化优先、透传兜底。
...
| change\_status | enum | Module\_09（回写） | 下发状态 | `pending` / `confirmed` / `deployed` / `none`；**MVP 随规则文件挂载启用**——规则保存 / 启停 / 删除即触发 M09 变更单（同采集 Job 动线）；`change_status` 随 M09 **确认 / 下发 / 废弃**回写（`pending→confirmed→deployed` 或废弃，与采集 Job 同机制，见 5.4「下发状态感知」）；**校验失败自动解锁**：M09 草稿落到 `failed + user_config` 时清除本规则 `pending` 锁（草稿保留、可重校/可废弃），用户得以直接修改规则；`platform_fault` 不清锁；v0.3 随字段化编辑增强为逐条 |
...
- **草稿与提交生效（v0.3）**：规则编辑 UI 新建规则时提供「保存草稿」与「提交并进入变更确认」（统一文案，替代原「提交生效」）两种保存模式；草稿态允许 PromQL 半成品暂存，不做完整校验；提交生效时调用 Module\_02 进行 PromQL 语法/指标存在性校验，通过后 `draft_status` 转 `ready`；**草稿仅存在于新建阶段**，已生效规则的修改直接走正常变更管线；
...
**规则粒度与前端展示**：规则按求值粒度分为**聚合规则**与 **per-instance 规则**两类，对 `MonitoringRule` 数据模型与 M09 生成逻辑完全透明（不新增字段、不区分处理）：

| 粒度类型 | 典型表达式 | 告警序列标签 | 前端「实例」列展示 |
|---------|-----------|-------------|------------------|
| 聚合规则 | `absent(up{job=~"node\|node-exporter\|linux"})` | 无 `instance` 标签 | 「全局/聚合」 |
| per-instance 规则 | `up == 0` | 携带 `instance` 标签 | 实例 IP/名称 |

差异仅在 `expr` 写法：聚合规则对整批目标做全局判断（如 `absent()`），per-instance 规则为每个实例单独求值。MVP 种子规则已同时包含两类（见 `config-output/rules.yml`），用户文件挂载时两类规则写法均可。v0.3 字段化编辑将提供 per-instance 规则模板（见 3.2）。

**规则挂载「检查」与提交门禁（MVP 随规则文件挂载启用）**：

- **两段式状态机**：抽屉内「检查」按钮常驻，点按后调用 `POST /api/v2/platform/monitoring-rules/:id/validate-yaml` 取得 `{valid, error?, job_ref?}`；**「提交并进入变更确认」按钮仅在 `canSubmit = 检查通过(valid=true) 且（无 error 级 job_ref 或 已勾选逃生门）` 时出现**（条件渲染，非 disabled），新建（前端传 `:id=0`）与编辑**同一套逻辑**；
- **`valid` = 预检是否可提交**：硬失败三类——YAML 语法 / `groups` 结构、**组名全局唯一性冲突**；`job_ref.severity=error` 为**可覆盖轴**、不改写 `valid`；
- **重置规则**：**规则内容变更**时「检查结论 + ack」一并作废、提交按钮重新隐藏、须重新检查；规则名 / 资源类别 / 监控对象类型等**非内容字段不触发重置**；
- **结果面板位置**：表单**下方**、操作按钮**上方**（与刚编辑的内容保持视线连续）；「检查未通过（红）」/「检查通过（绿，含服务端复核边界说明）」互斥呈现，job 引用提示按其 `severity` 呈**红（阻断，附逃生门）**或**黄（仅提示）**；
- **逃生门与留痕**：勾选「已知晓：先挂规则，稍后补建 Job」提交时携带 `ack_job_ref_errors=true`，问题降级 warning 落库留痕（与采集 Job 同机制）。

**规则启停 / 草稿 / 锁定口径**：

- **「停用」可编辑提级 MVP**：规则**停用可编辑**（复用「编辑」模式，`enabled=false` 即从后续 `rules.yml` 生成中摘除），**不新增独立携带 `enabled` 的字段集**（沿用 `MonitoringRule.enabled`）；
- **规则草稿（`draft`→`ready` 批量提交）推迟至 v0.3**：`MonitoringRule` 的「保存草稿 / 提交并进入变更确认 + 批量提交」能力随 **v0.3** 字段化编辑交付（提交按钮文案统一为「提交并进入变更确认」），MVP 阶段规则仅 `ready`（文件挂载保存即 `ready`）；
- **规则 pending 期锁定（同采集 Job）**：`change_status=pending` 期间**禁止编辑 / 启停 / 删除**该规则；解锁路径同采集 Job（M09 变更单**确认**、**废弃**），**并新增「校验失败自动解锁」：M09 草稿 `validation_status=failed` 且 `validation_cause=user_config` 时，平台自动清除本规则 `pending` 锁，草稿保留可重校/可废弃——避免失败单不可确认却锁死源数据、用户只能靠废弃解锁的死循环**（`platform_fault` 不清锁）。

### 5.6 Exporter 安装/注册登记（ExporterInstallationConfirmation，可选）

> 决策依据：design-decisions.md 决策 7 / 决策 47-1

**定位修订**：本记录由「生成 target 的前置闸门」降级为**可选登记**——纯留痕与人工背书（谁承诺装好了、工单号），**不再阻断 target 生成**：`unconfirmed` 实例照常进入 M09 配置生成。「是否采到数据」的事实反馈由 5.10 采集状态回显承担（`up` / `down` 是真相反馈，登记只是口头背书）。
...
**说明**：`ScrapeTarget` 由 Module\_01 的策略配置与 Module\_09 生成的配置共同决定，但**独立的运行时状态页面**由 [Module\_02: 查询中心](Module_02_Query_Center.md) 负责；本模块仅在 Job 上下文做**只读回显**（实例采集状态列 + 在线数汇总，消费 M02 `/api/v1/targets` 代理，见 5.10）。
...
**业务域聚合视图版本归属**：MVP 提供**轻量业务视图**（业务指标库页内 Tab「登记表 / 业务视图」）——按 `business_domain` 自动聚合成员（应用服务/微服务 + 中间件 + 主机，从 Resource.business_domain 归并）+ 业务指标清单 + 埋点/采集落地状态，把"微服务是业务域的实现载体、业务域是微服务的语义聚合"在 MVP 即可感知；**v0.3+ 独立业务目录**（Module\_07）提供完整业务域聚合视图——成员列表 + 健康度看板（M01-BIZ-02）+ 采集覆盖视图。**业务语义层不改变采集配置逻辑**（采集仍按监控对象类型 + 实例选择），仅影响视图 / 查询聚合（`biz`）/ 告警分组（v0.3+）/ 批量操作入口。
...
| 只读消费（本模块 ← Module\_02） | `GET /api/v1/targets`（M02 代理） | Job 实例采集状态回显（5.10）：按 Job 过滤取 health / lastScrape / lastError，本模块不直连 Prometheus |
...
| POST | `/api/v1/monitoring-rules` | 5.5 字段（`content_mode=yaml_passthrough` 时**必填** `rule_content`；可选 `ack_job_ref_errors` 默认 `false`） | 创建后的完整对象（保存即 `draft_status=ready`，进入 M09 变更检测） | `bad_request`：`rule_content` 为空 / YAML 语法非法（至少校验 `groups` 存在且为数组）/ **group 名全局冲突** / **存在 error 级 job 引用且未置 `ack_job_ref_errors=true`** |
| PUT | `/api/v1/monitoring-rules/{id}` | 5.5 可更新字段（`name`、`rule_content`、`enabled` 等；可选 `ack_job_ref_errors` 默认 `false`） | 更新后的完整对象 | `not_found`；`bad_request`：YAML 非法 / group 名冲突（排除自身）/ 存在 error 级 job 引用且未置 `ack_job_ref_errors=true` |
| POST | `/api/v1/monitoring-rules/{id}/validate-yaml` | `{ rule_content }`；`:id` 为真实规则 ID 或 `0`（新建，前端传 `0`，不排除自身） | `{ valid, error?, job_ref? }`——`valid` = **预检是否可提交**（YAML 语法 / `groups` 结构 / **组名全局唯一性**）；`job_ref[]` 元素 `{group, rule_name, expr, matcher, referenced_job, type, severity, message}`，`severity∈{error,warning}` | —（校验结论经响应体表达，不抛业务错误） |
| DELETE | `/api/v1/monitoring-rules/{id}` | — | `{ id }` | `not_found` |

说明：MVP 的 MonitoringRule 变更（保存 / 启停 / 删除）即触发 Module\_09 变更检测 → 生成 `rules.yml` 草稿 → 变更单**人工确认** → 下发（人工确认档，与采集 Job 同动线）。YAML 校验可复用 Module\_09 configgen `--config.check`，或后端 YAML 解析（至少校验 `groups` 存在且为数组）。

**`validate-yaml` 语义（2026-09-11）**：①在 YAML 语法通过后追加 **job 引用语义校验**（`up` / `absent(up)` 类不匹配 = `error`，其余 = `warning`；与当前生效采集 Job 比对）；②**并入组名全局唯一性**——与提交侧 `validateGroupNamesAvailable` **同实现、同输入集**，仅当目标规则生效时校验、停用规则不校验、`:id` 命中自身时排除自身；③`valid` 语义**由「仅语法」扩展为「预检是否可提交」**，硬失败（YAML 语法 / `groups` 结构 / 组名冲突，**不可覆盖**）折叠进 `valid=false + error`，`job_ref.severity=error` 保持为**可经逃生门覆盖**的独立轴、**不改写 `valid`**；④响应形状 `{valid, error?, job_ref?}` **保持不变**。前端消费方式见 §11.2「规则挂载『检查 → 提交』两段式」。
...
说明：登记主键维度为 `resource_id` × `scrape_job_id`（同一实例被不同 Job 选中时分别登记，见 5.6）；登记状态机见 8 章 ④；`not_applicable` 状态由 Job 类型决定，不生成登记记录。**登记与否不影响 target 生成**，接口仅承载可选留痕与 `actual_port` 登记。
...
| 指标库（Exporter 指标库） | ✅ | ❌ | ❌ | ❌ |
| Resource 采集状态 badge（三态） | 提供选中关系数据（`is_monitored`） | ✅ 展示（up/down 聚合来自 Module\_02 健康度 API） | ❌ | ❌ |
...
>
> **规则侧另有「校验失败自动解锁」路径**：`MonitoringRule` 在 M09 草稿 `failed + user_config` 时自动清锁（草稿保留），避免不可确认的失败单锁死源数据（见 5.5「规则 pending 期锁定」）。**采集 Job 侧是否同等覆盖为未决项**——Job 的废弃涉及「新建回退 draft / 删除停用自动恢复」的分类回写，与规则的纯清锁语义不同，需单独评估后再定（见 `docs/05-execution-records/module-09/rule-jobref-validation-lifecycle-fix.md` §7）。
...
manual（MVP：手动勾选，候选按类型+网域收敛）
 │
 ├──► filter（v0.2：按资源属性条件筛选，label 仅 UI 别名；每生成周期实时求值，新增资源自动纳入）
 │
...
> **修订说明**：登记不再作为生成 target 的前置——`unconfirmed` 实例**照常参与** M09 配置生成；target 生成只取决于 `selected_instance_ids`（+ `offline` 排除 + `enabled` + `draft_status`）。「是否采到数据」的事实反馈由 ⑥ 采集状态回显承担。
...
- [ ] {P0} 采集 Job 支持配置认证/TLS 最小集：`auth_type`（none/basic/bearer）＋对应 `username`/`password` 或 `token`、`tls_skip_verify`、`ca_file`；全部可选、默认 `auth_type=none`，不配置时与既有裸 http 采集完全兼容；表单以「认证与 TLS」折叠面板承载。
- [ ] {P0} 冻结（禁用）网域禁止新建 Job、存量 Job 禁止新增该域实例；允许移除该域实例、禁用/编辑 Job；保存时给出明确错误提示。
...
- [ ] {P0} 采集 Job 支持 `filter` 实例选择模式：按资源属性条件表达式筛选并预览匹配结果；保存后 M07 新导入/同步的匹配资源在下一配置生成周期自动纳入 targets，无需编辑 Job；不再匹配的资源自动移出
- [ ] {P0} 采集 Job 支持勾选多个已纳管网域；保存后 M09 按网域自动拆分生成各域配置与变更单，每域独立校验 / 确认 / 下发；MVP 单网域行为不变
...
- [ ] {P0} 选中实例（`offline` 除外）**均生成 target**，安装登记不作为生成前置；可以选登记 Resource/Target 的采集器安装/注册状态（含自研独立 exporter），登记与否不影响 target 生成。
- [ ] {P0} Job 详情/编辑抽屉实例列表展示「采集状态」列（待采集 / up / down / unknown）与外层汇总（在线数 up / 实例总数 / 待采集数）；新保存未下发的实例显示「待采集」、在线数不变；变更下发并完成首次抓取后状态自动转为 up / down。
- [ ] {P0} 已下发但 down 的实例展示提醒「配置已下发但未采集到数据，请检查采集器安装与网络连通」，并附 `lastError` 摘要。
- [ ] {P1} 安装登记时可登记实例上 exporter 实际监听端口（actual\_port）；配置生成时与生效端口不一致时提示，不自动改配置。
- [ ] {P0} 实例级端口覆盖生效：M07 Resource 可选 `scrape_port` 字段登记 / 编辑 / Excel 导入（留空走默认解析链）；M09 生成 target 时端口解析优先级为 `Resource.scrape_port`（实例自带）→ 网域覆盖表 `CITypeExporterMappingOverride` → 映射默认 `CITypeExporterMapping.default_port`（回落 `ExporterTemplate.default_port`）；`scrape_port` 留空时按默认解析链回落；Job 实例列表只读回显「生效端口」；**不提供 Job 级端口映射表**
- [ ] {P0} 规则编辑 UI 支持类 YAML 表单（expr / for / labels / annotations），调用查询中心进行 PromQL 校验，并提供指标实时预览。
...
- [ ] {P0} [Module\_07: 监控对象管理](Module_07_Monitoring_Object_Management.md) 的 Resource 列表可展示三态「采集状态」badge（采集中 / 已下发未采到 / 未监控）：选中关系（`is_monitored`）来源于本模块的 `ScrapeJob`，up/down 聚合来源于 [Module_02](Module_02_Query_Center.md) 健康度 API。
...
- [ ] {P0} 默认标签模板必须包含 `resource_id` 稳定资源身份标签（coverage 回连键；`hostname` 仅为可读别名、不替代 `resource_id`），`instance`（ip:port）不作为业务稳定关联键
...
- [ ] {P0} 「采集 Job」列表以「生效状态」+「变更进度」两列替代原「状态」聚合列（F-37）：生效状态列展示生命周期四态（草稿 / 待生效 / 已生效 / 已停用；MVP 阶段「草稿」态无实例，标签灰显并 Tooltip 提示「v0.3 支持保存草稿」，状态筛选器中「草稿」选项禁用）；变更进度列展示 change_status 四枚举映射（无变更 / 待确认 / 已确认待下发 / 已下发；MVP 阶段 M09 回写 pending/confirmed/none/deployed，deployed 提前到 MVP）；两列均带列头 Tooltip 角标，含「可等所有监控配置调好后再一次性确认下发」批量确认口径
- [ ] {P0} 「规则编辑」页支持上传 / 粘贴完整 `rules.yml`（**整文件透传**，`content_mode=yaml_passthrough`）挂载规则；「从本地选择 rules.yml」入口位于编辑框**上方**（「字段说明 → 选文件 → 编辑框」成一条垂直读序）；保存即 `draft_status=ready` 进入 M09 变更检测管线
- [ ] {P0} 规则挂载抽屉为**两段式**：footer 常驻「取消」+「检查」，**未检查 / 检查未通过时没有提交按钮**；提交按钮文案新建与编辑统一为「**提交并进入变更确认**」（**条件渲染而非 disabled**）
- [ ] {P0} 点「检查」后调用 `validate-yaml`，结果面板呈现在**表单下方、操作按钮上方**：YAML 语法 / `groups` 结构非法 → 红面板「规则检查未通过」+ 提交按钮不出现；**与既有生效规则组名冲突 → 红面板点明占用方 + 提交按钮不出现**（核心验收点）；编辑该规则自身、组名不变 → 检查**通过**（自排除生效）；编辑**停用**规则、组名与生效规则重复 → 检查**通过**（镜像提交侧条件，避免无出路）
- [ ] {P0} job 引用分级阻断在**检查阶段**即呈现：引用不存在 job 的**存活类**（`up` / `absent(up)`）规则 → 红面板 + 逃生门「已知晓：先挂规则，稍后补建 Job」，提交按钮不出现；**勾选逃生门 → 提交按钮出现**，提交成功后问题降级为 warning 落库留痕（请求携带 `ack_job_ref_errors=true`）；仅含 **warning** 级 job 引用的内容 → 黄面板 + 提交按钮直接出现
- [ ] {P0} 「检查通过」绿提示写明免责边界：**服务端校验覆盖「YAML 语法 + 组名唯一性 + job 引用」，提交时服务端仍会复核**（检查通过 ≠ 提交必过）；后端不可用回落本地兜底时（不含 job 引用校验）同样展示该边界，提交被 `errorType=job_ref_unresolved` 拦下时面板给出逃生门并隐藏提交按钮
- [ ] {P0} **规则内容变更后**：已通过的检查结论与 ack 一并失效、提交按钮消失、须重新检查；规则名 / 资源类别 / 监控对象类型等**非内容字段变更不触发重置**
...
- [ ] {P0} 规则挂载后，M09 生成的 `rules.yml` 草稿包含该透传规则内容，支持变更单**人工确认**后下发生效（与采集 Job 同动线）；下发后规则状态回写 `change_status`
- [ ] {P0} 采集 Job 新建表单提供「保存草稿」与「提交生效」两个主操作：「保存草稿」仅做基础校验，允许半成品暂存；「提交生效」做完整校验（含必填项、网域已纳管、实例同域等），通过后 `draft_status` 转 `ready` 并进入 M09 变更检测管线；已生效 Job 的编辑仅提供普通「保存」，直接走 M09 变更单确认管线（不做草稿快照）
- [ ] {P0} 采集 Job 列表支持多选「批量提交生效」：选中的 `draft_status=draft` 对象逐条校验，失败的给出逐条错误清单，成功的批量转 `ready`
- [ ] {P0} 采集 Job 列表操作列提供「克隆」入口：打开新建抽屉并预填源 Job 的采集参数 / 监控对象类型 / 采集实现 / 标签模板；同网域克隆可直接改选实例，跨网域克隆需改选目标网域、实例清空重选，并提示安装登记（可选）需重新进行
- [ ] {P0} 克隆产物为独立 Job，与源 Job 无绑定关系（源 Job 后续变更不影响克隆产物），不引入「Job 模板」持久实体
- [ ] {P0} 规则编辑 UI 同样提供「保存草稿」与「提交并进入变更确认」（仅新建阶段；提交按钮文案与 MVP 文件挂载统一）；草稿态允许 PromQL 半成品，提交时调用 Module_02 做 PromQL 校验
...
- [ ] {P0} `ExporterTemplate.source` 枚举（`official` / `third_party` / `internal`）落库；**用户登记三种来源均可**，创建时 `default_port` / `metrics_path` / `scheme` 均必填校验；与平台预置 seed 同名返回 409 Conflict；「采集 Job」列表接口支持 `network_domain_id` 查询参数（仅接受已纳管网域）。
...
- [ ] {P0} 保存草稿时仅更新 `draft_status`/`updated_at`，**不触发** M09 源数据版本重算；提交生效时 `updated_at` 变化且 `draft_status=ready`，M09 下一周期将其纳入源数据版本并生成配置草稿
- [ ] {P0} `GET /api/v1/scrape-jobs` 返回 `draft_status`，前端据此与 `change_status` 聚合状态列「草稿」态
- [ ] {P0} `POST /api/v1/scrape-jobs/{id}/clone` 复制源 Job 的采集参数 / `job_type` / 监控对象类型 / 采集实现 / 标签模板，**不携带** `selected_instance_ids` 与安装登记记录；跨网域克隆时校验目标网域已纳管
...
- [ ] {P0} `POST /api/v1/monitoring-rules/{id}/validate-yaml` 响应语义 = **预检是否可提交**：`valid=false` 覆盖三类硬失败——YAML 语法 / `groups` 结构、**组名全局唯一性冲突**（与提交侧 `validateGroupNamesAvailable` **同实现、同输入集**：仅当目标规则生效时校验、停用规则不校验、`:id` 命中自身时排除自身）；`job_ref[].severity=error` **不改写 `valid`**；响应形状 `{valid, error?, job_ref?}` 向后兼容
- [ ] {P0} POST / PUT `monitoring-rules` 支持可选 `ack_job_ref_errors`（默认 `false`）：存在 error 级 job 引用且未置 `true` → `bad_request`（`errorType=job_ref_unresolved`，消息汇总问题条数）；`warning` 级不受影响（**本轮不改后端判定逻辑**，仅前端提前调用同一接口并据此排布按钮）
- [ ] {P0} Module\_09 生成 `rules.yml` 时，`content_mode=yaml_passthrough` 的规则将 `rule_content` **原样并入**；规则保存 / 启停 / 删除引起 `updated_at` 变化后，M09 下一轮询周期检测并重新生成 `rules.yml` 草稿，经变更单**人工确认**后下发（pull 模式，M01 不主动通知；对齐）
- [ ] {P0} `MonitoringRule` 草稿机制与 `ScrapeJob` 一致（仅新建阶段），提交生效时调用 Module_02 PromQL 校验接口
***
...
| 后端术语 | 用户语言 | 说明 |
| ------------------------------------- | ---------------- | ------------------------------------------------------------ |
| `CITypeExporterMapping` | 默认采集配置 / 采集器管理 | 监控对象类型 ↔ 默认采集器（采集实现）的绑定；采集实现层预设，不绑网域；每类型可多行、`is_default` 标记默认；含安装指南；入口为独立「采集器管理」页面（`/collectors`），与「采集 Job」页（`/scrape-jobs`）为「采集策略」一级模块下的 Sider 二级导航项（与「规则编辑」「指标库」并列，以生产导航为准），承担类型级采集器指引（该装什么、怎么装），实例级安装登记（可选）在选实例时（5.6） |
| `CITypeExporterMappingOverride` | 网域级覆盖 | 按网域覆盖映射默认值（端口 / 协议 / 采集路径等），优先级高于映射默认值 |
| `monitor_type`（细粒度） | 监控对象类型 | `host_linux` / `host_windows` / mysql / redis / kafka / nginx / application\_http / snmp；增 `k8s_apiserver` / `k8s_kube_state_metrics` / `k8s_etcd`（generic_target 按端点子类型 `exporter_type` 推导）；派生的策略维度，只存在于监控平台内部、不回写 CMDB |
...
| `ExporterTemplate` | 采集实现 / 采集器 | 采集实现的默认参数、安装指南、`download_url` / `homepage` / `os` / `arch`（归属采集实现，非监控对象类型）；MVP 支持平台预置（`is_builtin=true`）与用户登记的官方 / 第三方 / 自研采集器（`is_builtin=false`）；预置参数 = 官方默认值参考；非市场运营对象，不支撑指标库分组锚点 |
| `ExporterTemplate.source` | 采集器来源 | `official`（开源官方）/ `third_party`（第三方）/ `internal`（自研）；用户登记三种来源均可，创建时默认端口 / 采集路径 / 协议均必填并提示按实际部署填写；与平台预置 seed 同名返回 409 Conflict；登记即入池，与预置采集器同动线 |
| `ScrapeJob` | 采集 Job | 实际采集任务（实例层），绑定单一网域 |
| `change_status` | 下发状态 | 采集 Job / 规则的变更下发状态：`pending`（存在 M09 待确认变更单）/ `confirmed`（变更单已确认，待下发）/ `deployed`（已下发生效，由 M09 依据 ConfigDeployment success 记录回写，提前到 MVP）/ `none`（无在途变更）；MVP 阶段回写 `pending/confirmed/none/deployed` |
...
| 克隆 Job | 克隆 Job | 一次性复制源 Job 的采集参数 / 监控对象类型 / 采集实现 / 标签模板生成新 Job（非持久模板、与源 Job 无绑定）；实例选择与安装登记（可选）重做，跨网域克隆时目标网域改选；**已移出 v0.2 范围**（扇出已覆盖跨网域复用主场景） |
...
| `ExporterInstallationConfirmation` | Exporter 安装登记（可选） | 目标实例独立进程型采集实现（内置 Exporter 或自研采集器）已安装/已注册的登记记录；**可选留痕，不作为生成 target 的前置**；`application_http` / `blackbox` 无需安装登记 |
| 采集状态（Job 实例回显） | 采集状态 | Job 详情/编辑抽屉实例列的真实采集状态：待采集 / 正常（up）/ 异常（down）/ 未知（unknown）；数据来自查询中心目标状态代理，只读（见 5.10） |
| 在线数 / 待采集数 | 在线数 / 待采集数 | Job 实例区顶部汇总：在线数 = up 实例数 / 实例总数；待采集数 = 已保存但变更未下发或未首次抓取的实例数 |
...
| 规则编辑（文件挂载·MVP） | 未检查（初始/重开后） | footer 只有「取消」+「检查」，**没有提交按钮** |
| 规则编辑（文件挂载·MVP） | 检查未通过（YAML 非法 / `groups` 结构 / 组名冲突） | 表单**下方**红面板「规则检查未通过」+ 原因（组名冲突点明被哪条规则占用）；**无提交按钮**，表单保持当前内容 |
| 规则编辑（文件挂载·MVP） | 检查通过（无 job 引用问题） | 表单**下方**绿面板「检查通过」，含边界说明「服务端校验覆盖语法 + 组名唯一性 + job 引用，提交时仍会复核」；「提交并进入变更确认」按钮**出现** |
| 规则编辑（文件挂载·MVP） | 检查发现 job 引用问题 | 表单下方面板按 severity 分级：存在 `error` 级（存活类规则缺 job）→ **红面板** + 逃生门「已知晓：先挂规则，稍后补建 Job」，勾选后提交按钮出现；仅 `warning` 级 → **黄面板**，提交按钮直接出现 |
| 规则编辑（文件挂载·MVP） | 检查后修改内容 | 检查结论与逃生门勾选**一并作废**，提示「规则内容已修改，请重新检查」；提交按钮重新隐藏（仅规则内容变更触发，改名不触发） |
...
- **规则挂载「检查 → 提交」两段式（交互重排）**：
 - **两段式**：抽屉 footer 常驻「取消」+「**检查**」，「**提交并进入变更确认**」按钮**按状态出现（条件渲染，非 disabled）**——`canSubmit = 检查通过(valid=true) 且（无 error 级 job 引用 或 已勾选逃生门）`；新建与编辑**同一套状态机**，文案统一为「提交并进入变更确认」（原「提交生效」/「保存变更」作废）。
 - **检查语义**：`validate-yaml` 的 `valid` = **预检是否可提交**，硬失败（**不可覆盖**）三类——YAML 语法 / `groups` 结构非法、**组名全局唯一性冲突**（与提交侧同实现同输入集，仅生效规则校验、停用不校验、编辑时排除自身）；`job_ref.severity=error`（存活类 `up`/`absent(up)` 规则缺 job）为**可覆盖轴**、不改写 `valid`。
 - **分级呈现与逃生门**：结果面板位于**表单下方、操作按钮上方**；「检查未通过（红）」/「检查通过（绿，含边界说明）」互斥，job 引用提示按 severity 呈**红（阻断 + 逃生门「已知晓：先挂规则，稍后补建 Job」，勾选后提交按钮出现、问题降级 warning 落库留痕）**或**黄（仅提示，不阻断）**，面板须能逐条列出规则名 / matcher / 引用 job / 说明。
 - **检查通过 ≠ 提交必过（用户可见的免责边界）**：「检查通过」绿提示须写明「服务端校验覆盖「语法 + 组名唯一性 + job 引用」，**提交时服务端仍会复核**」；后端不可用回落本地兜底时不展示「组名唯一性 / job 引用」结论但**仍须展示该边界**——提交若被 `errorType=job_ref_unresolved` 拦下，面板给出逃生门并隐藏提交按钮，勾选后按钮重新出现。
 - **重置**：**规则内容变更**时检查结论与 ack 一并作废、提交按钮重新隐藏；规则名等非内容字段不触发重置。
- **失败单不锁死源数据**：M09 变更单校验失败且归因 `user_config` 时，本模块源数据的 `pending` 锁**自动清除**（草稿保留可重校/可废弃），用户可直接回本模块修改规则；列表「变更进度」列刷新后不再显示锁定态。归因 `platform_fault` 时保持锁定（非用户可修）。
- **参数继承来源视觉标记**：编辑表单中继承字段的 label 旁增加 inline Tag，标记来源状态（继承自映射/已覆盖/待同步），详见 5.4「参数继承来源视觉标记」。
- **认证/TLS 表单折叠区（MVP）**：采集 Job 表单底部提供「认证与 TLS」**折叠面板**（默认折叠，不展开不增加裸 http 用户的视觉负担）：
...
- **克隆 Job（v0.3+ 待评估，已移出 v0.2 范围， 扇出已覆盖跨网域复用主场景）**：列表操作列提供「克隆」入口 → 打开新建抽屉并预填源 Job 的采集参数 / 监控对象类型 / 采集实现 / 标签模板；同网域克隆可直接改选实例，跨网域克隆需改选目标网域、实例清空重选，并提示「安装登记（可选）需重新进行」；克隆产物为独立 Job，不引入「Job 模板」持久实体。
- **实例采集状态回显**：Job 详情/编辑抽屉实例区展示「在线 X / 总数 Y · 待采集 Z」汇总与实例级「采集状态」列；回显只读、异常驱动展示（正常低饱和、异常高饱和并附 `lastError` 摘要与「配置已下发但未采集到数据，请检查采集器安装与网络连通」提醒）；数据来自 Module_02 `/api/v1/targets` 代理，定时刷新（15~30s）+ 手动刷新，不阻断编辑与保存。
