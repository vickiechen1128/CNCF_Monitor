...
> **动线顺序与按钮层级**：「采集器管理」Tab 的**主流程**是「配置默认采集配置（监控对象类型 ↔ 采集器）」；「登记采集器」是**低频前置补救动作**，仅在没有合适采集器时执行。页面顶部**主按钮为「新增默认采集配置」**，「登记采集器」入口降级为次级按钮，并同时内置于采集器选择器的空态引导中；页面引导以编号动线说明呈现：「① 登记采集器（仅自研 / 第三方需要）→ ② 配置默认采集（监控对象类型绑定采集器 + 参数）→ ③ 到「采集 Job」创建任务并选实例时确认安装」，一眼看清顺序与可选性。
>
...
**入口与命名**：本预设层**不再独立导航**，**入口承载于「采集 Job」页「采集器管理」Tab**——创建 Job 时自动套用该监控对象类型的默认值，用户在 Tab 内查看/维护预设（采集器 / 参数 / 安装指南 / 标签模板）。

> **安装动线指引与职责边界**：本 Tab 承担**类型级采集器指引**——"该监控对象类型该装什么采集器（默认/可选，已按 OS 平台区分）、怎么装、去哪下载（安装指南 / download_url / homepage 明显展示）"；**不做实例级安装确认**（那是 5.6 `ExporterInstallationConfirmation`，在「采集 Job」创建任务选实例时进行，resource×exporter 维度）。动线闭环以**文案衔接**闭合：「看指南 → 去线下目标机安装/下载 → 装完回到「采集 Job」选实例时标记已安装 → 生成配置」——避免两处重复的确认 UI。
...
> - **表单内预览（紧凑卡片）**：映射 / Job 表单选择标签模板后，以紧凑卡片展示模板头部（名称 + ID + 默认标记）与映射明细（小表格/列表），替代 Tag 堆砌；并提供「前往标签模板管理」跨模块跳转（Module\_07 为模板 CRUD 归属方，本模块不重复编辑）。跨模块跳转由**统一导航配置承载**，原型可暂用相对路径演示，实现期不得在代码中写死路径，统一走导航/路由配置。

> **标签模板创建引导**：
>
> - **预设抽屉「默认标签模板（可选）」字段**：新增 / 编辑默认采集配置的表单提供该可选字段——**按当前资源类别过滤候选**（该类别下所有模板），label「默认标签模板（可选）」+ extra「该监控对象类型的默认标签模板：创建采集 Job 时自动预填，可更换；不选则创建 Job 时再选择」。不隐藏、不强制——标签模板与采集器正交，字段存在是为了让「默认模板标记到监控对象类型」在映射层可落地；用 `Divider` 与采集参数隔开（分隔标题「标签模板（与采集器正交，可选）」），视觉上声明"这不是采集器的一部分"；
> - **补配入口收敛（一个动作两个触发点）**："补配"的本质是在映射层设置默认模板，**统一动作 = 打开该监控对象类型映射行的编辑抽屉**（不需要第三个页面）：①Job 表单缺模板 Alert 主按钮 → 跳采集器管理并**自动打开该映射的编辑抽屉**（带参 `?view=collectors&edit=<mapping_id>`）；②映射列表「补配」按钮 / 「待配置」badge → **同页直接打开本行编辑抽屉**；③「前往标签模板管理（M07）」**仅保留在抽屉内标签模板选择器的空态**（该类别真的无模板时才需要去 M07 创建，复用空态依赖引导）；
...
>   - **「稍后再说」**：关闭提示，映射列表该行「标签模板」列显示「标签模板待配置」badge，用户可后续点击「补配」**同页打开本行编辑抽屉**重新设置；
> - **缺口类型区分与软引导**：Job 表单 Alert 文案按缺口区分——①映射有候选但未关联模板：「该监控对象类型的默认采集配置尚未关联标签模板」+ 主按钮「立即补配」（打开映射编辑抽屉）；②该资源类别下无任何模板：「该资源类别尚无标签模板」+ 主按钮「前往创建模板」（M07，此时才是真阻塞）。**软引导、不阻塞保存**：缺模板只是监控数据缺归属标签，Job 仍可运行——保持 warning 提示、不升级为必填校验（与默认采集器必填不同）；
...
> - **application\_http 在「采集器管理」Tab 呈现为引导卡、不进登记流程**：`application_http` 不是采集器、无安装动作——Tab 内对该监控对象类型展示**引导卡**而非采集器登记入口：「业务应用自带 `/metrics` 端点，无需安装采集器；指标语义请前往业务指标库（5.9）登记；创建采集 Job 时端口 / 路径按应用实际 endpoint 手填」。避免用户把每个微服务登记成一个"采集器"。
...
| network\_domain\_id       | string    | Module\_09            | 网域              | 归属网域；**必填**，所有 ScrapeJob 必须绑定且仅绑定单一**已纳管网域**（未在 M09 完成监控纳管的网域不可选，保存时校验）；「采集 Job」列表页提供网域查询条件（选项 = 已纳管网域） |
...
> **filter 模式字段语义（v0.3+）**：`instance_filter` 的筛选字段 = **Resource 属性字段**（`env` / `cluster` / `app_name` / `business_domain` / `service_name` / `middleware_type` 等，即标签模板映射的**源字段**），筛选**不写任何标签**、与标签管理正交（选择器 vs 描述器，见 Module\_07 5.3「标签配置唯一入口原则」）：
>
...
> **端口不在 Job 层的理由**：`scrape` 端口**不纳入 `ScrapeJob` 可覆盖字段**——端口是 target 级参数，且直接影响 Prometheus 的 `instance` 标签（ip:port），端口 / 漂移会使其不稳定；`instance` 仅作为 **Prometheus 抓取目标身份**，不作为业务关联身份（业务关联走 `app` / `biz` / 稳定资源身份标签，见 5.1）。端口分层解决：MVP 通过 `CITypeExporterMapping.default_port` 解决类型级非标端口；v0.2 通过 `CITypeExporterMappingOverride` 解决网域级端口（含安全 / 高危端口场景）；v0.2+ 再评估实例级端口覆盖。Job 表单只提供 Job 级统一参数：间隔、超时、协议、指标路径。
...
> - **补配动作收敛**：Job 表单缺模板 Alert 的**主按钮 = 跳采集器管理并自动打开该映射行的编辑抽屉**（带参 `?view=collectors&edit=<mapping_id>`，落位即开）；「前往标签模板管理（M07）」**不再作为 Alert 次级入口**（仅保留在抽屉内标签模板选择器空态）；
...
> **标签配置引导落地（原型对齐）**：
>
> - **Job 表单主引导**：「标签模板」为空且对应监控对象类型映射未配置模板（`has_label_template=false`）时，显示「该监控对象类型尚未配置标签模板」提示，**主引导为「立即补配」（跳采集器管理并自动打开该映射行编辑抽屉）**；「前往标签模板管理」不再并列于 Alert（仅抽屉选择器空态出现）；
...
| exporter\_template\_id | string   | 默认采集器     | 关联默认采集器（采集实现）                                      |
...
> **职责边界**：安装确认是"状态登记 + 人工背书"，**不承担端口编辑**（维度为 resource×exporter、不分 Job）；实际监听端口仅作登记与一致性提示，端口不一致的解决手段见 5.1「端口一致性说明」（映射层 default\_port 可编辑 → 网域覆盖 v0.2 → 实例级端口覆盖 v0.2+）。
...
> 说明：安装确认状态机见 8 章 ④；`not_applicable` 状态由 Job 类型决定，不生成确认记录。
...
- [ ] {P0} 默认采集配置**入口承载于「采集 Job」页「采集器管理」Tab**（不独立导航，导航无「监控对象类型 ↔ 默认采集器 / CI-Exporter」等入口）；创建 Job 时自动套用该监控对象类型的默认采集配置，Tab 内可维护预设（采集器 / 参数 / 安装指南 / 标签模板）
- [ ] {P0} 「采集器管理」Tab 承担**类型级采集器指引**（该监控对象类型该装什么采集器、安装指南 / 下载地址 / 官方文档入口明显展示），**不做实例级安装确认**——确认在「采集 Job」选实例时进行（5.6），动线以文案衔接（看指南 → 线下安装/下载 → 选实例时标记已安装）
- [ ] {P0} 「采集器管理」Tab 列表支持按监控对象类型 + 来源（开源官方 / 第三方 / 自研）筛选；登记表单选择「自研」时默认端口 / 采集路径 / 协议必填并提示「按实际部署填写」；预置参数标注「官方默认值参考」
- [ ] {P0} 自研采集器登记后即入池：可被映射引用为默认采集器、创建 Job 时预填参数、可走实例级安装确认（与平台预置采集器一致）
- [ ] {P0} `application_http` 在「采集器管理」Tab 呈现为引导卡（无需安装采集器 / 指标语义到业务指标库登记 / Job 参数按 endpoint 手填），不提供采集器登记入口
...
- [ ] {P0} 默认标签模板至少包含一个稳定资源身份标签（`resource_id` / `hostname`），`instance`（ip:port）不作为业务稳定关联键
...
- [ ] {P0} 新增 / 编辑默认采集配置表单提供「默认标签模板（可选）」字段，按当前资源类别过滤候选，extra 说明预填语义
...
- [ ] {P0} 预设抽屉「默认标签模板（可选）」用 Divider 与采集参数分组展示（分隔标题「标签模板（与采集器正交，可选）」）
- [ ] {P0} 补配入口收敛：Job 表单缺模板 Alert 主按钮带 `?view=collectors&edit=<mapping_id>` 跳转并自动打开映射编辑抽屉；映射列表「补配」/「待配置」badge 同页打开本行编辑抽屉；「前往标签模板管理」仅保留在抽屉选择器空态
...
- [ ] {P0} `ExporterTemplate` 增加 `os / arch / download_url / homepage` 字段并落库；`host` 粗粒度资源按 `os_type` 映射为 `host_linux` / `host_windows` 细粒度监控对象类型；自研采集器（`is_builtin=false`）可创建、查询并在「采集器管理」Tab 展示。
...
| 后端术语                                  | 用户语言             | 说明                                                           |
| ------------------------------------- | ---------------- | ------------------------------------------------------------ |
| `CITypeExporterMapping`               | 默认采集配置 / 采集器管理 | 监控对象类型 ↔ 默认采集器（采集实现）的绑定；采集实现层预设，不绑网域；每类型可多行、`is_default` 标记默认；含安装指南；入口在「采集 Job」页「采集器管理」Tab，承担类型级采集器指引（该装什么、怎么装），实例级安装确认在选实例时（5.6） |
...
| 采集器管理 Tab | 加载中 | 表格骨架屏 |
| 采集器管理 Tab | 空态 | 「暂无默认采集配置」，提供「新增默认采集配置」引导；「登记采集器」为次级入口 |
| 采集器管理 Tab | 接口错误 | Alert 提示「采集器数据加载失败，请稍后重试」，提供「重新加载」按钮 |
| 采集器管理 Tab | 数据超量 | 表格分页，支持按监控对象类型/来源筛选 |
| 技术指标库 | 加载中 | 表格骨架屏 |
...
- **批量提交生效（v0.2）**：列表支持多选草稿态对象，toolbar 展示「批量提交生效」；提交失败项保留 `draft` 并给出逐条错误，成功项转 `ready`；批量提交不阻塞已成功的项。
...
> **Change Log 定位**：本表为业务沟通决策的精简记录（**保留最近 3 版**一句话摘要）；**完整历史（v3.18 及以前的逐版变更详情）已迁移至 `docs/05-execution-records/module-01/design-decisions.md`「Change Log（完整历史）」小节**。Change Log 主要记录业务侧沟通决策与文档变更，**不承载开发契约**（开发契约见 5.x 数据模型 / 9 验收标准 / 10 术语映射）。
...
> 完整 Change Log 历史（v3.18 及以前）见 `docs/05-execution-records/module-01/design-decisions.md`「Change Log（完整历史）」。