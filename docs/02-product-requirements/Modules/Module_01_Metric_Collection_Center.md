...
| 规则文件挂载 | MVP 规则管理入口（承载于独立「规则编辑」页，导航位于「指标库」之后）：上传 / 粘贴完整 `rules.yml`（**整文件透传**，`content_mode=yaml_passthrough`）→ YAML 语法校验 → **job 引用语义校验（决策 66）：解析规则表达式中 `job="..."` / `job=~"..."` matcher 与当前生效 `ScrapeJob` 列表比对，`up` / `absent(up)` 类不匹配 = error、其他 job 引用不匹配 = warning；编辑期仅提示、不阻断保存** → 落库 `MonitoringRule`（`draft_status=ready`）→ 触发 M09 变更检测，由 M09 生成 / 确认 / 下发 `rules.yml`（**人工确认档**，对齐决策 38-1）——**保证规则变更与采集 Job 一样走 M09 统一配置下发闭环，不绕过 M09**；列表展示规则名 / 规则条数 / 更新时间 / 启用状态 / 下发状态（`change_status`）；支持启停 / 删除 / 详情（YAML 只读视图）；**不做字段级 PromQL 创作编辑**（v0.3 升级 `structured` 字段化表单，见 3.2） | P0         |
...
> - **M01 编辑期校验（提示不阻断）**：规则保存/编辑时即解析 `job="..."` / `job=~"..."` matcher，与当前生效 `ScrapeJob` 列表比对；`up` / `absent(up)` 类不匹配 = error、其他不匹配 = warning；error/warning 均行内提示，**不阻止保存**，兼容「先挂规则、后建 Job」的合法流程。
> - **M09 发布期校验（强制门禁）**：规则进入 M09 变更单后，`rules.yml` 生成完毕会按**同一套判定逻辑**复验；`up` / `absent(up)` 类不匹配 = error，**阻断确认发布**；其他不匹配 = warning，允许确认但高亮提示。
> - **推荐动线为先 Job 后规则，但不强制**：规则编辑页/校验提示文案引导用户「建议先创建对应采集 Job」；若用户选择先挂规则，仅触发 warning，待 Job 创建后发布即可通过。
...
| PromQL 校验 + job 引用校验 | 调用 [Module\_02: 查询中心](Module_02_Query_Center.md) 或 Prometheus 接口校验表达式语法；**同时按决策 66 校验 `job` matcher 与当前生效 `ScrapeJob` 列表的匹配情况**（`up`/`absent(up)` 不匹配 = error，其他不匹配 = warning；编辑期提示不阻断保存） | P0 / v0.3 |
