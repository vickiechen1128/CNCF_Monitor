...
| :--- | :--------- | :--- | :---------------------------------------------------------------------------------------------------------------- | :---------------- | :-- |
| v3.1 | 2026-08-12 | 修改   | 标签模板创建引导与展示列（第九轮需求讨论，与 Module_07 v2.5 对齐）：CI-Exporter 映射页新增标签模板创建引导（检测无模板→提示创建→预填推荐映射）；映射列表新增「标签模板」展示列（查看/更换/补配）；ScrapeJob 标签模板引用体验增强（卡片式选择器+内联预览+更换操作）；5.1 新增 has_label_template 字段；验收标准与原型同步 | MVP / v0.2 / v0.3 / v1.0 | 设计中 |
