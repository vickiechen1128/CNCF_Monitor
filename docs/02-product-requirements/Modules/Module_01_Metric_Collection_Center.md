...
> **原型版本**: v3.37（⚠️ v3.40 待原型同步：规则挂载「提交生效」需按决策 67-2 呈现 error 默认阻断 + 显式覆盖逃生门）
> **更新日期**: 2026-09-10
...
| 规则文件挂载 | MVP 规则管理入口（承载于独立「规则编辑」页，导航位于「指标库」之后）：上传 / 粘贴完整 `rules.yml`（**整文件透传**，`content_mode=yaml_passthrough`）→ YAML 语法校验 → **job 引用语义校验（决策 66）：解析规则表达式中 `job="..."` / `job=~"..."` matcher 与当前生效 `ScrapeJob` 列表比对，`up` / `absent(up)` 类不匹配 = error、其他 job 引用不匹配 = warning；error 默认阻断「提交生效」/「保存变更」并展示逐条问题清单（用户勾选「已知晓：先挂规则，稍后补建 Job」显式覆盖后方可放行，降级为 warning 落库留痕）、warning 仅行内提示** → 落库 `MonitoringRule`（`draft_status=ready`）→ 触发 M09 变更检测，由 M09 生成 / 确认 / 下发 `rules.yml`（**人工确认档**，对齐决策 38-1）——**保证规则变更与采集 Job 一样走 M09 统一配置下发闭环，不绕过 M09**；列表展示规则名 / 规则条数 / 更新时间 / 启用状态 / 下发状态（`change_status`）；支持启停 / 删除 / 详情（YAML 只读视图）；**不做字段级 PromQL 创作编辑**（v0.3 升级 `structured` 字段化表单，见 3.2） | P0         |
...
> - **M01 编辑期校验（fail-fast，error 默认阻断 + 显式覆盖）**：规则保存/编辑时即解析 `job="..."` / `job=~"..."` matcher，与当前生效 `ScrapeJob` 列表比对；`up` / `absent(up)` 类不匹配 = error、其他不匹配 = warning。**error 默认阻断「提交生效」/「保存变更」**，行内展示逐条问题清单（规则名 / matcher / 引用 job / 说明）；用户勾选**逃生门**「已知晓：先挂规则，稍后补建 Job」显式覆盖后方可放行，问题降级为 warning 落库留痕；**warning 仅行内提示、不阻断**。逃生门即决策 66 原「先挂规则、后建 Job」合法时序的保护通道——从「默认放行」改为「显式确认」。
...
| PromQL 校验 + job 引用校验 | 调用 [Module\_02: 查询中心](Module_02_Query_Center.md) 或 Prometheus 接口校验表达式语法；**同时按决策 66 校验 `job` matcher 与当前生效 `ScrapeJob` 列表的匹配情况**（`up`/`absent(up)` 不匹配 = error，其他不匹配 = warning；**error 默认阻断「提交生效」+ 逃生门显式覆盖，warning 仅提示**，决策 66 / 67-2） | P0 / v0.3 |
...
- **规则挂载「提交生效」分级门（决策 66，经决策 67-2 修订）**：规则 job 引用校验 `error` 级（存活类 `up`/`absent(up)` 规则缺 job）时**默认阻断**「提交生效」/「保存变更」并展示逐条问题清单（规则名 / matcher / 引用 job / 说明）；提供**逃生门复选框**「已知晓：先挂规则，稍后补建 Job」，勾选后放行并将问题降级为 warning 落库留痕；`warning` 级仅行内提示不阻断；YAML 语法错误仍硬阻断。提示需按 severity 分级呈现（error 红 / warning 黄）。
...
| v3.38 | 2026-09-09 | 修改 | 规则粒度与模块边界对齐（决策 65）：§3.2 新增「规则粒度说明」注记与 per-instance 规则模板行（P1/v0.3）；§5.5 新增「规则粒度与前端展示」对照表——聚合规则（如 `absent(up{job=...})`）无 instance 标签，告警 instance 列显示「全局/聚合」，per-instance 规则（如 `up{job=...} == 0`）携带 instance 标签，显示故障实例 IP/名称；规则编辑入口归本模块（MVP yaml_passthrough，v0.3 字段化），per-instance 可视化编辑归 v0.3 规则 UI | 3.2 / 5.5 | 文档自身 | 设计中 |

> 完整 Change Log 历史（v3.37 及以前）见 `docs/05-execution-records/module-01/design-decisions.md`「Change Log（完整历史）」。
