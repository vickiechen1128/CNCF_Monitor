...
| 实例地址解析不出不得静默放行 | 生成目标文件时，资源不存在（`resource_not_found`）/ 地址为空（`address_empty`）/ 实例下线（`offline`）须记录跳过归因（精确到 ResourceID + 原因），**不得静默跳过**；空 targets 由 [3.2.3](#323-下发前校验与失败出口) 校验层判非法（产物仍生成、校验不放行），避免缺陷配置下发后边缘永久卡死「同步中」 | P0  | MVP  |
...
| **channel**             | **enum** | **✅** | **下发通道（只读）** | 配置下发通道：`local`（中心同机写盘 reload）/ `agent_pull`（Edge Sync Agent 心跳拉包）；由 [Module\_11](Module_11_Edge_Access_and_Agent_Delivery.md) 纳管按 `ChannelForDomainType(domain_type)` 写入（**由 `domain_type` 派生、为单一事实来源**），本模块只读消费以决定产物形态与下发方式（见 [6.4.1](#641-下发通道与配置产物形态)）；MVP 按网域固定（`default` 域 = `local`，其他网域 = `agent_pull`），**`domain_type` 未知时不得回退 `local`**（历史 bug 根因）；同一网域混合通道与通道切换为 {v0.4+} 演化场景 |
...
- **MVP 边界**：一个网域同一时刻只有一种下发通道、对应一个逻辑采集组；`channel` **由 `domain_type` 派生**（`management`→`local`、`edge`→`agent_pull`），不提供通道切换入口、亦非独立可设字段；**`domain_type` 未知时按契约不得回退 `local`**（避免边缘域被误判为直连域的历史 bug）；同一网域内不混合通道（`local` 与 `agent_pull` 并存属 {v0.4+} 演化）。
...
> 本表为业务沟通决策的精简记录，保留最近 3 版一句话摘要；更早版本（v1.78 及以前）的完整历史见 `docs/05-execution-records/module-09/design-decisions.md`「Change Log（完整历史）」小节。
