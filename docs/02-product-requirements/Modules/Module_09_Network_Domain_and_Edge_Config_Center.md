...
|------|------|----------|----------|--------------|------|
| v1.25 | 2026-08-13 | 修改 | 变更摘要生成机制补充「标签模板变更话术」（决策 22 对象对齐）：change_items.target=标签模板 时的人话摘要示例与风险等级建议（high），与 Module_07 v2.7「被引用 Job / 保存后影响提示」联动 | MVP / v0.2 | 设计中 |
| v1.24 | 2026-08-07 | 新增 | PRD 骨架细化落地：新增 4.8 数据模型状态机（集中定义 ConfigDraft / ConfigDeployment / config_sync_status / NetworkDomain 状态流转）；9.1/9.2 验收标准加 P0/P1/P2 标注；第 2 章用户故事改为引用全局库（01_User_Stories.md）；Change Log 精简（完整历史迁移 design-decisions.md） | MVP / v0.2 | 设计中 |
| v1.23 | 2026-08-07 | 新增 | 按 PRD 骨架规范补齐：4.1~4.6 字段表加「UI 展示名」列、新增第 10 章「术语映射（用户词汇表）」、验收标准分层（9.1 用户 / 9.2 技术）、4.6 补漏 validation_status / includes_blackbox | MVP / v0.2 | 设计中 |
| v1.22 | 2026-08-07 | 修改 | 决策 22：变更对象 = 源数据对象统一枚举 + 「影响的配置文件」派生列、变更单级确认、全链路关联（change_no → cv → deploy 双向可追溯 + 回滚入口） | MVP / v0.2 | 设计中 |
