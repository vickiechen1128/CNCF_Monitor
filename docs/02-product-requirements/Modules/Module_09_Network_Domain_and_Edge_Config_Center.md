...
- **草稿状态过滤（{v0.2}/{v0.3}）**：候选集过滤条件以 [6.8.2](#682-文件分工与生成规则) 为权威（`draft_status=ready` 且 `enabled=true`）；本层要点是触发语义——草稿（`draft_status=draft`）或停用（`enabled=false`）对象不参与生成，其 `updated_at` 变化不会触发有效配置变更；但 M09 仍在 `source_data_version` 聚合中感知其变化，生成空跑后通过 checksum 裁决丢弃（空变更抑制），避免草稿对象在确认页产生噪音。
...
| 状态（用户可见标签） | 枚举值（代码）    | 含义                       | 进入条件                                                                 | 后续流转（引导）                                                    |
| ----------- | ----------- | ------------------------ | ---------------------------------------------------------------------- | ------------------------------------------------------------- |
| 已同步       | `in_sync`   | 边缘实际生效版本与中心版本一致          | Agent 拉包生效后 checksum 校验一致                                               | 稳定态，纯展示                                                      |
| 未同步       | `out_of_sync` | 中心版本与边缘生效版本不一致           | 按 `out_of_sync_cause` 区分成因：`pending_draft` / `pull_pending` / `local_reset` / `rollback_diverged` | 按成因引导：前往配置确认 / 查看下发记录 / 立即同步（`force_pull`）/ 前往 M01、M08 核对 |
| 未知         | `unknown`   | 尚未获得边缘生效版本信息             | Agent 未上线或尚未首次上报                                                        | 首次上报后转入其余四档                                                   |
| 人工覆盖       | `manual_override` | 边缘本地手工修改过配置，平台不强制回拉 | 边缘本地手工修改                                                                | 需用户在 UI 重新确认并下发以恢复平台一致性                                       |
| 未下发配置      | `no_version` | Agent 已上线但网域尚无成功下发过的 `ConfigVersion` | 网域无成功下发记录                                                               | 引导「去配置采集 Job」（跳 M01 采集 Job 页）                                 |
...
| ----------- | ------- | ----------- | ------------ | ----------------------- |
| 未知         | `unknown` | 尚未获得心跳信息    | Agent 未上线或未首次心跳 | 首次心跳 → 在线             |
| 在线         | `online`  | Agent 心跳正常  | Agent 心跳上线    | 失联超阈值 → 离线（触发 `EdgeSiteOffline` 告警） |
| 离线         | `offline` | Agent 失联     | 失联超过阈值        | 心跳恢复 → 在线               |
