...
> **PRD 状态**: `dev-ready`（Track B 增量 v1.3，2026-09-14 用户书面确认；原型验证豁免，记录见 `docs/05-execution-records/module-05/design-decisions.md`「Track B 豁免记录」）
> **PRD 版本**: v1.3
> **产品版本覆盖**: MVP / v0.3 / v1.0
> **原型版本**: v1.1（未对齐，待按 v1.3 修订：首页 MVP 子集——快速入口 / 使用指引 / 告警状态数字——需原型承载）
> **更新日期**: 2026-09-14
...
- **{MVP} 首页 MVP 子集（决策 72）**：**系统快速入口**（核心功能页快捷卡片）、**使用指引**（开箱动线：登记网域 → 导入资源 → 建采集 Job → 下发 → 查指标，第 5 步深链 `/query` 查询页）、**告警状态数字**（以 AM 治理态为主数字、Prom firing/pending 为辅，详见 §3.1「首页（MVP 子集）交互契约」）。本阶段不做轻量实时图表，不提供「进入可视化大屏」快捷入口（豁免至 v0.3）。
...
| 首页 / Dashboard | 平台默认落地页。**{MVP}（决策 72）**：系统快速入口（核心功能页快捷卡片）+ 使用指引（开箱动线：登记网域 → 导入资源 → 建采集 Job → 下发 → 查指标，决策 51 动线）+ 告警状态数字（AM 治理态为主、Prom firing/pending 为辅，带空态引导）；保留既有平台统计卡（资源总数 / 待确认草稿 / 已纳管网域 / 最近下发）。**{v0.3}（决策 51）**：升级概览 Dashboard（轻量实时图表、采集覆盖率聚合卡片），概览页顶部提供显眼的「进入可视化大屏」快捷入口（卡片 / 按钮） | Module 01/06/08/09/10（数据） | P0（MVP 子集）/ P1（v0.3 升级） |
...
#### 首页（MVP 子集）交互契约（决策 72）

- **页面结构**（自上而下）：既有平台统计卡（资源总数 / 待确认草稿 / 已纳管网域）→ **告警状态数字区** → **系统快速入口区** → **使用指引区** → 既有「最近下发」表。
- **告警状态数字**：
  ...
  - **空态引导**：未挂载 AM 通知配置时 `alerting:` 段不生成（决策 68-2：中心求值器在「AM 配置已挂载 + `--alertmanager.url` 可解析」时才生成投递配置），AM 数字恒为 0——此时卡片展示「尚未挂载通知配置，去配置 →」深链 M08 告警配置，把 0 转化为引导动线而非误导。
- **系统快速入口**：核心功能页快捷卡片（资源管理 / 采集 Job / 配置预览下发 / 指标查询 / 告警状态），入口集合以 MVP 已上线页面为限。
- **使用指引**：五步开箱动线（登记网域 → 导入资源 → 建采集 Job → 下发 → 查指标，决策 51 动线提前至 MVP），每步配深链与完成态标识；**第 5 步「查指标」深链 `/query` 查询页**——配套要求：`QueryPage` 组件挂入 `/query` 路由（组件已存在，本需求一并落地）。
...
- [ ] {MVP，决策 72，P0} 首页提供系统快速入口区（资源管理 / 采集 Job / 配置预览下发 / 指标查询 / 告警状态快捷卡片），点击可正确跳转对应页面
- [ ] {MVP，决策 72，P0} 首页提供使用指引区，按五步开箱动线（登记网域 → 导入资源 → 建采集 Job → 下发 → 查指标）展示并配深链；第 5 步「查指标」深链 `/query` 查询页（QueryPage 已挂路由）
- [ ] {MVP，决策 72，P0} 首页展示告警状态数字：主数字为 AM 治理态「通知中」，次要行为已静默 / 已抑制，并列展示 Prom firing/pending；数据来自前端直调 `/api/v1/alerts` 与 `/api/v2/platform/alertmanager/alerts` 计数，无新增后端接口；未挂载 AM 通知配置时展示空态引导（「尚未挂载通知配置，去配置 →」深链 M08）
...
> 完整 Change Log 历史（v1.0 ~ v1.1 早期）见 `docs/05-execution-records/module-05/design-decisions.md`「Change Log（完整历史）」。
