...
> **PRD 状态**: `dev-ready`（Track B 增量 v1.6，2026-09-18 用户确认首页视觉密度与告警卡重构意见（决策 73）；原型验证豁免，记录见 `docs/05-execution-records/module-05/design-decisions.md`「Track B 豁免记录」与「决策 72 / 73」）
> **PRD 版本**: v1.6
> **产品版本覆盖**: MVP / v0.3 / v1.0
> **原型版本**: v1.3（**已对齐 v1.6（决策 73）**：指标卡纵向排版 30px 数字 + 44px 图标容器 / 告警卡四格统计条（当日 / 近 7 天为主数字）+ 最新 8 条两行制列表（含告警内容）/ 快捷入口一行五列压扁 / 使用指引移入右列；原型入口 `docs/prototypes/module-05/`）
> **更新日期**: 2026-09-18
...
- **{MVP} 首页 MVP 子集**：**系统快速入口**（核心功能页快捷卡片，一行五列）、**使用指引**（六步开箱动线：登记网域 → 导入资源 → 建采集 Job → 下发 → 配置告警通知 → 查指标/看告警，第 6 步深链 `/query` 自定义查询页或 `/alert-status`，置于首页右列）、**关键指标卡（6 张纯资产/治理进度口径：资源总数 / 已监控 / 采集 Job / 已纳管网域 / 待确认草稿 / 采集覆盖率，每张右上角 ⓘ 中文口径注释，见 §3.1）**、**告警状态卡（当日 / 近 7 天告警数量为主数字、AM 治理态并列，最新 8 条两行制告警列表含告警具体内容，首页唯一告警入口，见 §3.1）**。本阶段不做轻量实时图表，不提供「进入可视化大屏」快捷入口（豁免至 v0.3）。
...
| 网域管理页 | 网域注册、Token 管理、Edge Agent 状态、租户-网域关联展示 {v0.2} | Module 11 | P0（v0.2） |
| 监控源登记册页 | 外部 Prometheus / Zabbix / 云监控接入管理 | Module 10 | P0（集成模式） |
| 首页 / Dashboard | 平台默认落地页。**{MVP}（决策 72 / 72-1 / 72-2 / 72-3 / 73）**：系统快速入口（核心功能页快捷卡片，一行五列）+ 使用指引（六步开箱动线：登记网域 → 导入资源 → 建采集 Job → 下发 → 配置告警通知 → 查指标/看告警，决策 72-1，置于右列）+ 关键指标卡（6 张资产/治理进度口径）+ 告警状态卡（当日 / 近 7 天告警数量为主数字、AM 治理态并列，最新 8 条两行制列表含告警具体内容，带空态引导）。**{v0.3}（决策 51）**：升级概览 Dashboard（轻量实时图表、采集覆盖率聚合卡片），概览页顶部提供显眼的「进入可视化大屏」快捷入口（卡片 / 按钮） | Module 01/06/08/09/10（数据） | P0（MVP 子集）/ P1（v0.3 升级） |
...
- **页面结构**（自上而下）：
  ...
  2. **关键指标卡区**：6 张纯资产/治理进度卡片（资源总数 / 已监控 / 采集 Job / 已纳管网域 / 待确认草稿 / 采集覆盖率），6 列网格（`Row gutter={[16,16]}`、`xl=4`），每张右上角 `ⓘ` 中文口径注释；**不含任何告警数字**（告警表达全部收敛到告警状态卡，见第 3 项）；卡内纵向排版（标签 12px 在上、数字 30px/800 在下、44px 圆角图标容器右下，内 22px 图标品牌青 `#0ECDEB`、容器品牌青 10% 浅底），提高数字醒目度。
  3. **告警状态卡**（左侧约 40% 宽，首页唯一告警入口）：顶部为**四格统计条**（当日告警 / 近 7 天告警为主数字 24px/700，通知中（红色语义）/ 已静默·已抑制为次，格间竖分隔线，每格悬浮显示口径注释）；下方展示**最新 8 条两行制告警列表**（行 1：级别浅底 Tag + 告警名 + 相对时间 + 状态；行 2：实例名 · 告警具体内容 12px 灰字，`resource_name` 为空显示 `-`；行间细分隔线，超长省略悬浮全文）；底部 11px 灰字参考行「Prometheus 原始求值 · 触发中 N / 求值中 M」（仅参考，不作行动对象）+ 标题右侧「查看全部 →」深链 `/alert-status`（附次级入口「配置告警通知」）。
  4. **系统快速入口区**（右列第一张卡）：5 个核心功能页快捷入口（资源管理 / 采集 Job / 配置预览下发 / 指标查询 / 告警状态）**一行五列压扁布局**（28px 圆角图标容器 + 名称 13px，一句描述移入悬浮提示），hover 态高亮。
  5. **使用指引区**（右列第三张卡，决策 73 自底部整行移入）：六步开箱动线 Steps 组件（登记网域 → 导入资源 → 建采集 Job → 下发 → 配置告警通知 → 查指标/看告警），每步配深链；第 5 步深链 `/alert-config`，第 6 步深链 `/query` 或 `/alert-status`。
  6. **最近下发记录表**（右列第二张卡）：继续展示 `dashboard.summary.recent_deployments` 前 5 条，表格列精简为「变更单号 / 网域 / 状态 + 时间」；标题右侧加「查看全部 →」深链 `/deployments`。右列自上而下为：系统快速入口 → 最近下发记录 → 使用指引，与左列告警状态卡高度对齐。
- **关键指标卡口径**：
...
- **告警状态数字**：
  - **主数字 = 当日告警 / 近 7 天告警**（决策 73：用户最关心「最近告警多不多」）；AM 治理态「通知中」（`active`，红色语义）与「已静默 · 已抑制」作为次级数字并列四格统计条；Prom 当前触发（firing / pending）**仅作 11px 灰字参考行**（触发中 / 求值中），不作行动对象。治理层是用户主视图，点击跳转 `/alert-status`。
  - **数据来源**：前端直调既有只读接口计数——Prom 触发告警接口（firing/pending）、AM 通知状态接口（通知四态，见 §6）与 M02 历史告警接口 `/api/v1/alerts/history`（当日 = 今日 0 点起、近 7 天 = `fired_at ≥ now-7d`，均含已恢复，按 `fired_at` 前端计数；告警列表行 2 消费 `summary` 字段展示告警具体内容）。[待确认 M02] history 接口是否支持时间范围参数（若支持可服务端过滤，减少前端全量拉取）；[待确认 M08] AM alerts 代理是否透传 `annotations`（若透传，列表行 2 可优先用 AM 实时数据）。history 取数失败时当日 / 近 7 天两格显示 `-`（不显示 0）。门户侧在 `platform/dashboard/summary` 新增 `monitored_count` / `scrape_job_count` / `scrape_job_enabled_count` 三个计数字段（增字段、向后兼容），供指标卡 2/3 使用；接口语义不变，契约快照见 `api-contract-snapshot.md`。
...
- **系统快速入口**：核心功能页快捷入口（资源管理 / 采集 Job / 配置预览下发 / 指标查询 / 告警状态），一行五列压扁布局（图标 + 名称，描述悬浮提示），入口集合以 MVP 已上线页面为限。
- **使用指引**（右列第三张卡，决策 73 自底部整行移入）：六步开箱动线：登记网域 → 导入资源 → 建采集 Job → 下发 → **配置告警通知** → 查指标/看告警；每步配深链与完成态标识；**第 5 步「配置告警通知」深链 `/alert-config`；第 6 步「查指标 / 看告警」深链 `/query` 自定义查询页或 `/alert-status`**。配套要求：`QueryPage` 组件挂入 `/query` 路由并补齐最小 PromQL 查询能力（输入框 + 结果表格/JSON，调用 M02 `/api/v1/query` 代理，不复用 Prometheus UI）。
- **视觉 Token 规范**：
  - 色彩收敛：品牌青 `#0ECDEB` 仅用于主按钮 / 关键图标 / 链接；功能色（成功绿 / 告警红 / 警告橙）仅用于状态语义，不用于装饰性图标；**指标卡装饰图标统一品牌青，红色语义收敛到告警状态卡**。
  - 卡片规范：圆角 8px、默认阴影 `0 1px 2px rgba(0,0,0,0.06)`、hover 阴影 `0 4px 12px rgba(0,0,0,0.08)`、指标卡内边距 `14px 16px`。
  - 间距体系：页面内边距 24px；卡片 gutter 16px；标题与内容间距 12px。
  - 字体层级：页面标题 `Title level={3}`；区块标题 `Title level={5}`；指标卡数字 30px/800、统计条数字 24px/700（等宽数字 `tabular-nums`）；辅助说明 14px `colorTextSecondary`。
  - 图标尺寸：指标卡 44px 圆角容器（内 22px 图标）、快捷入口 28px 圆角容器（内 16px 图标），容器统一品牌青 10% 浅底，统一使用 `@ant-design/icons`。
  - 指标卡卡内排版：标签在上、大数字在中下、图标容器右下（纵向，决策 73），30px/800 数字比横向排版更醒目；快捷入口一行五列压扁、使用指引移入右列，右列（入口 + 下发 + 指引）与左列告警状态卡高度对齐。
...
首页告警状态卡采用前端直调计数，数据来自三个只读接口的数组长度计数：`/api/v1/alerts`（Prom firing/pending）、`/api/v2/platform/alertmanager/alerts`（AM 通知四态）与 `/api/v1/alerts/history`（M02 历史告警，当日 / 近 7 天计数与列表行 2 告警内容）。

- **主数字**：当日告警 / 近 7 天告警（M02 history 按 `fired_at` 计数，含已恢复）；AM 治理态「通知中」（`active`，红色语义）与「已静默 · 已抑制」为次级数字；Prom firing/pending 仅作 11px 灰字参考行（触发中 / 求值中），不作行动对象。
...
- **服务降级**：数据取数失败时对应数字显示 `-`（不显示 0；history 失败仅影响当日 / 近 7 天两格，AM / Prom 各自独立降级）；单链路失败 warning 局部降级、双链路失败 error + 重试。
...
- [ ] {MVP，P0} 首页提供系统快速入口区（资源管理 / 采集 Job / 配置预览下发 / 指标查询 / 告警状态快捷入口），一行五列压扁布局（图标 + 名称，描述悬浮提示），点击可正确跳转对应页面
- [ ] {MVP，P0} 首页提供使用指引区（右列第三张卡），按六步开箱动线（登记网域 → 导入资源 → 建采集 Job → 下发 → 配置告警通知 → 查指标/看告警）展示并配深链；第 5 步深链 `/alert-config`，第 6 步深链 `/query` 或 `/alert-status`
- [ ] {MVP，P0} 首页展示告警状态卡：四格统计条主数字为当日告警 / 近 7 天告警，次级数字为通知中（红色）/ 已静默·已抑制，Prom firing/pending 仅参考行；数据来自前端直调 `/api/v1/alerts`、`/api/v2/platform/alertmanager/alerts` 与 M02 `/api/v1/alerts/history` 计数，无新增后端接口；未挂载 AM 通知配置时展示空态引导（「尚未挂载通知配置，去配置 →」深链 `/alert-config`）
- [ ] {MVP，P0} 首页按视觉 Token 规范呈现：卡片圆角 8px、统一阴影与间距、图标尺寸统一、色彩收敛（品牌青仅用于主按钮/关键图标，功能色仅用于状态语义）
- [ ] {MVP，P0} 首页关键指标卡为 6 张纯资产/治理进度口径（资源总数 / 已监控 / 采集 Job / 已纳管网域 / 待确认草稿 / 采集覆盖率），**不出现任何告警数字**；每张卡右上角 ⓘ 可读出中文口径注释；卡内纵向排版（标签在上、数字 30px/800 在下、44px 图标容器右下），6 列网格
...
- [ ] {MVP，P0} 告警状态卡为首页唯一告警入口：四格统计条（当日告警 / 近 7 天告警 / 通知中 / 已静默·已抑制，每格悬浮口径注释）；展示最新 8 条两行制告警列表（行 1：级别浅底 Tag + 告警名 + 相对时间 + 状态；行 2：实例名 · 告警具体内容 summary），`resource_name` 为空显示 `-`，超长省略悬浮全文；Prom firing/pending 仅作 11px 灰字参考行（触发中 / 求值中），页面不出现「待处理」字样
- [ ] {MVP，P0} 当日告警 = 今日 0 点起触发过的告警条数（含已恢复），近 7 天告警 = `fired_at ≥ now-7d`（含已恢复），均按 M02 `/api/v1/alerts/history` 的 `fired_at` 前端计数；history 取数失败时两格显示 `-`（不显示 0）
- [ ] {MVP，P0} 告警列表行 2 展示「实例名 · 告警具体内容（summary）」，级别 Tag 采用浅底色 + 深字（critical / warning / info 三档浅底配色）
...
| AM 治理态「通知中」 | Alertmanager 通知状态 `active`，首页告警状态卡四格统计条第三格（红色语义） |
| 当日告警 | 今日 0 点起触发过的告警条数（含已恢复），按 M02 `/api/v1/alerts/history` 的 `fired_at` 前端计数，首页告警状态卡主数字 |
| 近 7 天告警 | `fired_at ≥ now-7d` 的告警条数（含已恢复），数据来源与当日告警相同，首页告警状态卡主数字 |
...
布局与列设计：自上而下为标题区、关键指标卡区（6 列网格）、双列区——左列告警状态卡（四格统计条 + 最新 8 条两行制告警列表），右列系统快速入口（一行五列）→ 最近下发记录表 → 使用指引（六步开箱动线）。

关键交互规则：6 张指标卡右上角 `ⓘ` 提供中文口径注释，四格统计条每格悬浮口径注释；告警卡标题右侧「查看全部 →」深链 `/alert-status`，最近下发记录表标题右侧深链 `/deployments`，两块列表不引入分页器；快捷入口卡片 hover 态高亮；空态时引导配置动作。
