...
> **PRD 状态**: `设计中`（v2.24 决策 52 已落版、原型已对齐 v2.24）
> **PRD 版本**: v2.24
> **产品版本覆盖**: MVP / v0.2 / v0.4 / v1.0
> **原型版本**: v2.24（✅ 已对齐 v2.24）
> **更新日期**: 2026-08-31
...
> - **v0.2 起 `network_domain` 列可留空（决策 52）**：留空时由 M06 `NetworkDomain.ip_cidrs` 规则表按 IP 推导归属（最长前缀优先）；推导失败或歧义（同网段跨网域重复）时该行导入报错并在结果清单标注「网域未决」，由用户在导入结果页人工指派或补充网段规则后重导；显式填写网域的列优先于推导（显式 > 推导）。MVP 不变：`network_domain` 必填。完整解析链见 5.16.4。
...
#### 5.16.4 网域归属解析链（v0.2 / v0.4，决策 52）
...
| ③ | **IP 段推导**：按资源 IP 匹配 M06 `NetworkDomain.ip_cidrs`，**最长前缀优先** | Excel 导入（列留空）/ 同步源无拓扑字段 | v0.2 |
| ④ | **待分配队列 + 规则化**：推导失败或歧义（同网段跨网域重复判歧义、不猜测）的资源进入「待分类」队列的「网域未决」类型；v0.2 支持**规则化**动作——按未决 IP 聚合出候选 CIDR（默认 /24，可编辑前缀长度），一键追加到目标网域的 `ip_cidrs`，追加后触发重推导，命中则从未决队列移除；剩余仍未命中或歧义者人工一次性指派 | 兜底 | v0.2（导入报错标注 + 规则化动作）/ v0.4（队列） |
...
- **规则化（决策 55，v0.2）**：「待分配」队列不应成为持续的人力黑洞。对未决 IP 按**相同 /24 段聚合计数**，给出候选 CIDR（支持用户调整前缀长度），确认后**一键追加到 M06 目标网域的 `ip_cidrs`**；追加后自动触发该批资源的重推导，命中者从未决队列移除。规则化只生成平台侧 `ip_cidrs` 数据，**不回写 CMDB**。
- **未匹配 IP 对账视图（v0.2，决策 55）**：资源管理页提供「网域未决」看板，展示近 7 天未匹配 IP 的 TopN 聚合（按候选 CIDR / 出现次数），辅助管理员发现规则缺口；高频出现的未决段优先通过「规则化」收敛为 `ip_cidrs`。
...
- [ ] {v0.2} Excel 导入 `network_domain` 列可留空，由 M06 `NetworkDomain.ip_cidrs` 规则自动推导归属（最长前缀优先）；推导失败/歧义的行在导入结果中标注「网域未决」并给出可执行指引（决策 52）
- [ ] {v0.2} 「网域未决」队列支持按 IP 聚合候选 CIDR（默认 /24，可调整前缀），一键追加到目标网域的 `ip_cidrs` 并触发重推导；提供近 7 天未匹配 IP 的 TopN 对账视图（决策 55）
...
| ---- | ---------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | --- |
| v2.24 | 2026-08-31 | 新增 | 决策 52 落版（网域归属解析链）+ 决策 55 落版（未决队列规则化 + 未匹配 IP 对账）：①§5.4 新增「网域心智原则与归属解析链」——网域 = 部署拓扑属性（非资产属性），「接入可见、消费隐藏」，四级优先级链（CMDB 字段映射 > 同步通道绑定 > IP 段推导 > 待分配队列），平台侧数据不回写 CMDB；②新增 §5.16.4「网域归属解析链」——四级链细则、显式优先、冲突告警、IP 推导边界（私有地址段重叠判歧义）、多 IP/无 IP 处理、blackbox 目标例外（拨测网域 = 发起侧）；§5.16.4 第④级补「规则化」动作（按未决 IP 聚合候选 CIDR 追加到 `ip_cidrs`）与「近 7 天未匹配 IP TopN 对账视图」；③§5.16.1 补 v0.2 起 `network_domain` 列可留空由 IP 推导、推导失败标注「网域未决」；④§5.16.2 补「网域启用态」校验、§11.2 补资源新增/编辑表单「网域」字段必须是 Select（仅启用态网域可选）、禁止自由输入，M06 接口同步消费说明；⑤§9.1 新增 v0.2 推导、v0.4 同步映射及决策 55 规则化/对账验收；⑥产品版本覆盖补 v0.2；原型待对齐 | 5.4 / 5.16 / 9.1 / 11.2 | v0.2 / v0.4 | 设计中 |
