...
| v2.35 | 2026-09-15 | 修改 | **新增入口收敛与采集参数归位（决策 84）**：①§3.1 登记入口收敛——资源新增入口 5 项、与列表 Tab 1:1，「登记 K8s 集群」不再占类型入口（集群是部署形态而非资源类型），收编为「其他监控目标」表单首问「登记对象」的 K8s 集群子动线（{v2.36} 决策 85 入口形态回归单按钮，分流实质保留）；②「高级采集设置」彻底删除——端口 / 采集路径 / 协议不在 M07 表单与详情抽屉出现，采集参数归 M01 默认采集配置统一承载（解析链：Job 覆盖 → CITypeExporterMapping → ExporterTemplate），消除与 M01 的双头维护，实例偏差正规出口 = M01 映射端口编辑（MVP）/ `Resource.scrape_port`（{v0.2}）；③§5.9 字段表删除 port / metrics\_path / scheme 三行、「端点类型」改「登记对象（首问）」且不再联动带出采集参数；④§9 验收与 §11.2 行为规则同步（首问分流 / 采集参数不出现）；⑤原型同步 v2.35（预设精简为 key / label / exporter\_type 判别值）。不改 M07 数据模型与接口契约 | 3.1 / 5.4 / 5.9 / 9.1 / 11.2 | MVP 生效 | ready |
