...
| **Module_01 监控策略与指标管理** | CI↔Exporter 模板绑定；ScrapeJob；实例选择；Blackbox 拨测；规则编辑 UI；静态指标库 | 网域级覆盖表 `CITypeExporterMappingOverride`（按网域差异化默认采集参数）；参数继承/同步（创建时快照 + 手动「同步映射默认值」） | - | - | Recording Rules；指标库管理增强 |
